...
import {normalizeMeasurementText} from './material-measurement-text'
...
export function materialCleanLine(input: { name: string; quantity: string | number; unit: string; fields: RequestItemField[]; details: string }) {
  ...
  const measurement=(id:string)=>normalizeMeasurementText(input.fields.find(f=>f.id===id)?.value||'').match(/^\s*(\d+(?:\.\d+)?)\s*(in|ft)\s*$/i)
  ...
  const canonical=(text:string)=>normalizeMeasurementText(text).toLowerCase().replace(/(\d+)[ -]+(\d+)\/(\d+)/g,(_,whole,n,d)=>String(Number(whole)+Number(n)/Number(d))).replace(/\b(\d+)\/(\d+)\b/g,(_,n,d)=>String(Number(n)/Number(d))).replace(/(\d)\s*(ft|inches|inch|in)\b/g,'$1 $2').replace(/\b(?:series)\b/g,'').replace(/[^a-z0-9]+/g,' ').trim().replace(/\s+/g,' ')
