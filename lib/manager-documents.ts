...
export type ManagerDocumentStatus =
  "processing" | "needs_review" | "ready" | "routed" | "archived" | "error";
...
export type ManagerDocumentItemRecord = {
  ...
  selected: boolean;
  matched_common_material_rule_id: string | null;
  matched_catalog_item_id: string | null;
  match_method: string | null;
  match_confidence: number | null;
  catalog_import_status:
    "not_requested" | "pending_review" | "imported" | "failed";
};
...
export function managerDocumentTypeLabel(type: ManagerDocumentType) {
  return (
    ...
    } satisfies Record<ManagerDocumentType, string>
  )[type];
}
