import {measurementNumber} from '@/lib/material-measurement-text'
...
export function quoteWordingDifferences(wording: string, notes: string[]) {
  const pattern = /(?:\d*\s*[¼½¾⅛⅜⅝⅞⅓⅔]|\d+(?:[ -]\d+\s*\/\s*\d+|\.\d+|\s*\/\s*\d+)?)/g
  const number = (value:string) => measurementNumber(value) ?? NaN
  ...
  const parts = wording.split(/((?:\d*\s*[¼½¾⅛⅜⅝⅞⅓⅔]|\d+(?:[ -]\d+\s*\/\s*\d+|\.\d+|\s*\/\s*\d+)?))/g)
  return parts.filter(Boolean).map(text => ({ text, different: /^[\d¼½¾⅛⅜⅝⅞⅓⅔]/.test(text.trim()) && values.some(value => Math.abs(number(value) - number(text)) < 0.00001) }))
}
