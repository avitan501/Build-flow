...
function family(value: string) {
  const v=text(value)
  for(const [name,pattern] of [["hanger",/hanger|face mount|top mount/],["joist",/tji|ni.?40|pwi|pwt|i.?joist/],["lvl",/\blvl\b|laminated veneer/],["plywood",/plywood|\bcdx\b|\bosb\b|edge gold/],["nails",/nails?\b/],["blade",/blades?\b|sawzall/],["adhesive",/glue|adhesive|provan/],["lumber",/lumber|douglas fir|\bspf\b|\b2\s*x\s*\d+/]] as const) if(pattern.test(v))return name
  return null
}
...
export function receivedProductPriceRows(items: QuoteComparisonItemRecord[], quotes: ReceivedSupplierQuote[]) {
  ...
  return items.map(item=>({item,cells:quotes.map((quote,index)=>{
