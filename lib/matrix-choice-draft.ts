...
import {parseMatrixAcceptances,matrixAcceptancesError,type MatrixAcceptance} from './matrix-match-acceptance'

/** Separate purchasing intentions from reviewed bid selections and order approval. */
export type MatrixChoiceDraft = { baselineQuoteId:string; selections:Record<string,string>; sourceFingerprint:string; acceptances?:MatrixAcceptance[]; calculationMode?:'mixed'|'selected' }
export function parseMatrixChoiceDraft(value:unknown):MatrixChoiceDraft|null {
 ...
 const acceptances=raw.acceptances===undefined?undefined:parseMatrixAcceptances(raw.acceptances)
 if(acceptances===null)return null
 if(raw.calculationMode!==undefined&&!['mixed','selected'].includes(String(raw.calculationMode)))return null
 return {baselineQuoteId:raw.baselineQuoteId,selections:Object.fromEntries(entries),sourceFingerprint:raw.sourceFingerprint,...(acceptances?{acceptances}:{}),...(raw.calculationMode?{calculationMode:raw.calculationMode as 'mixed'|'selected'}:{})}
}
export function matrixChoiceScopeError(draft:MatrixChoiceDraft,items:QuoteComparisonItemRecord[],quotes:ReceivedSupplierQuote[]) {
 ...
 return draft.baselineQuoteId&&!quoteIds.has(draft.baselineQuoteId)||Object.entries(draft.selections).some(([id,q])=>!ids.has(id)||(q!==''&&!quoteIds.has(q)))?'A selected item or quote is no longer in this comparison. Reload before saving.':matrixAcceptancesError(draft.acceptances??[],items,quotes)
}
...
export function restoreMatrixChoices(value:unknown,fingerprint:string) {
 ...
 return {baselineQuoteId:stale?'':parsed?.baselineQuoteId??'',selections:stale?{}:parsed?.selections??{},acceptances:stale?[]:parsed?.acceptances??[],calculationMode:stale?'mixed' as const:parsed?.calculationMode??'mixed' as const,warning:stale?'Request or source quotes changed. Review your purchasing draft again.':''}
}
