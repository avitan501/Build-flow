...
export function materialListClarifications(source: string): MaterialClarification[] {
  ...
  if (/\btji\b/i.test(source)) questions.push({ id: "tji-depth", label: "TJI depth convention", question: 'When this list says 10-inch TJI, does it mean 9½-inch?', scope: "TJI only; preserve series, length and quantity. Never apply to LVL.", options: ["TJI 10-inch means 9½-inch; preserve the specified series", "Keep the written TJI depth; request confirmation", unknown], reference: "https://www.weyerhaeuser.com/woodproducts/engineered-lumber/tji-joists/" })
  if (/\blvl\b/i.test(source)) questions.push({ id: "lvl-depth", label: "LVL depth convention", question: 'For LVL written as 10-inch, which depth is required?', scope: "Only LVL lines written as 10-inch; preserve all other dimensions.", options: ["LVL 10-inch means 9½-inch; retain thickness and length", "Keep the written LVL depth; request confirmation", unknown] })
  ...
  if (/plywood|\bosb\b|\bcdx\b/i.test(source)) questions.push({ id: "panel-material", label: "Panel substitutions", question: "Can OSB be offered as an alternative to requested plywood?", scope: "Keep thickness, grade, sheet dimensions and edge profile explicit.", options: ["Keep plywood and OSB separate; no material substitution", "Show OSB as a separate alternative, not an exact match", unknown] })
