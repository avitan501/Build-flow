...
export const ownerQuoteSummary = {
  ...
  job: "26-50 211TH ST, BAYSIDE",
  subtotal: 43889.86,
  tax: 3785.5,
  total: 47675.36,
} as const;

export const ownerQuoteRows: OwnerQuoteRow[] = [
...
export function buildOwnerQuoteDuplicateKey(row: OwnerQuoteRow) {
  if (row.itemNo.trim()) {
    return `${ownerQuoteSummary.supplier}|${ownerQuoteSummary.quoteDate}|${row.itemNo.trim()}`;
  }

  return `${ownerQuoteSummary.supplier}|${normalizeOwnerQuoteText(row.description)}|${row.unit.trim().toUpperCase()}`;
}
