...
export default async function CustomerRequestsPage({ searchParams }: CustomerRequestsPageProps) {
  ...
  const openedRequest = selectedRequest
    ...
    : null;
  const totalMaterialLines = requests.reduce((total, request) => total + request.items.length, 0);

  return (
    <main className="min-h-screen bg-[#f5f5f7] px-4 py-5 text-slate-950 sm:px-6 sm:py-7 lg:px-8">
      ...
      <div className="mx-auto w-full max-w-6xl">
        ...
        <header className="border-b border-slate-200 pb-4 sm:flex sm:items-end sm:justify-between sm:gap-6">
          <div className="min-w-0">
            <p className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-[#0066cc]"><ShieldCheck className="h-3.5 w-3.5" aria-hidden="true" />Customer portal</p>
            <h1 className="mt-1 text-2xl font-black tracking-[-0.035em] sm:text-3xl">{openedRequest ? <>Request #{openedRequest.publicNumber}</> : "My material requests"}</h1>
            <p className="mt-1 text-sm text-slate-500">{openedRequest ? `${openedRequest.statusLabel} · ${openedRequest.items.length} material line${openedRequest.items.length === 1 ? "" : "s"}` : `${requests.length} request${requests.length === 1 ? "" : "s"} · ${totalMaterialLines} material line${totalMaterialLines === 1 ? "" : "s"}`}</p>
          </div>
          <Link href="/account" className="mt-3 inline-flex min-h-10 items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 text-xs font-bold text-slate-700 transition hover:border-sky-300 hover:text-[#0066cc] sm:mt-0">
        ...
        <div id="material-requests" className={`${openedRequest ? "mt-4" : "mt-6"} flex scroll-mt-24 flex-wrap items-center justify-between gap-2`}>
        ...
        {requests.length ? (
          <div className="mt-3 grid gap-3">
            {requests.map((request, requestIndex) => {
              ...
              <article
                ...
                className={`overflow-hidden rounded-xl border bg-white ${openedFromText ? "border-sky-400 ring-2 ring-sky-100" : "border-slate-200"}`}
              >
          ...
          <section className="mt-3 rounded-xl border border-slate-200 bg-white p-6 text-center sm:p-10">
            <span className="mx-auto grid h-10 w-10 place-items-center rounded-lg bg-sky-50 text-sky-700"><PackageCheck className="h-4 w-4" aria-hidden="true" /></span>
            <h2 className="mt-3 text-sm font-black">No requests are linked yet</h2>
