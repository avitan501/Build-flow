...
export async function recognizeRequestProductLine(input:{requestId:string;itemId:string;revision:string;text:string;newRow?:boolean}) {
  ...
  if(input.newRow){
    const {data:request}=await supabase.from("quote_requests").select("id").eq("id",input.requestId).maybeSingle()
    if(!request||item)return{ok:false as const,error:"Reload this request before recognizing the new row."}
  }else if(!item||item.metadata?.ai_organized!==true)return{ok:false as const,error:"This product is no longer available."}
  const sourceId=typeof item?.metadata?.source_item_id==="string"?item.metadata.source_item_id:null
  let source:ReviewableMaterialItem|null=null
  if(sourceId){const result=await supabase.from("quote_request_items").select(columns).eq("request_id",input.requestId).eq("id",sourceId).maybeSingle<ReviewableMaterialItem>();source=result.data}
  else if(input.newRow||item?.metadata?.request_item_origin==="manually_added"){
    const {data:candidates}=await supabase.from("quote_request_items").select(columns).eq("request_id",input.requestId).limit(100).returns<ReviewableMaterialItem[]>()
    source=candidates?.find(candidate=>candidate.metadata?.ai_organized!==true&&Boolean(candidate.metadata?.material_clarifications))||null
  }
  if(!input.newRow&&(!item||(sourceId&&!source)||requestItemRevision(item,sourceId?source:null)!==input.revision))return{ok:false as const,error:"The product or original changed. Reload before recognizing."}
  ...
  try{
    const {data,error}=await createAdminClient().functions.invoke("material-row-recognition",{body:{rows:[{id:input.itemId,text:input.text}],sharedAnswers:fields.map(f=>`${f.label}: ${f.value}`).join("\n"),conventions}})
    if(error||!data?.ok)return{ok:false as const,error:"Recognition is unavailable. Your existing product is unchanged."}
    return{ok:true as const,row:validateRecognizedRows(data,[{id:input.itemId,text:input.text}],conventions)[0]}
