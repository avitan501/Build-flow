      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mt-4 grid gap-4">
          <section className="rounded-lg border border-slate-200 bg-white p-5">
            <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-[.12em] text-[#0071e3]">AI organized</p><h2 className="mt-1 text-xl font-bold">Material list</h2><p className="mt-1 text-sm text-slate-500">Review quantities and specifications before requesting supplier pricing.</p></div>{!organizedItems.length && organizationStatus !== "processing" ? <OrganizeMaterialListButton requestId={request.id} /> : null}</div>
            {organizedItems.length ? <>
              <div className="mt-4 grid gap-2 sm:hidden">{organizedItems.map((item) => { const dimensions = typeof item.metadata?.dimensions === "string" ? item.metadata.dimensions : ""; const thickness = typeof item.metadata?.thickness === "string" ? item.metadata.thickness : ""; const details = typeof item.metadata?.request_details === "string" ? item.metadata.request_details : ""; return <article key={item.id} className="min-w-0 rounded-lg border border-slate-200 bg-white p-3"><div className="flex min-w-0 items-start justify-between gap-3"><h3 className="min-w-0 break-words text-sm font-bold">{item.name}</h3><span className="shrink-0 rounded-md bg-slate-950 px-2 py-1 text-xs font-bold text-white tabular-nums">{item.quantity} {item.unit || "each"}</span></div>{item.metadata?.needs_review === true ? <span className="mt-2 inline-flex rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold uppercase text-amber-800">Review</span> : null}<p className="mt-2 break-words text-xs leading-5 text-slate-600">{[dimensions && `Size: ${dimensions}`, thickness && `Thickness: ${thickness}`, details].filter(Boolean).join(" · ") || "No additional details"}</p></article> })}</div>
              <div className="mt-4 hidden overflow-hidden rounded-lg border border-slate-200 sm:block"><table className="w-full table-fixed border-collapse text-left text-sm"><thead className="bg-slate-950 text-white"><tr><th className="w-28 px-3 py-2.5">Quantity</th><th className="w-1/3 px-3 py-2.5">Item</th><th className="px-3 py-2.5">Size and details</th></tr></thead><tbody className="divide-y divide-slate-200">{organizedItems.map((item) => { const dimensions = typeof item.metadata?.dimensions === "string" ? item.metadata.dimensions : ""; const thickness = typeof item.metadata?.thickness === "string" ? item.metadata.thickness : ""; const details = typeof item.metadata?.request_details === "string" ? item.metadata.request_details : ""; return <tr key={item.id} className="align-top"><td className="bg-slate-50 px-3 py-3 font-bold tabular-nums">{item.quantity} {item.unit || "each"}</td><td className="break-words px-3 py-3 font-semibold">{item.name}{item.metadata?.needs_review === true ? <span className="ml-2 rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold uppercase text-amber-800">Review</span> : null}</td><td className="break-words px-3 py-3 leading-5 text-slate-600">{[dimensions && `Size: ${dimensions}`, thickness && `Thickness: ${thickness}`, details].filter(Boolean).join(" · ") || "No additional details"}</td></tr> })}</tbody></table></div>
