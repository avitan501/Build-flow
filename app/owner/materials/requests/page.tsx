...
export default async function MaterialRequestsInboxPage({
  ...
  const [{ data, error }, { data: customerProfiles }, { data: categories }, { data: textDrafts }] = await Promise.all([
    supabase
      .from("quote_requests")
      .select("id, public_number, owner_id, title, status, manager_assignee, created_at, submitted_at, projects(name,address), material_questionnaire_responses(id,category_name_snapshot,status)")
  ...
  function requestCard(request: InboxRequest) {
    ...
    return <article key={request.id} className={`grid gap-4 rounded-[20px] border bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,.05)] sm:grid-cols-[minmax(0,1fr)_13rem] sm:items-center ${created === request.id ? "border-emerald-400 ring-2 ring-emerald-100" : "border-slate-200"}`}>
      <Link href={`/owner/materials/requests/${request.id}`} className="min-w-0 rounded-lg outline-none transition hover:text-[#0066cc] focus-visible:ring-2 focus-visible:ring-[#0066cc] focus-visible:ring-offset-4">
        <div className="flex flex-wrap items-center gap-2"><span className="text-xs font-bold text-[#0066cc]">#{request.public_number}</span><h2 className="font-bold">{request.title}</h2><span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold uppercase text-slate-600">{statusLabels[request.status] || request.status.replaceAll("_", " ")}</span></div>
        ...
        <p className="mt-2 text-xs text-slate-500">{profile?.full_name || profile?.email || "Client"} · {formatSiteDateTime(request.created_at)}</p>
