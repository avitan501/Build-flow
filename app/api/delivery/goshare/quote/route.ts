...
import { GoShareError, quoteGoShare } from "@/lib/goshare";
...
export async function POST(request: Request) {
  try {
    ...
    if (!parsed.success) return NextResponse.json({ ok: false, code: "invalid_quote", error: "Choose both addresses and enter the material description, quantity, weight, and vehicle." }, { status: 400 });

    const quote = await quoteGoShare(parsed.data);
    return NextResponse.json({ ok: true, provider: "GoShare", quote }, { headers: { "Cache-Control": "no-store, max-age=0" } });
  } catch (error) {
    await captureOperationalError(error, { feature: "delivery", operation: "request-quote", provider: "goshare", safeCode: error instanceof GoShareError ? error.code : "goshare-quote-failed" });
    if (error instanceof GoShareError) return NextResponse.json({ ok: false, code: error.code, error: error.message }, { status: error.code === "structured_address_required" || error.code === "invalid_schedule" ? 400 : 502 });
    return NextResponse.json({ ok: false, code: "quote_failed", error: "GoShare could not return a live estimate right now." }, { status: 502 });
  }
}
