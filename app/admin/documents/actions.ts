...
import {
  extractManagerDocument,
  type ManagerDocumentAiInvoker,
} from "@/lib/manager-document-intelligence";
import {
  MANAGER_DOCUMENT_BUCKET,
...
async function runDocumentExtraction(
  ...
  try {
    const edgeResult = await extractManagerDocument(
      file,
      rawText,
...
function importedSupplierId(name: string) {
  const slug = name
    ...
    .replace(/^-+|-+$/g, "")
    .slice(0, 120);
  return `document-${slug || "supplier"}-${crypto.randomUUID().slice(0, 8)}`;
}
function catalogItemKey(value: string) {
  return value
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .trim();
...
async function readableText(file: File, browserOcrText: string) {
  if (
    ...
    try {
      ...
      try {
        return (await extractText(pdf, { mergePages: true })).text.slice(
          0,
          250_000,
        );
      } finally {
      ...
      console.error("Document text layer extraction failed", error);
    }
  }
  if (
    file.type === "text/csv" ||
    ...
    return (await file.text()).slice(0, 250_000);
  return browserOcrText.slice(0, 250_000);
}

export async function uploadManagerDocumentAction(
  formData: FormData,
): Promise<Result<{ documentId: string }>> {
  const { supabase, user } = await requireStaffProfile("suppliers");
  ...
  if (duplicate.data) {
    if (duplicate.data.status === "error") {
      const retry = await retryManagerDocumentExtractionAction(
        duplicate.data.id,
        true,
      );
      return {
        ok: true,
        data: { documentId: duplicate.data.id },
        message: retry.ok
          ? "The saved original was re-read with AI. Opened the refreshed review."
          : "The original is safe in Documents. Open it and use Re-read with AI.",
      };
    }
    return {
      ok: true,
      data: { documentId: duplicate.data.id },
      message:
        "This exact document is already in Documents. Opened the saved copy instead of creating a duplicate.",
    };
  }
  ...
  const sourceChannel = allowedSources.has(requestedSource)
    ? requestedSource
    : "website_upload";
  const sourceLabel =
  ...
  const { error: insertError } = await supabase
    ...
    .insert({
      ...
      department,
      storage_bucket: MANAGER_DOCUMENT_BUCKET,
...
export async function approveManagerDocumentAction(
  documentId: string,
): Promise<Result<{ approved: true }>> {
  const { supabase, user } = await requireStaffProfile("suppliers");
  if (!UUID_PATTERN.test(documentId))
    return { ok: false, error: "Invalid document." };
  const { data: document } = await supabase
    .from("manager_documents")
    .select("id,status,warnings,department,document_type")
    .eq("id", documentId)
    .maybeSingle<{
      id: string;
      status: string;
      warnings: string[];
      department: string;
      document_type: ManagerDocumentRecord["document_type"];
    }>();
  if (!document) return { ok: false, error: "Document not found." };
  if (document.department === INTAKE_DEPARTMENT)
    return {
      ok: false,
      error:
        "Choose and save the correct department before approving this document.",
    };
  const { data: items } = await supabase
    ...
    .eq("selected", true)
    .returns<Array<{ validation_status: string }>>();
  const requiresItem = [
    "supplier_quote",
    "supplier_invoice",
    "receipt",
  ...
  if (requiresItem && !(items ?? []).length)
    return {
      ok: false,
      error:
        "Select at least one dependable supplier item before approving this document.",
    };
  if (
    (document.warnings ?? []).length ||
    (items ?? []).some((item) => item.validation_status !== "valid")
  )
    return {
      ok: false,
      error:
        "Clear the warning and review flags before approving this document.",
    };
  ...
  return {
    ok: true,
    data: { approved: true },
    message: "Document approved. It is ready for a destination.",
  };
}
...
export async function saveManagerDocumentReviewAction(input: {
  ...
  if (reviewedItems.some((item) => item.selected && !item.description))
    return { ok: false, error: "Every selected line needs a description." };
  const warnings: string[] = [];
  // The printed subtotal belongs to the whole quote, not to the subset the
  // manager chooses for Avantia's lazy catalog. A one-product selection must
  // therefore never be compared with the full quote subtotal.
  const completeLineTotal = reviewedItems.reduce<number | null>(
    (sum, item) =>
      item.lineTotal === null ? sum : (sum ?? 0) + item.lineTotal,
    null,
  );
  if (
    completeLineTotal !== null &&
    subtotal !== null &&
    Math.abs(completeLineTotal - subtotal) > 0.03
  )
    warnings.push(
      `All line totals add to $${completeLineTotal.toFixed(2)}, but subtotal is $${subtotal.toFixed(2)}.`,
    );
  const expectedTotal =
    subtotal !== null && taxAmount !== null
      ? Math.round((subtotal - discount + deliveryCharge + taxAmount) * 100) /
        100
      : null;
  if (
    expectedTotal !== null &&
    total !== null &&
    Math.abs(expectedTotal - total) > 0.03
  )
    warnings.push(
      `The reviewed amounts calculate to $${expectedTotal.toFixed(2)}, but total is $${total.toFixed(2)}.`,
    );
  if (
    reviewedItems.some(
      (item) => item.selected && item.validationStatus === "mismatch",
    )
  )
    warnings.push(
  ...
  if (
    reviewedItems.some(
      (item) => item.selected && item.validationStatus === "needs_review",
    )
  )
    warnings.push(
  ...
  const { error: documentError } = await supabase
    ...
    .update({
      ...
      tax_percent: taxPercent === null ? null : Math.min(100, taxPercent),
      total,
      status: "needs_review",
    ...
    .eq("id", input.documentId);
  if (documentError)
    return { ok: false, error: "The document review could not be saved." };
  ...
  return {
    ok: true,
    data: { warningCount: uniqueWarnings.length },
    message: uniqueWarnings.length
      ? "Review saved. Resolve the remaining warning before approval."
      : "Review saved. The document is ready for approval.",
  };
}

export async function retryManagerDocumentExtractionAction(
  documentId: string,
  replaceExisting = false,
): Promise<Result<{ itemCount: number }>> {
  const { supabase, user } = await requireStaffProfile("suppliers");
  if (!UUID_PATTERN.test(documentId))
    return { ok: false, error: "Invalid document." };
  const { data: document } = await supabase
  ...
  if (previousItems?.length && !replaceExisting)
    return {
      ok: false,
      error:
...
export async function routeManagerDocumentToSupplierPricingAction(
  ...
  if (document.legacy_supplier_quote_id) {
    ...
    if (linkedItemsError || !linkedItems?.length)
      return {
        ok: false,
  ...
  if (!items?.length)
    return {
      ok: false,
      error: "Select at least one dependable product row before routing.",
    };
  ...
  if (copyError)
    return {
      ok: false,
      error:
        "The reviewed document could not be prepared for supplier pricing.",
    };
  ...
  const { error: quoteError } = await supabase.from("supplier_quotes").insert({
    ...
    extraction_note: "Reviewed in Documents before supplier pricing.",
    delivery_charge: document.delivery_charge,
    tax_percent: document.tax_percent ?? 0,
    notes: match
      ? ""
      : "Confirm the Supplier Directory match before catalog routing.",
    created_by: user.id,
    updated_by: user.id,
  });
  if (quoteError) {
...
async function legacyAddManagerDocumentItemsToCatalogAction(
  documentId: string,
): Promise<
  Result<{ itemCount: number; priceCount: number; supplierName: string }>
> {
  const { supabase, user } = await requireStaffProfile("suppliers");
  if (!UUID_PATTERN.test(documentId))
    return { ok: false, error: "Invalid document." };
  ...
  if (invalid.length) {
    await supabase
      .from("manager_document_items")
      .update({ catalog_import_status: "pending_review" })
      .in(
        "id",
    ...
    return {
      ok: false,
      error: `Line ${invalid[0].line_number} must be reviewed before importing. Nothing was imported.`,
    };
  }
  ...
  if (!pricedItems.length)
    return {
      ok: false,
      error: "The selected rows do not have reviewed unit prices.",
    };
  ...
  if (!detectedName)
    return {
      ok: false,
      error: "Confirm the vendor name before adding these prices.",
    };
  const matchedSupplier = detectSupplierMatch(
    suppliers,
    detectedName,
    document.raw_text || detectedName,
  );
  let supplier: CatalogSupplier | null = matchedSupplier
    ? (suppliers.find((entry) => entry.id === matchedSupplier.id) ?? null)
    : null;
  if (!supplier) {
    const candidate: CatalogSupplier & Record<string, unknown> = {
      id: importedSupplierId(detectedName),
      name: clean(detectedName, 160),
      email: "",
      phone: "",
      whatsapp: "",
      portalUrl: "",
    ...
    const { data, error } = await supabase.rpc(
      ...
      { p_supplier: candidate, p_create: true },
    );
    if (error || !data)
      return {
        ok: false,
        error: "The vendor could not be added to Supplier Directory.",
      };
    supplier = data as CatalogSupplier;
  } else {
    const catalogDepartments = [
      ...new Set([...(supplier.catalogDepartments ?? []), department]),
    ];
    const catalogEnabledDepartments = [
      ...new Set([...(supplier.catalogEnabledDepartments ?? []), department]),
    ];
    const { data, error } = await supabase.rpc(
      "staff_upsert_supplier_directory_entry",
      {
    ...
    if (error || !data)
      return {
        ok: false,
        error: "The vendor could not be enabled for this catalog department.",
      };
    supplier = data as CatalogSupplier;
  }
  ...
  const [
    ...
    { data: existingSupplierPrices, error: existingPriceError },
  ] = await Promise.all([
    supabase
    ...
    supabase
      ...
      .select("item_id,supplier_sku")
      .eq("supplier_id", supplier.id)
      .returns<Array<{ item_id: string; supplier_sku: string }>>(),
  ]);
  if (existingError)
    return { ok: false, error: "The existing catalog could not be checked." };
  if (existingPriceError)
    return {
      ok: false,
      error: "The vendor's existing catalog codes could not be checked safely.",
    };
  const byName = new Map(
    (existingRows ?? [])
      .filter((row) => row.category === department)
      .map((row) => [catalogItemKey(row.name), row]),
  );
  ...
  const bySupplierCode = new Map(
    [...skuCandidates].flatMap(([key, rows]) =>
      rows?.length === 1 ? [[key, rows[0]] as const] : [],
    ),
  );
  const usedCodes = new Set(
    (existingRows ?? [])
      .map((row) => catalogItemKey(row.item_code))
      .filter(Boolean),
  );
  ...
  for (const item of items) {
    const supplierCodeMatch = item.item_code
      ? bySupplierCode.get(catalogItemKey(item.item_code))
      : null;
    const nameMatch = byName.get(catalogItemKey(item.description));
    const existing = supplierCodeMatch ?? nameMatch;
    if (existing) {
      catalogRows.set(item.id, {
        ...existing,
        matchMethod: supplierCodeMatch
          ? "exact_supplier_sku"
          : "normalized_name",
        matchConfidence: supplierCodeMatch ? 1 : 0.65,
      });
      continue;
    }
    ...
    let code = baseCode;
    let suffix = 1;
    while (usedCodes.has(catalogItemKey(code))) {
      suffix += 1;
    ...
    const { data, error } = await supabase
      ...
      .insert({
        category: department,
        item_code: code,
        name: clean(item.description, 240),
        description: clean(item.specification, 1000),
        package_quantity: 1,
        package_unit: clean(item.unit, 60) || "each",
        comparison_quantity: 1,
        comparison_unit: clean(item.unit, 60) || "each",
        review_status: "needs_review",
        quality_notes: `Imported from reviewed source ${clean(document.file_name, 180)}.`,
        default_quantity: 1,
        unit: clean(item.unit, 60) || "each",
        status: "active",
        source: `Manager document: ${clean(document.file_name, 180)}`,
        created_by: user.id,
        updated_by: user.id,
      })
    ...
    if (error || !data) {
      ...
      if (recovered.data) {
        byName.set(catalogItemKey(item.description), {
          ...recovered.data,
          category: department,
      ...
      console.error("Reviewed document catalog item creation failed", {
        code: error?.code,
      ...
      return {
        ok: false,
        error: catalogLineFailureMessage({
          item,
          step: "item",
          code: error?.code,
        }),
    ...
    catalogRows.set(item.id, {
      ...data,
      matchMethod: "created_from_document",
      matchConfidence: 1,
    });
    createdCount += 1;
  }
  const priceCandidates = pricedItems.flatMap((item) => {
    ...
    const row = reviewedDocumentPriceRow({
      document,
      item,
      catalogItem,
      supplier: supplier!,
  ...
  const selectedPrices = [...candidateGroups.values()].map((group) => {
    const lowest = group.reduce((best, candidate) =>
      candidate.row.unit_price < best.row.unit_price ? candidate : best,
    );
    return group.length === 1
      ...
      : {
          ...
          row: {
            ...lowest.row,
            notes: clean(
              `${lowest.row.notes} Lowest of ${group.length} selected quote rows for the same product.`,
              1000,
            ),
          },
        };
  ...
  let { error: priceError } = await supabase
    .from("material_catalog_supplier_prices")
  ...
  if (priceError) {
    ...
    const compatiblePrices = prices.map(
      ({
        source_document_id,
        source_file_name,
        source_quote_number,
        ...
        void source_quote_number;
        void source_document_date;
        void source_quantity;
        void source_unit;
        void source_line_total;
        void source_page;
        void source_text;
        return price;
      },
    ...
    const fallback = await supabase
      .from("material_catalog_supplier_prices")
      .upsert(compatiblePrices, { onConflict: "item_id,supplier_id" });
  ...
  if (priceError) {
    ...
    for (const candidate of selectedPrices) {
      ...
      const attempt = await supabase
        .from("material_catalog_supplier_prices")
        .upsert(compatiblePrice, { onConflict: "item_id,supplier_id" });
      if (attempt.error)
        failingLines.push(
          catalogLineFailureMessage({
            item: candidate.item as CatalogSourceLine,
            step: "price",
    ...
    if (failingLines.length) {
      await supabase
        .from("manager_document_items")
        .update({ catalog_import_status: "failed" })
        .in(
          "id",
          items.map((item) => item.id),
        )
        .eq("document_id", documentId);
      return { ok: false, error: failingLines.slice(0, 5).join(" ") };
    }
    usedCompatibilitySave = true;
  }
  for (const item of items) {
    const matched = catalogRows.get(item.id);
    if (!matched) continue;
    const linked = await supabase
      .from("manager_document_items")
      .update({
        matched_catalog_item_id: matched.id,
        match_method: matched.matchMethod,
        match_confidence: matched.matchConfidence,
      ...
      .select("id")
      .maybeSingle<{ id: string }>();
    if (linked.error || linked.data?.id !== item.id)
      return {
        ok: false,
        error:
          "Catalog data was saved, but the document line status could not be linked. Reload before retrying.",
      };
  }
  await supabase
    .from("manager_documents")
  ...
  return {
    ...
    message: `${prices.length} supplier price${prices.length === 1 ? "" : "s"} saved from ${items.length} selected quote line${items.length === 1 ? "" : "s"} for ${supplier.name}. Source and date were saved${usedCompatibilitySave ? " in the price note" : ""}.`,
  };
}
...
> {
  ...
  const matchedSupplier = detectedName
    ...
    : null;
  const existingSupplier = matchedSupplier
    ? (suppliers.find((entry) => entry.id === matchedSupplier.id) ?? null)
    : null;
  ...
  const supplier =
    isSupplierDocument && detectedName
      ? existingSupplier
        ? {
            ...existingSupplier,
            catalogDepartments: [
            ...
            catalogEnabledDepartments: [
              ...new Set([
                ...(existingSupplier.catalogEnabledDepartments ?? []),
                department,
              ]),
            ],
          }
  ...
  if (error) {
    console.error("Atomic document catalog import failed", {
      code: error.code,
      message: error.message,
      documentId,
    });
    return {
      ok: false,
      error: "Nothing was imported. Reload the document and try again.",
    };
  }
  const result = data as {
    ...
    lines?: Array<{ lineNumber?: number; reason?: string }>;
  } | null;
  if (!result?.ok) {
    ...
    return {
      ...
      error:
        ...
        "Nothing was imported. Review the selected rows and try again.",
    };
  }
  ...
  const supplierName = result.supplierName || null;
  return {
    ok: true,
    data: { itemCount, priceCount, supplierName },
    message: supplierName
      ? `${itemCount} product${itemCount === 1 ? "" : "s"} and ${priceCount} reviewed price${priceCount === 1 ? "" : "s"} imported for ${supplierName}.`
      : `${itemCount} product${itemCount === 1 ? "" : "s"} imported. No supplier price was attached.`,
  };
}
