...
import {
  addAuraCommunicationLinks,
  type AuraEmailEntityType,
} from "@/lib/aura/email-links";
import {
  isSmsCorrectionReason,
  redactSmsTrainingText,
  smsTrainingIntent,
  smsTrainingLanguage,
...
export async function completeSmsReplyDraftAction(input: {
  draftId: string;
  reply: string;
  teachAi: boolean;
  correctionReasons?: SmsCorrectionReason[];
}) {
  const { supabase, user, access } = await requireManagerPortalProfile();
  if (!access.customers || !/^[0-9a-f-]{36}$/i.test(input.draftId))
    return { ok: false as const, error: "Choose a valid AI draft." };
  const reply = String(input.reply || "")
    .trim()
    .slice(0, 1600);
  if (!reply)
    return { ok: false as const, error: "The sent reply cannot be empty." };

  const { data: draft, error: draftError } = await supabase
    .from("aura_sms_reply_drafts")
  ...
  if (draftError || !draft)
    return { ok: false as const, error: "The AI draft could not be found." };
  ...
  if (communicationError || !communication?.body?.trim())
    return {
      ok: false as const,
      error: "The customer message for this draft could not be found.",
    };
  ...
  const { data: trainingContact } = communication.contact_id
    ? await supabase
        ...
        .eq("id", communication.contact_id)
        .maybeSingle<{ full_name: string | null; company: string | null }>()
    : { data: null };
  ...
  if (updateError)
    return {
      ok: false as const,
      error: "The AI draft status could not be updated.",
    };

  const corrected = reply !== draft.reply_text.trim();
  const correctionReasons = [
    ...new Set((input.correctionReasons || []).filter(isSmsCorrectionReason)),
  ].slice(0, 6);
  const language = smsTrainingLanguage(communication.body);
  const intent = smsTrainingIntent(communication.body);
  const privateSafeOriginalReply = redactSmsTrainingText(
    draft.reply_text.trim(),
    privateTrainingValues,
  );
  const privateSafeCorrectedReply = redactSmsTrainingText(
    reply,
    privateTrainingValues,
  );
  const { error: feedbackError } = await supabase
    .from("aura_ai_reply_feedback")
    .insert({
  ...
  if (feedbackError)
    return {
      ok: false as const,
      error: "The reply was sent, but its AI feedback could not be saved.",
    };

  if (input.teachAi) {
    const privateSafeCustomerMessage = redactSmsTrainingText(
      communication.body,
      privateTrainingValues,
    );
    const privateSafeApprovedReply = privateSafeCorrectedReply;
    const { error: exampleError } = await supabase
      .from("aura_ai_reply_examples")
      .upsert(
        {
          ...
          privacy_redacted: true,
          tags: [
            ...(corrected ? ["manager-corrected"] : ["manager-approved"]),
            ...correctionReasons,
          ],
          enabled: true,
    ...
    if (exampleError)
      return {
        ok: false as const,
        error:
          "The reply was sent, but the approved AI example could not be saved.",
      };
  }
...
export async function saveSmsAutomationAction(input: {
  phone: string;
  mode: SmsAiMode;
  style: SmsAiStyle;
  ...
  if (!access.customers)
    return {
      ok: false as const,
      error: "Customer communication access is required.",
    };
  if (
    !new Set<SmsAiMode>(["off", "draft", "auto_safe"]).has(input.mode) ||
    !new Set<SmsAiStyle>(["professional", "friendly", "brief"]).has(input.style)
  )
    return { ok: false as const, error: "Choose valid AI settings." };
  const phone = normalizeAuraPhone(input.phone);
  if (!phone)
    return {
      ok: false as const,
      error: "Choose a conversation with a valid phone number.",
    };
  const { data, error } = await supabase.functions.invoke<{
    ...
    body: {
      action: "save_sms_automation",
      phone,
      mode: input.mode,
      style: input.style,
      autoCreateRequestDrafts: Boolean(input.autoCreateRequestDrafts),
    },
  });
  if (error || !data?.ok)
    return {
      ok: false as const,
      error: data?.error || "The AI reply settings could not be saved.",
    };
  revalidatePath("/admin/communications");
  return { ok: true as const };
}

export async function generateSmsReplyAction(input: {
  communicationId: string;
}) {
  const { supabase, access } = await requireManagerPortalProfile();
  if (!access.customers || !/^[0-9a-f-]{36}$/i.test(input.communicationId))
    return { ok: false as const, error: "Choose an incoming text message." };
  const { data, error } = await supabase.functions.invoke<{
    ok?: boolean;
    reply?: string;
    safetyReason?: string;
  ...
  if (error || !data?.ok || !data.reply)
    return {
      ok: false as const,
      error: data?.error || "AI could not prepare a reply right now.",
    };
  return {
    ok: true as const,
    reply: data.reply,
    safetyReason: data.safetyReason || "Review before sending.",
    requestDetected: Boolean(data.requestDetected),
  };
}

export async function reviewSmsRequestAction(input: {
  communicationId: string;
}) {
  const { supabase, access } = await requireManagerPortalProfile();
  if (!access.customers || !/^[0-9a-f-]{36}$/i.test(input.communicationId))
  ...
  const { data, error } = await supabase.functions.invoke<{
    ok?: boolean;
    proposal?: SmsRequestProposal;
    error?: string;
  }>("aura-messaging-broker", {
    body: {
      action: "review_sms_request",
      communicationId: input.communicationId,
    },
  });
  if (error || !data?.ok || !data.proposal)
    return {
      ok: false as const,
      error: data?.error || "AI could not review this conversation right now.",
    };
...
export async function createSmsMaterialRequestAction(input: {
  ...
  const customerName =
    String(input.customerName || "")
      ...
      .replace(/\s+/g, " ")
      .slice(0, 160) || phone;
  const customerAddress = String(input.customerAddress || "")
    .trim()
    .replace(/\s+/g, " ")
    .slice(0, 500);
  const title = String(input.title || "")
    ...
    .slice(0, 180);
  const department =
    String(input.department || "Unassigned")
      .trim()
      .replace(/\s+/g, " ")
      .slice(0, 100) || "Unassigned";
  const items = Array.isArray(input.items)
    ? input.items.slice(0, 50).flatMap((item) => {
    ...
    : [];
  const sourceCommunicationIds = [
    ...new Set([
      input.communicationId,
      ...(input.sourceCommunicationIds || []),
    ]),
  ]
    .filter((id) => /^[0-9a-f-]{36}$/i.test(id))
    .slice(0, 20);
  const confirmationCommunicationId = String(
    input.confirmationCommunicationId || "",
  ).trim();
  if (!phone || !title || !items.length || !sourceCommunicationIds.length)
    return {
      ok: false as const,
      error: "Add a request title and at least one material item.",
    };
  if (
    !/^[0-9a-f-]{36}$/i.test(confirmationCommunicationId) ||
    !sourceCommunicationIds.includes(confirmationCommunicationId)
  )
    return {
      ok: false as const,
      error: "Choose the customer's explicit confirmation message.",
    };

  const { supabase, user, access } = await requireManagerPortalProfile();
  if (!access.customers)
    return { ok: false as const, error: "Customer access is required." };
  const admin = createAdminClient();
  const confirmation = await admin
    .from("aura_communications")
    .select("id,channel,direction,counterparty_phone,body")
    .eq("id", confirmationCommunicationId)
    .maybeSingle<{
      id: string;
      channel: string;
  ...
  if (
    confirmation.error ||
    !confirmation.data ||
    confirmation.data.channel !== "sms" ||
    confirmation.data.direction !== "incoming" ||
    normalizeAuraPhone(confirmation.data.counterparty_phone || "") !== phone ||
    !isExplicitCustomerRequestConfirmation(confirmation.data.body)
  )
    return {
      ok: false as const,
      error: "The selected inbound text does not clearly confirm this request.",
    };
  const reservation = await admin
    .from("aura_sms_request_confirmations")
    .insert({
      confirmation_communication_id: confirmationCommunicationId,
      normalized_phone: phone,
      confirmation_actor_id: user.id,
    })
    .select("confirmation_communication_id")
    .maybeSingle();
  if (reservation.error) {
    const existing = await admin
      .from("aura_sms_request_confirmations")
      .select("request_id")
      .eq("confirmation_communication_id", confirmationCommunicationId)
      .maybeSingle<{ request_id: string | null }>();
    if (!existing.data?.request_id)
      return {
        ok: false as const,
        error:
          "This confirmation is already being processed. Refresh before trying again.",
      };
    ...
    return {
      ok: true as const,
      requestId: existing.data.request_id,
      publicNumber: prior.data?.public_number || null,
      invitation:
        "The material request was submitted for review. Its secure status invitation was sent or queued.",
    };
  }

  const tagged = await quickTagPhoneContactAction({
    phone,
    kind: "customer",
  ...
  const notes = [
    `Created from SMS after manager review.`,
    `Customer phone: ${phone}`,
    customerAddress ? `Job address: ${customerAddress}` : "",
    `Source messages: ${sourceCommunicationIds.length}`,
  ]
    .filter(Boolean)
    .join("\n");
  const { data: requestId, error } = await supabase.rpc(
    ...
    {
      p_customer_id: tagged.sourceId,
      p_department: department,
      p_title: title,
      p_lines: items,
      p_notes: notes.slice(0, 4000),
    },
  );
  if (error || typeof requestId !== "string") {
    ...
    return {
      ok: false as const,
      error: "The material request could not be created.",
    };
  }
  ...
  await admin
    ...
    .eq("confirmation_communication_id", confirmationCommunicationId);
  const requestNumber = await admin
    .from("quote_requests")
    .select("public_number")
    .eq("id", requestId)
    .single<{ public_number: number }>();
  if (requestNumber.error || !requestNumber.data?.public_number)
    return {
      ok: false as const,
      error: "The request was created, but its public number needs attention.",
      requestId,
    };
  const portalAccess = await admin
    .from("customer_request_portal_access")
    .upsert(
      {
        ...
        normalized_phone: phone,
        delivery_address: customerAddress,
        claimed_by: tagged.sourceId,
        invited_at: new Date().toISOString(),
      },
      { onConflict: "request_id" },
    );
  if (portalAccess.error)
    return {
      ok: false as const,
      error: "The request was created, but portal access needs attention.",
      requestId,
    };
  const queuedInvite = await admin
    .from("customer_request_portal_invite_outbox")
    .upsert(
      {
        request_id: requestId,
  ...
  if (queuedInvite.error)
    return {
      ok: false as const,
      error:
        "The request was created, but its secure invitation could not be queued.",
      requestId,
    };
  await admin
    .from("aura_sms_request_confirmations")
  ...
  const request = await supabase
    .from("quote_requests")
    .update({ manager_assignee: "carlos" })
    .eq("id", requestId)
    .select("project_id")
  ...
  if (request.error || !request.data)
    return {
      ok: false as const,
      error: "The request was created, but Carlos could not be assigned.",
    };
  if (customerAddress) {
    const addressUpdate = await supabase
      .from("projects")
      .update({ address: customerAddress })
      .eq("id", request.data.project_id);
    if (addressUpdate.error)
      return {
        ok: false as const,
        error: "The request was created, but the address could not be saved.",
      };
  }
  const linked = await supabase.functions.invoke<{
    ...
    body: {
      action: "link_sms_material_request",
      requestId,
      phone,
      customerName,
      communicationIds: sourceCommunicationIds,
    },
  });
  if (linked.error || !linked.data?.ok)
    return {
      ...
      error:
        linked.data?.error ||
        "The request was created, but the conversation link needs attention.",
      requestId,
    };
  const invitationDelivery = await supabase.functions.invoke<{
    ok?: boolean;
    delivery?: string;
    error?: string;
  }>("aura-messaging-broker", {
    body: { action: "send_customer_request_invite", requestId },
  });
  revalidatePath("/admin/communications");
  ...
  return {
    ok: true as const,
    requestId,
    publicNumber: requestNumber.data.public_number,
    invitation:
      invitationDelivery.data?.delivery === "sent"
        ? "Secure request status invitation sent."
        : "Secure request status invitation queued for retry.",
  };
}

export async function quickTagPhoneContactAction(input: {
  phone: string;
  kind: ContactKind;
  name?: string;
}) {
  ...
  if (
    !phone ||
    !new Set<ContactKind>(["customer", "lead", "supplier"]).has(input.kind)
  )
    return {
      ok: false as const,
      error: "Choose a valid phone conversation and tag.",
    };
  ...
  if (input.kind === "customer") {
    ...
    if (existing.error)
      return {
        ok: false as const,
        error: "The lead directory could not be checked.",
      };
    if (existing.data?.id) sourceId = existing.data.id;
      ...
      if (created.error || !created.data)
        return { ok: false as const, error: "The lead could not be added." };
      sourceId = created.data.id;
    }
  } else {
    if (!access.suppliers)
      return { ok: false as const, error: "Supplier access is required." };
    ...
    const existing = suppliers.find(
      (supplier) =>
        normalizeAuraPhone(supplier.phone || supplier.whatsapp || "") === phone,
    );
    ...
    if (!existing) {
      ...
      if (saved.error)
        return {
          ok: false as const,
          error: "The supplier could not be added.",
        };
    }
  }
...
export async function saveCommunicationLogAction(input: {
  ...
  if (!access.customers)
    return {
      ok: false,
      error: "Customer access is required to add a communication log.",
    };
  if (!clientId || !summary)
  ...
  const clientName = String(
    client.data.full_name || client.data.email || "Client",
  ).slice(0, 120);
  ...
  const result = await supabase.from("manager_goals").insert({
    ...
    title:
      `${input.channel === "call" ? "Call" : "WhatsApp"} · ${clientName}`.slice(
        0,
        120,
      ),
...
export async function linkCommunicationContactAction(input: {
  ...
  if (!conversationPhone && !conversationEmail)
    return {
      ok: false as const,
...
export async function linkEmailConversationAction(input: {
  conversationEmail: string;
  entityType: AuraEmailEntityType;
  entityId: string;
}): Promise<Result> {
  const { user, access } = await requireManagerPortalProfile();
  const email = normalizeAuraEmail(input.conversationEmail);
  if (!access.customers || !email)
    return { ok: false, error: "Choose a valid email conversation." };
  if (
    !new Set<AuraEmailEntityType>([
      "client",
      "lead",
      "supplier",
      "material_request",
    ]).has(input.entityType) ||
    !/^[A-Za-z0-9_-]{1,160}$/.test(input.entityId)
  )
    return { ok: false, error: "Choose what this email belongs to." };
  const admin = createAdminClient();
  ...
  if (input.entityType === "client") {
    ...
    const { data } = await admin
      ...
      .maybeSingle();
    if (!data)
      return { ok: false, error: "That material request could not be found." };
    label = data.title;
    ...
    const supplier = data?.state?.qualificationSettings?.suppliers?.find(
      (item) => item.id === input.entityId,
    );
    if (!supplier)
      return { ok: false, error: "That supplier could not be found." };
    label = supplier.name;
  }
  ...
  if (error || !rows?.length)
    return {
      ok: false,
      error: "No email messages were found in this conversation.",
    };
  await addAuraCommunicationLinks(
    rows.map((row) => row.id),
    [
  ...
  revalidatePath("/admin/communications");
  revalidatePath("/admin/vendors");
  if (input.entityType === "material_request")
    revalidatePath(`/owner/materials/requests/${input.entityId}`);
  return { ok: true };
}

export async function markEmailConversationReadAction(input: {
  conversationEmail: string;
}): Promise<Result> {
  return markCommunicationConversationReadAction({
    conversationEmail: input.conversationEmail,
  });
}

export async function markCommunicationConversationReadAction(input: {
  conversationPhone?: string;
  conversationEmail?: string;
}): Promise<Result> {
