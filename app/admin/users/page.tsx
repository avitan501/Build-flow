    ...
    <main className="min-h-screen bg-[#f5f5f7] px-4 pb-28 pt-6 text-slate-950 sm:px-8 sm:pb-12">
      <div className="mx-auto max-w-7xl">
        ...
        <nav className="mt-4 grid grid-cols-3 gap-1 rounded-lg border border-slate-200 bg-white p-1" aria-label="Customers, leads, and requests views">
          ...
          <Link href="/admin/users?view=leads" className={`flex min-h-12 min-w-0 items-center justify-center gap-2 rounded-md px-2 text-sm font-semibold ${view === "leads" ? "bg-slate-950 text-white" : "text-slate-600 hover:bg-slate-50"}`}><Target className="h-4 w-4 shrink-0" /><span className="truncate">Leads</span><span className={`rounded-full px-2 py-0.5 text-xs ${view === "leads" ? "bg-white/15 text-white" : "bg-slate-100 text-slate-700"}`}>{leads.length}</span></Link>
        ...
        {view === "customers" ? (
          ...
          <div className="mt-4"><OutreachLeadDirectory leads={filteredLeads} conversations={leadConversations} senderName={senderName} /></div>
        ) : (
