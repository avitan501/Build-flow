...
export default async function WebsiteWorkPage() {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id,title,category,status,assigned_agent,progress_percent,summary,next_step,updated_at",
    )
        ...
        <header className="flex flex-wrap items-end justify-between gap-4 border-b border-slate-200 pb-5">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">
              Only open work · latest decision wins · {items.length} items
            </p>
          </div>
          ...
          {groups.map(([category, categoryItems]) => (
            ...
            >
              <header className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-4 py-3">
                <h2 className="text-xs font-bold uppercase tracking-[.12em] text-slate-700">
              ...
              <div className="hidden grid-cols-[minmax(12rem,1.2fr)_7rem_6rem_5rem_minmax(13rem,1.4fr)] gap-3 border-b border-slate-100 px-4 py-2 text-[10px] font-bold uppercase tracking-wide text-slate-400 md:grid">
                <span>Task</span>
                <span>Status</span>
                <span>Owner</span>
                <span>Done</span>
              ...
              {(categoryItems ?? []).map((item) => (
                <article
                  key={item.id}
                  className="grid gap-2 border-b border-slate-100 px-4 py-3 last:border-0 md:grid-cols-[minmax(12rem,1.2fr)_7rem_6rem_5rem_minmax(13rem,1.4fr)] md:items-center md:gap-3"
                >
                  ...
                  <span
                    className={`w-fit rounded-full px-2.5 py-1 text-[10px] font-bold capitalize ring-1 ${statusStyles[item.status] ?? statusStyles.open}`}
                  >
                    {statusLabel(item.status)}
                  </span>
                  <p className="text-xs font-semibold text-slate-700">
                    <span className="mr-1 text-slate-400 md:hidden">
                      Owner:
                    </span>
                    {item.assigned_agent || "—"}
                  </p>
