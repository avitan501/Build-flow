...
import { supplierQuoteStatusLabel, type SupplierQuoteClient, type SupplierQuoteRecord } from "@/lib/supplier-quotes"
...
export default async function SupplierQuotesPage({ searchParams }: {
  ...
  const [quotesResult, suppliersResult, clientsResult] = await Promise.all([
  ...
  const enabled = !quotesResult.error && !suppliersResult.error && !clientsResult.error
    ...
    <main className="min-h-screen bg-[#f5f5f7] px-4 pb-16 pt-6 text-slate-950 sm:px-8 lg:px-10">
      <div className="mx-auto max-w-7xl">
        <header className="flex flex-col gap-5 border-b border-slate-200 pb-6 xl:flex-row xl:items-end xl:justify-between">
          ...
          <SupplierQuoteUploadForm clients={clients} suppliers={suppliers.map((supplier) => ({ id: supplier.id, name: supplier.name, catalogDepartments: supplier.catalogDepartments }))} departments={materialCatalogDepartmentOptions()} enabled={enabled} aiEnabled={Boolean(process.env.OPENAI_API_KEY)} />
        </header>
