...
export const metadata = pageMetadata({
  title: "Flooring Material Calculator | Avantia Build",
  description: "Calculate flooring quantities and extract room square footage from an uploaded floor plan.",
  path: "/shop/wood-floor/flooring-calculator",
});
    ...
    <main className="min-h-screen bg-[#f7f8fa] px-4 py-4 pb-28 text-slate-900 sm:px-6 sm:py-5 sm:pb-10 lg:px-8">
      <section className="mx-auto flex max-w-7xl flex-col gap-4">
        <Link href="/shop/wood-floor" className="w-fit text-sm font-bold text-sky-700">
          {t("Back to Wood Floor")}
        </Link>
        <div>
          <h1 className="text-[2rem] font-bold tracking-normal text-slate-950 sm:text-[2.4rem]">{t("Wood floor calculator")}</h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            {t("Extract room square footage from a floor plan, choose which rooms get wood floor, and save a reviewed takeoff with a marked plan attachment.")}
          </p>
        </div>
