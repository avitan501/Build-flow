...
-- Contact matching cannot rely on contact_id alone because provider webhooks
-- may create the communication before the contact association is populated.
...
begin
  ...
  where (p_contact_id is not null and contact.id = p_contact_id)
     or (
       phone_digits <> ''
       and regexp_replace(coalesce(contact.normalized_phone, ''), '[^0-9]', '', 'g') = phone_digits
     )
     or (
       email_value <> ''
       and lower(trim(coalesce(contact.email, ''))) = email_value
     )
  order by
    case
      when p_contact_id is not null and contact.id = p_contact_id then 0
      when phone_digits <> '' and regexp_replace(coalesce(contact.normalized_phone, ''), '[^0-9]', '', 'g') = phone_digits then 1
      else 2
    end,
