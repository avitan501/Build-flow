-- Keep the long-running Quo recovery poll off the customer webhook function.
-- The dedicated worker uses the same constant-time dispatch secret and lease,
-- while the primary aura-messaging-broker stays available for live webhooks.
...
comment on function public.dispatch_quo_fast_poll() is
  'Dispatches bounded Quo recovery to its isolated Edge worker so live customer webhooks remain responsive.';
