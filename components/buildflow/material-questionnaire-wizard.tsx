...
function configuratorGroupsFor(snapshot: MaterialQuestionnaireSnapshot) {
  ...
  return [
    { id: "material", title: isDoor ? "Choose Doors or Molding" : isFraming ? "Lumber" : isElectrical ? "Cable" : "Material", description: isDrywall ? "Choose the board and performance type." : isTile ? "Choose the tile-setting materials." : isDoor ? "Select one for this request." : isFraming || isElectrical ? "" : "Choose the flooring construction and appearance." },
...
function configuratorGroupFor(question: MaterialQuestion) {
  ...
  if (/(accessor|underlay|adhesive|glue|paper|nosing|transition|waste|bullnose|screw|compound|corner|bead|tape|stud|reference|catalog|note|requirement|spacer|waterproof|primer|sealant)/.test(key)) return "extras"
