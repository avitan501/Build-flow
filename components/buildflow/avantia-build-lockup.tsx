    ...
    <span className={`flex min-w-0 flex-col ${className}`} data-testid="avantia-build-lockup">
      ...
      >
        <span className={compact ? "text-[1.08rem] sm:text-[1.15rem]" : "text-[1.55rem] sm:text-[1.75rem]"} aria-hidden="true">
          av<span className="relative inline-block"><span className="bg-[linear-gradient(135deg,#1E9BFF_0%,#1FC9C6_24%,#2BD98A_42%,#7E5BEA_68%,#F0419E_86%,#FF5BC2_100%)] bg-clip-text text-transparent">a</span>{showHardHat ? <HardHat aria-hidden="true" className="absolute -top-[0.78em] left-1/2 h-[0.72em] w-[0.72em] -translate-x-1/2 text-[#7E5BEA]" strokeWidth={2.4} /> : null}</span>ntia
        </span>
