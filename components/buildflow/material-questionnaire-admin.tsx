...
function QuestionEditor({ question, questions, index, total }: { question: MaterialQuestion; questions: MaterialQuestion[]; index: number; total: number }) {
  ...
  const [draft, setDraft] = useState({
    ...
    itemSizes: (question.configuration.itemSizes ?? []).join(", "),
    itemLengths: (question.configuration.itemLengths ?? []).join(", "),
    cableNumbers: (question.configuration.cableNumbers ?? []).join(", "),
    questionImageUrl: question.configuration.imageUrl ?? "",
  ...
  return <article className={`rounded-2xl border bg-white ${question.is_active ? "border-slate-200" : "border-slate-200 opacity-65"}`}>
    ...
    {open ? <div className="grid gap-4 border-t border-slate-100 p-4">
      ...
      <label className="grid gap-1.5 text-sm font-semibold">Small question image <span className="font-normal text-slate-500">Optional</span><input value={draft.questionImageUrl} onChange={(event) => setDraft({ ...draft, questionImageUrl: event.target.value })} placeholder="/images/... or https://..." className={inputClass} /></label>
      ...
      {draft.questionType === "item_list" ? <div className="grid gap-3 sm:grid-cols-3"><label className="grid gap-1.5 text-sm font-semibold">Item types / sizes<input value={draft.itemSizes} onChange={(event) => setDraft({ ...draft, itemSizes: event.target.value })} placeholder="Romex, BX or 2×4, 2×6" className={inputClass} /><span className="text-xs font-normal text-slate-500">Comma-separated pull-down choices.</span></label><label className="grid gap-1.5 text-sm font-semibold">Item lengths<input value={draft.itemLengths} onChange={(event) => setDraft({ ...draft, itemLengths: event.target.value })} placeholder="8 ft, 10 ft, 12 ft" className={inputClass} /><span className="text-xs font-normal text-slate-500">Comma-separated pull-down choices.</span></label><label className="grid gap-1.5 text-sm font-semibold">Cable numbers <span className="font-normal text-slate-500">Electrical only</span><input value={draft.cableNumbers} onChange={(event) => setDraft({ ...draft, cableNumbers: event.target.value })} placeholder="14/2, 12/2, 10/3" className={inputClass} /><span className="text-xs font-normal text-slate-500">Comma-separated cable choices.</span></label></div> : null}
