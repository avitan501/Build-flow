...
function QuestionEditor({ question, questions, index, total }: { question: MaterialQuestion; questions: MaterialQuestion[]; index: number; total: number }) {
  ...
  return <article className={`rounded-2xl border bg-white ${question.is_active ? "border-slate-200" : "border-slate-200 opacity-65"}`}>
    ...
    {open ? <div className="grid gap-4 border-t border-slate-100 p-4">
      ...
      {draft.questionType === "item_list" ? <div className="grid gap-3 sm:grid-cols-2"><label className="grid gap-1.5 text-sm font-semibold">Item sizes<input value={draft.itemSizes} onChange={(event) => setDraft({ ...draft, itemSizes: event.target.value })} placeholder="2×4, 2×6, 2×8" className={inputClass} /><span className="text-xs font-normal text-slate-500">Comma-separated pull-down choices.</span></label><label className="grid gap-1.5 text-sm font-semibold">Item lengths<input value={draft.itemLengths} onChange={(event) => setDraft({ ...draft, itemLengths: event.target.value })} placeholder="8 ft, 10 ft, 12 ft" className={inputClass} /><span className="text-xs font-normal text-slate-500">Comma-separated pull-down choices.</span></label></div> : null}
