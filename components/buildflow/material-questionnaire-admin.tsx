...
export function MaterialQuestionnaireAdmin({ categories }: { categories: MaterialQuestionnaireCategory[] }) {
  ...
  if (!selected) {
    return <div className="grid gap-5">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{categories.map((category) => <article key={category.id} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><span className="inline-flex h-11 w-11 items-center justify-center rounded-lg bg-slate-950 text-white"><Building2 className="h-5 w-5" /></span><span className={`rounded-full border px-3 py-1 text-xs font-semibold ${category.is_active ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-slate-200 bg-slate-50 text-slate-500"}`}>{category.is_active ? "Quick Order on" : "Disabled"}</span></div><h3 className="mt-5 text-lg font-bold">{category.name}</h3><p className="mt-1 text-sm text-slate-500">Connected to {category.department_key}</p><div className="mt-4 flex items-center gap-2 text-sm text-slate-600"><ListChecks className="h-4 w-4" />{category.questions.length} questions</div><button type="button" onClick={() => choose(category)} className="mt-5 min-h-11 w-full rounded-lg border border-slate-300 bg-white px-4 text-sm font-semibold text-slate-950 hover:border-sky-400 hover:bg-sky-50">Manage department</button></article>)}</div>
      {!categories.length ? <div className="rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center text-sm text-slate-500">Add your first department to begin.</div> : null}
