import { Check, ChevronDown, FileText, LockKeyhole, Truck } from "lucide-react"
...
export function RequestFulfillmentOverview({ phase, done, status, document, primaryAction, priceBreakdown, paymentDetails, deliveryDetails, deliveryLabel }: {
  ...
  return <div className="min-w-0" data-testid="fulfillment-overview">
    ...
    <section aria-label="Current client action" className="grid gap-4 rounded-xl border border-slate-200 bg-white p-4 sm:grid-cols-2 sm:items-center sm:p-5">
      <div className="min-w-0">{document ? <><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{document.label} {document.number}</p><p className="mt-2 break-words text-3xl font-bold tracking-tight text-slate-950">{new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(document.total)}</p><p className="mt-2 text-xs text-slate-500">Updated {document.updated}</p></> : <><p className="text-sm font-bold text-slate-950">Client & delivery</p><p className="mt-2 text-xs leading-5 text-slate-500">{status}</p></>}</div>
      <div className="min-w-0 sm:border-l sm:border-slate-200 sm:pl-5"><p className="mb-3 w-fit rounded-lg bg-slate-50 px-2 py-1.5 text-xs font-semibold text-slate-700" role="status">{status}</p>{primaryAction}{document ? <a href={document.href} target="_blank" rel="noreferrer" className="mt-2 flex min-h-11 items-center justify-center text-xs font-semibold text-[#0066cc] underline">View {document.label.toLowerCase()}</a> : null}</div>
    </section>
    <div className="mt-3 grid gap-2">{[
      { label: "Price breakdown", icon: FileText, content: priceBreakdown, detail: "" },
      { label: "Payment & receipt", icon: LockKeyhole, content: paymentDetails, detail: done[2] ? "Payment recorded" : "" },
      { label: "Delivery details", icon: Truck, content: deliveryDetails, detail: deliveryLabel },
    ].map(({label,icon:Icon,content,detail}) => <details key={label} className="group rounded-lg border border-slate-200 bg-white"><summary className="flex min-h-12 cursor-pointer list-none items-center gap-2 px-3 text-xs font-semibold [&::-webkit-details-marker]:hidden"><Icon className="h-4 w-4 shrink-0 text-slate-600"/><span>{label}</span><span className="ml-auto text-[10px] font-normal text-slate-500">{detail}</span><ChevronDown className="h-4 w-4 shrink-0 text-slate-500 group-open:rotate-180"/></summary><div className="border-t border-slate-100 p-3">{content}</div></details>)}</div>
  </div>
}
