          ...
          <article key={request.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                ...
                <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1 text-xs text-slate-600"><span>{DELIVERY_VEHICLES[request.vehicle as keyof typeof DELIVERY_VEHICLES]?.label || request.vehicle}</span><span>{request.providerQuote?.distanceMiles ?? request.estimate.estimatedRoadMiles} mi {request.providerQuote?.distanceMiles ? "provider route" : "planned"}</span><span>{currency.format(request.providerQuote?.total ?? request.estimate.total)} {request.providerQuote ? `${request.providerQuote.provider} quote` : "planning estimate"}</span><span>Created {formatSiteDateTime(request.createdAt)}</span>{request.scheduledPickupAt ? <span>Pickup {formatSiteDateTime(request.scheduledPickupAt, { month: "short", day: "numeric", hour: "numeric", minute: "2-digit", timeZoneName: "short" })}</span> : null}</div>
                {request.providerDelivery ? <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-900"><div className="flex flex-wrap items-center gap-3"><strong>{request.providerDelivery.provider} {request.providerDelivery.status}</strong><span>ID {request.providerDelivery.deliveryId}</span>{request.providerDelivery.driverName ? <span>Driver {request.providerDelivery.driverName}</span> : null}{request.providerDelivery.vehicleDescription ? <span>{request.providerDelivery.vehicleDescription}</span> : null}{request.providerDelivery.trackingUrl ? <a href={request.providerDelivery.trackingUrl} target="_blank" rel="noreferrer" className="font-bold underline underline-offset-4">Open live tracking</a> : null}</div>{request.providerDelivery.proofOfDeliveryUrls?.length ? <div className="mt-2 flex flex-wrap gap-2">{request.providerDelivery.proofOfDeliveryUrls.map((url, index) => <a key={url} href={url} target="_blank" rel="noreferrer" className="font-bold underline underline-offset-4">Proof of delivery {index + 1}</a>)}</div> : null}</div> : null}
              </div>
