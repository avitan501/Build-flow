...
function CopyButton({ text, label = "Copy" }: { text: string; label?: string }) {
  ...
  return <button type="button" onClick={() => void copy()} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-3 text-xs font-bold text-slate-800 transition hover:border-sky-300 hover:text-sky-800"><span className="inline-flex h-5 w-5 items-center justify-center">{copied ? <Check className="h-4 w-4 text-emerald-600" /> : <Clipboard className="h-4 w-4" />}</span>{copied ? "Copied" : label}</button>
}
  ...
  return <div className="min-h-screen bg-[#f3f3f1] text-slate-950">
    <header className="border-b border-slate-800 bg-[#071126] text-white">
      <div className="mx-auto max-w-7xl px-4 py-7 sm:px-7 sm:py-10">
        <Link href="/admin/ai-tools" className="inline-flex min-h-10 items-center text-xs font-bold text-sky-300 hover:text-white">← Manager Tools</Link>
        <div className="mt-3 grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-end">
          <div>
            ...
            <h1 className="mt-3 max-w-3xl text-4xl font-black tracking-[-0.04em] sm:text-6xl">Media &amp; Messages</h1>
            <p className="mt-4 max-w-2xl text-sm leading-6 text-slate-300 sm:text-base">Choose the audience, preview the proven asset, copy its exact approved wording, or open an editable Communications draft. Nothing sends from this page.</p>
          </div>
    ...
    <main className="mx-auto max-w-7xl px-4 py-5 sm:px-7 sm:py-8">
      <section className="sticky top-0 z-20 -mx-4 border-b border-slate-200 bg-[#f3f3f1]/95 px-4 py-3 backdrop-blur sm:-mx-7 sm:px-7" aria-label="Audience filters">
        <div className="flex gap-2 overflow-x-auto pb-1">
          {audienceFilters.map((item) => <button key={item} type="button" onClick={() => setAudience(item)} className={`min-h-10 shrink-0 rounded-full px-4 text-xs font-bold transition ${audience === item ? "bg-slate-950 text-white" : "border border-slate-300 bg-white text-slate-700"}`}>{item}</button>)}
        </div>
      </section>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {filteredStories.map((story) => <article key={story.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_14px_35px_rgba(15,23,42,0.06)]">
            ...
            <div className="p-4">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2"><CopyButton text={story.transcript} label="Copy words" /><Link href={draftHref(story.transcript)} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg bg-slate-950 px-3 text-xs font-bold text-white"><MessageSquareText className="h-4 w-4" />Open draft</Link></div>
      ...
      {filteredReview.length ? <details className="mt-10 overflow-hidden rounded-2xl border border-amber-200 bg-amber-50/60">
        ...
        <div className="grid gap-2 border-t border-amber-200 p-3 sm:grid-cols-2 sm:p-4 xl:grid-cols-3">{filteredReview.map((item) => <article key={item.id} className="rounded-xl border border-amber-200 bg-white p-3"><div className="flex items-start justify-between gap-2"><div><p className="text-[9px] font-bold uppercase tracking-[0.14em] text-amber-800">{item.audience}</p><h3 className="mt-1 text-sm font-black">{item.title}</h3></div><StatusPill status="Needs review" /></div><p className="mt-2 text-xs leading-5 text-slate-600">{item.text}</p><div className="mt-3 flex gap-2"><a href={item.src} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-9 flex-1 items-center justify-center gap-1 rounded-lg border border-slate-300 text-xs font-bold"><Play className="h-3.5 w-3.5" />Preview</a><CopyButton text={item.text} /></div><p className="mt-2 text-[10px] text-slate-500">Source: repository `/public/videos/marketing` · version not approved</p></article>)}</div>
      </details> : null}
