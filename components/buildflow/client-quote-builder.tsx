      ...
      {!selectedBid ? (
        ...
        <>
          ...
          <div className="grid gap-4 p-5 sm:px-6">
            <label className="grid gap-1.5 text-xs font-bold text-slate-600">Message on client quote <span className="font-medium text-slate-400">Optional · always shown in full</span><textarea value={clientMessage} onChange={(event) => setClientMessage(event.target.value)} rows={4} placeholder="Delivery terms, exclusions, or a short note to the client" className="min-h-28 resize-y rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium leading-6" /></label>
        ...
        <div className="fixed inset-0 z-[100] grid place-items-center overflow-y-auto bg-slate-950/60 p-4" role="dialog" aria-modal="true" aria-labelledby="client-preview-title">
          <div className="my-8 w-full max-w-3xl overflow-hidden rounded-xl bg-white shadow-2xl">
            ...
            <div className="p-5 sm:p-8">
              ...
              {clientMessage ? <p className="mt-6 whitespace-pre-wrap break-words rounded-lg bg-slate-50 p-4 text-sm leading-6 text-slate-600">{clientMessage}</p> : null}
              <div className="mt-6 border-t border-slate-200 pt-5"><p className="text-xs font-bold uppercase tracking-[0.1em] text-slate-500">Terms &amp; conditions</p><p className="mt-2 whitespace-pre-wrap break-words text-xs leading-5 text-slate-600">{DEFAULT_PROPOSAL_TERMS}</p></div>
              <div className="mt-7 flex flex-col gap-3 border-t border-slate-200 pt-5 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between"><span>Avantia Build · (516) 990-1990 · office@avantiabuild.com</span><span className="inline-flex items-center gap-1 font-bold text-[#0066cc]">avantiabuild.com <ArrowRight className="h-3.5 w-3.5" /></span></div>
            </div>
