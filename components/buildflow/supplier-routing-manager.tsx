...
type SupplierDirectoryGroup = "trial" | "verified"
...
export function SupplierRoutingManager({
  ...
  const groupedDirectorySuppliers = useMemo(
    () => supplierDirectoryGroup === "verified" ? verifiedSuppliers : trialSuppliers,
    [supplierDirectoryGroup, trialSuppliers, verifiedSuppliers],
  )
  ...
  function addSupplier() {
    ...
    const supplier: SupplierRoutingOption = {
      ...
      deliveryCharge: supplierDraft.deliveryCharge.trim() ? Number(supplierDraft.deliveryCharge) : null,
      deliveryChargeNote: supplierDraft.deliveryChargeNote.trim(),
  ...
  function addTrialVendor(entry: TrialVendorEntry) {
    if (supplierDirty) {
      setTrialCatalogStatus("Save the current supplier changes before adding a trial supplier.")
      return
    }
    ...
    startSupplierSave(async () => {
      try {
        ...
        setSupplierDirty(false)
        setDirectorySaveState("saved")
        setTrialCatalogStatus(`${result.supplier.name} was added as a first-time trial supplier.`)
      } catch {
        setTrialCatalogStatus("The server could not add this trial supplier. Please try again.")
      }
    })
  }
              ...
              <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_14px_42px_rgba(15,23,42,0.07)]">
                <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-slate-500">Approved relationship leads automatically move into Verified Suppliers.</p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Link href="/admin/supplier-network" className="inline-flex min-h-10 items-center rounded-lg border border-sky-200 bg-sky-50 px-3 text-xs font-semibold text-sky-800 hover:bg-sky-100">Open relationship leads</Link>
                ...
                <div className="mb-3 grid grid-cols-2 gap-1 rounded-lg border border-slate-200 bg-slate-50 p-1" aria-label="Supplier directory groups">
                ...
                <label className="relative block">
                  ...
                  <input
                    ...
                    placeholder={`Search ${supplierDirectoryGroup} suppliers`}
                ...
                <div className="grid gap-2">
                  ...
                  {filteredDirectorySuppliers.map((supplier) => (
                    ...
                    >
                      <span className="flex items-start justify-between gap-2"><span className="min-w-0 truncate text-sm font-semibold">{supplier.name}</span><SupplierProgramBadges channels={supplier.programChannels ?? []} /></span>
                  ...
                  <button
                    ...
                    onClick={() => {
                      setSupplierFormError("")
                      setSupplierDraft((draft) => ({ ...draft, trustLevel: supplierDirectoryGroup === "verified" ? "verified" : "first-time" }))
                      setSupplierAddOpen(true)
                    }}
                    className={`flex min-h-14 items-center gap-3 rounded-[18px] border border-dashed px-4 py-3 text-left transition ${supplierDirectoryGroup === "verified" ? "border-emerald-300 bg-emerald-50 text-emerald-800 hover:border-emerald-500 hover:bg-emerald-100" : "border-sky-300 bg-sky-50 text-sky-800 hover:border-sky-500 hover:bg-sky-100"}`}
                  >
                    ...
                    <span><span className="block text-sm font-semibold">Add {supplierDirectoryGroup} supplier</span><span className="mt-0.5 block text-xs opacity-80">{supplierDirectoryGroup === "verified" ? "Create a supplier you already work with" : "Create a new supplier lead"}</span></span>
                  </button>
                  {supplierDirectoryGroup === "trial" ? <button
                    type="button"
                    onClick={() => {
                      setTrialCatalogStatus("")
                  ...
                  >
                    ...
                    <span><span className="block text-sm font-semibold">Add Trial Suppliers</span><span className="mt-0.5 block text-xs text-slate-600">Open the reusable {TRIAL_VENDOR_ENTRY_COUNT}-entry supplier-lead catalog</span></span>
                  </button> : null}
                </div>
              </section>
                ...
                <div className="fixed inset-0 z-[110] flex items-end justify-center bg-slate-950/45 p-3 backdrop-blur-[2px] sm:items-center" role="dialog" aria-modal="true" aria-labelledby="trial-vendor-catalog-title" onMouseDown={(event) => { if (event.currentTarget === event.target && !supplierSavePending) setTrialCatalogOpen(false) }}>
                  <section className="flex max-h-[94vh] w-full max-w-5xl flex-col overflow-hidden rounded-[22px] bg-white shadow-[0_28px_80px_rgba(15,23,42,0.35)]">
                    <header className="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 bg-white px-5 py-4 sm:px-6">
                      <div>
                        <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-emerald-700">Trial supplier catalog</p>
                        <h2 id="trial-vendor-catalog-title" className="mt-1 text-xl font-bold text-slate-950">Choose a trial supplier</h2>
                      ...
                      <button type="button" onClick={() => setTrialCatalogOpen(false)} disabled={supplierSavePending} aria-label="Close trial supplier catalog" className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-slate-200 text-slate-600 disabled:opacity-40"><X className="h-5 w-5" /></button>
                    </header>
                    ...
                    <div className="shrink-0 border-b border-slate-200 p-4 sm:px-6">
                      <div className="mb-4 rounded-lg border border-sky-200 bg-sky-50 px-4 py-3 text-sm leading-6 text-sky-950">
                        <strong>Supplier leads, not verified suppliers.</strong> Adding a company places it in Trial Suppliers. Deleted trial suppliers stay hidden and cannot return from an old browser request.
                      </div>
                      <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_15rem]">
                        <label className="relative block">
                          <Search aria-hidden="true" className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                          <span className="sr-only">Search trial suppliers</span>
                        ...
                        <label>
                          <span className="sr-only">Filter trial suppliers by department</span>
                ...
                <div className="fixed inset-0 z-[110] flex items-end justify-center bg-slate-950/45 p-3 backdrop-blur-[2px] sm:items-center" role="dialog" aria-modal="true" aria-labelledby="add-supplier-title" onMouseDown={(event) => { if (event.currentTarget === event.target && !supplierSavePending) setSupplierAddOpen(false) }}>
                  <section className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[22px] bg-white shadow-[0_28px_80px_rgba(15,23,42,0.35)]">
                    ...
                    <div className="grid gap-4 p-5 sm:grid-cols-2 sm:p-6">
                      ...
                      <fieldset className="sm:col-span-2"><legend className="text-sm font-semibold text-slate-900">Supplier programs</legend><p className="mt-1 text-xs font-normal text-slate-500">Choose every way Avantia can work with this supplier.</p><div className="mt-2 flex flex-wrap gap-2">{SUPPLIER_PROGRAM_CHANNELS.map((channel) => { const checked = supplierDraft.programChannels.includes(channel); return <label key={channel} className={`inline-flex min-h-10 cursor-pointer items-center gap-2 rounded-lg border px-3 text-xs font-bold ${checked ? SUPPLIER_PROGRAM_COLORS[channel] : "border-slate-200 bg-white text-slate-500"}`}><input type="checkbox" checked={checked} onChange={(event) => setSupplierDraft((draft) => ({ ...draft, programChannels: event.target.checked ? [...draft.programChannels, channel] : draft.programChannels.filter((entry) => entry !== channel) }))} className="h-4 w-4 accent-[#0071e3]" />{channel}</label> })}</div></fieldset>
                      ...
                      <label className="grid gap-2 text-sm font-semibold text-slate-900">Delivery charge <span className="relative"><span className="pointer-events-none absolute left-4 top-3.5 text-slate-400">$</span><input type="number" min="0" step="0.01" value={supplierDraft.deliveryCharge} onChange={(event) => setSupplierDraft((draft) => ({ ...draft, deliveryCharge: event.target.value }))} placeholder="0.00" className="min-h-12 w-full rounded-xl border border-slate-300 pl-8 pr-4 text-sm font-medium outline-none focus:border-sky-400 focus:ring-4 focus:ring-sky-100" /></span></label>
                      <label className="grid gap-2 text-sm font-semibold text-slate-900">Delivery price note <input value={supplierDraft.deliveryChargeNote} onChange={(event) => setSupplierDraft((draft) => ({ ...draft, deliveryChargeNote: event.target.value }))} placeholder="Free over $500, per truck…" className="min-h-12 rounded-xl border border-slate-300 px-4 text-sm font-medium outline-none focus:border-sky-400 focus:ring-4 focus:ring-sky-100" /></label>
                      <label className="grid gap-2 text-sm font-semibold text-slate-900 sm:col-span-2">Delivery or contact instructions <span className="text-xs font-normal text-slate-500">Optional · separate from the delivery price</span><textarea value={supplierDraft.deliveryNotes} onChange={(event) => setSupplierDraft((draft) => ({ ...draft, deliveryNotes: event.target.value }))} rows={3} placeholder="Best hours, quote process, delivery area, or other instructions" className="rounded-xl border border-slate-300 px-4 py-3 text-sm font-medium outline-none focus:border-sky-400 focus:ring-4 focus:ring-sky-100" /></label>
                      <div className="sm:col-span-2">
