...
const dateText = (value: string | null) => value ? formatSiteDate(value.slice(0, 10)) : "—";
  ...
  return <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm" aria-labelledby="affiliate-goal-title">
    ...
    {panel === "readiness" ? <ReadinessPanel settings={settings} run={run} pending={isPending} /> : <>
      ...
      {integrations.length ? <div className="m-4 grid gap-2">{integrations.map((integration) => {
        const approved = integration.status === "Approved";
        const title = integration.supplier_name === "Lowe's" ? "Lowe’s Developer/API Onboarding" : `${integration.supplier_name} ${integration.relationship_type}`;
        return <div key={integration.id} className={`grid gap-3 border-l-4 p-3 text-sm sm:grid-cols-[minmax(0,1fr)_auto] ${approved ? "border-emerald-500 bg-emerald-50" : "border-amber-400 bg-amber-50"}`}>
          <div><div className="flex flex-wrap items-center gap-2"><strong>{title}</strong><span className={`rounded-full border px-2 py-0.5 text-[11px] font-semibold ${approved ? "border-emerald-300 text-emerald-800" : "border-amber-300 text-amber-800"}`}>{integration.status}</span></div><p className="mt-1 text-slate-700">{integration.submitted_at ? `Submitted ${dateText(integration.submitted_at)} · ` : ""}{integration.current_stage}. {integration.next_action}</p></div><p className={`self-center text-xs font-semibold ${approved ? "text-emerald-900" : "text-amber-900"}`}>{integration.supplier_name === "Impact Marketplace" ? "Brand approvals are separate" : "Separate from affiliate application"}</p>
        </div>;
      })}</div> : null}
      ...
      <details className="border-y border-slate-200 bg-slate-50">
        <summary className="flex min-h-9 cursor-pointer list-none items-center justify-between px-3 text-[11px] font-bold text-slate-600"><span>Search &amp; filters</span><span>{filtered.length}/{topPrograms.length}</span></summary>
        <div className="grid gap-2 border-t border-slate-200 p-2 sm:grid-cols-2 lg:grid-cols-[minmax(13rem,1fr)_repeat(4,minmax(8rem,auto))]">
          <label className="relative"><Search className="absolute left-2.5 top-2.5 h-3.5 w-3.5 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search supplier" className="h-8 w-full rounded-md border border-slate-300 bg-white pl-8 pr-2 text-xs" /></label>
          <select value={status} onChange={(event) => setStatus(event.target.value)} className="h-8 rounded-md border border-slate-300 bg-white px-2 text-xs"><option>All</option>{AFFILIATE_STATUSES.map((item) => <option key={item}>{item}</option>)}</select>
          <select value={priority} onChange={(event) => setPriority(event.target.value)} className="h-8 rounded-md border border-slate-300 bg-white px-2 text-xs"><option>All</option><option>A</option><option>B</option><option>C</option></select>
          <select value={network} onChange={(event) => setNetwork(event.target.value)} className="h-8 rounded-md border border-slate-300 bg-white px-2 text-xs"><option>All</option>{networks.map((item) => <option key={item}>{item}</option>)}</select>
          <select value={sort} onChange={(event) => setSort(event.target.value)} className="h-8 rounded-md border border-slate-300 bg-white px-2 text-xs"><option value="priority">Sort: priority</option><option value="status">Status</option><option value="commission">Commission</option><option value="cookie">Cookie duration</option><option value="ease">Application ease</option><option value="fit">AvantiaBuild fit</option><option value="followup">Next follow-up</option><option value="updated">Last updated</option></select>
        </div>
      </details>
...
function ProgramDrawer({ program, checklist, activities, attachments, close, run, pending }: { program: AffiliateProgram; checklist: AffiliateChecklistItem[]; activities: AffiliateActivity[]; attachments: AffiliateAttachment[]; close: () => void; run: (op: () => Promise<{ ok: boolean; error?: string }>, success: string) => void; pending: boolean }) {
  ...
  const [affiliateTestUrl, setAffiliateTestUrl] = useState(program.affiliate_test_url ?? "");
  ...
  return <div className="fixed inset-0 z-50 bg-slate-950/45" role="dialog" aria-modal="true"><div className="absolute inset-y-0 right-0 flex w-full max-w-3xl flex-col bg-white shadow-2xl">
    ...
    <div className="flex-1 overflow-y-auto p-4">
      {program.affiliate_test_url ? <AffiliateDisclosure className="mb-2" /> : null}
      <div className="flex flex-wrap gap-2">{program.affiliate_test_url ? <a href={program.affiliate_test_url} target="_blank" rel="sponsored noopener noreferrer" aria-label={`Open ${program.supplier_name} affiliate link in a new tab`} className="inline-flex min-h-10 items-center gap-2 rounded-md bg-emerald-700 px-4 text-sm font-semibold text-white">Open affiliate link<ArrowUpRight className="h-4 w-4" aria-hidden="true" /></a> : null}<a href={program.application_url} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-10 items-center gap-2 rounded-md bg-slate-950 px-4 text-sm font-semibold text-white">Start application<ArrowUpRight className="h-4 w-4" /></a><a href={program.application_url} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-10 items-center gap-2 rounded-md border border-slate-300 px-4 text-sm font-semibold">Open program<ArrowUpRight className="h-4 w-4" /></a><select value="" onChange={(event) => event.target.value && changeStatus(event.target.value as AffiliateStatus)} className="h-10 rounded-md border border-slate-300 px-3 text-sm font-semibold"><option value="">Change status…</option>{AFFILIATE_STATUSES.map((item) => <option key={item}>{item}</option>)}</select></div>
...
function StatusForm({ program, status, close, run }: { program: AffiliateProgram; status: "Applied" | "Approved"; close: () => void; run: (op: () => Promise<{ ok: boolean; error?: string }>, success: string) => void }) {
  const [date, setDate] = useState(() => siteBusinessDateKey() ?? ""); const [email, setEmail] = useState(""); const [confirmation, setConfirmation] = useState(false); const [followUp, setFollowUp] = useState(() => addSiteCalendarDays(siteBusinessDateKey() ?? "", 14) ?? "");
