...
export function ProductQuoteCard({ row, onSelect, onClear, onReview }: { row: ProductRow; onSelect: (bidId: string) => void; onClear: () => void; onReview?: () => void }) {
  ...
  return <article className="min-w-0 overflow-hidden rounded-xl border border-slate-200 bg-white" data-testid="product-quote-card">
    ...
    <summary className="flex min-h-20 cursor-pointer list-none items-center gap-3 px-4 py-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-sky-500 [&::-webkit-details-marker]:hidden"><div className="min-w-0 flex-1">
      ...
      {row.selected ? <p className="mt-1 break-words text-xs text-sky-800">Draft · {row.selected.bid.supplier_name_snapshot} · {formatComparisonMoney(row.selected.unitPrice!)} / {row.item.unit}</p> : <p className="mt-1 text-xs text-slate-500">{row.offers.length} supplier responses{reviewCount ? ` · ${reviewCount} need review` : ""}</p>}
