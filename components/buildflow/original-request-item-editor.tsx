...
export function OriginalRequestItemEditor({ requestId, item, mode = "edit" }: { requestId: string; item?: ReviewableMaterialItem; mode?: "edit" | "add" }) {
  ...
  const autosave = useSequencedAutosave<ItemDraft>({
    save: (value, version) => saveOriginalMaterialItemAction({ requestId, itemId: item?.id, name: value.name, quantity: Number(value.quantity), unit: value.unit, details: value.details, version }),
  ...
  function save() {
    ...
    startTransition(async () => {
      const result = await saveOriginalMaterialItemAction({ requestId, itemId: item?.id, name: draft.name, quantity: Number(draft.quantity), unit: draft.unit, details: draft.details })
