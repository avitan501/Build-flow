...
export function OriginalRequestItemEditor({ requestId, item, mode = "edit", itemKind = "original", trigger = "button", children }: { requestId: string; item?: ReviewableMaterialItem; mode?: "edit" | "add"; itemKind?: "original" | "organized"; trigger?: "button" | "content"; children?: ReactNode }) {
  ...
  return <div className={trigger === "content" ? "block w-full" : "inline-flex"}>
    ...
    {open ? <div className="fixed inset-0 z-[180] grid place-items-center bg-slate-950/45 p-3" role="dialog" aria-modal="true" aria-label={mode === "add" ? "Add original request item" : "Edit request item"} onMouseDown={(event) => { if (event.target === event.currentTarget && !pending) setOpen(false) }}><section className="w-full max-w-md rounded-xl bg-white shadow-2xl"><header className="flex items-center justify-between border-b border-slate-200 px-4 py-3"><div><p className="text-[9px] font-bold uppercase tracking-[.14em] text-[#0066cc]">{itemKind === "organized" ? "AI organized" : "Original request"}</p><h3 className="text-base font-bold">{mode === "add" ? "Add item" : "Edit item"}</h3></div><button type="button" onClick={() => setOpen(false)} aria-label="Close" className="inline-flex h-11 w-11 items-center justify-center rounded-md border border-slate-200 sm:h-9 sm:w-9"><X className="h-4 w-4" /></button></header><div className="grid gap-3 p-4 sm:grid-cols-[6rem_7rem_minmax(0,1fr)]"><label className="grid gap-1 text-xs font-bold">Quantity<input type="number" min="0.01" step="0.01" value={draft.quantity} onChange={(event) => updateDraft({ quantity: event.target.value })} className="h-11 rounded-md border border-slate-300 px-2 font-normal sm:h-10" /></label><label className="grid gap-1 text-xs font-bold">Unit<input value={draft.unit} onChange={(event) => updateDraft({ unit: event.target.value })} className="h-11 rounded-md border border-slate-300 px-2 font-normal sm:h-10" /></label><label className="grid gap-1 text-xs font-bold sm:col-span-1">Item<input autoFocus value={draft.name} onChange={(event) => updateDraft({ name: event.target.value })} className="h-11 rounded-md border border-slate-300 px-2 font-normal sm:h-10" /></label><label className="grid gap-1 text-xs font-bold sm:col-span-3">Details<textarea rows={3} value={draft.details} onChange={(event) => updateDraft({ details: event.target.value })} className="rounded-md border border-slate-300 p-2 font-normal" /></label>{feedback ? <p className="text-xs font-bold text-rose-700 sm:col-span-3">{feedback}</p> : null}</div><footer className="flex items-center justify-between gap-2 border-t border-slate-200 bg-slate-50 p-3">{mode === "edit" ? <AutosaveStatus status={autosave.status} error={autosave.error} retry={autosave.retry} /> : <span /> }<div className="flex gap-2"><button type="button" onClick={() => setOpen(false)} disabled={pending} className="h-11 rounded-md border border-slate-300 bg-white px-3 text-xs font-bold">{mode === "add" ? "Cancel" : "Close"}</button>{mode === "add" ? <button type="button" onClick={save} disabled={pending || !valid(draft)} className="inline-flex h-11 items-center gap-1 rounded-md bg-slate-950 px-4 text-xs font-bold text-white disabled:opacity-40"><Check className="h-3.5 w-3.5" />{pending ? "Adding…" : "Add item"}</button> : null}</div></footer></section></div> : null}
  </div>
}
