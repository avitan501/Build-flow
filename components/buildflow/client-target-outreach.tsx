...
export const LEAD_GROUPS = [
  ...
  { value: 4, label: "Worked with" },
  { value: 5, label: "Active contractor" },
] as const;

function leadGroupLabel(level: number) {
  return LEAD_GROUPS.find((group) => group.value === level)?.label ?? "New lead";
}
      ...
      <div className="fixed inset-0 z-[165] grid place-items-center overflow-y-auto bg-slate-950/50 p-4" role="dialog" aria-modal="true" aria-labelledby={`edit-lead-${lead.id}`} onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
        <section className="my-auto w-full max-w-lg overflow-hidden rounded-lg bg-white shadow-2xl">
          ...
          <div className="grid max-h-[65vh] gap-4 overflow-y-auto p-5 sm:grid-cols-2">
            ...
            <label className="grid gap-1.5 text-sm font-semibold">Lead group<select value={form.relationshipLevel} onChange={(event) => setForm((current) => ({ ...current, relationshipLevel: Number(event.target.value) }))} className="min-h-11 rounded-md border border-slate-300 bg-white px-3 font-normal">{LEAD_GROUPS.map((group) => <option key={group.value} value={group.value}>{group.label}</option>)}</select></label>
...
export function OutreachLeadDirectory({ leads, conversations, senderName }: { leads: OutreachLeadRecord[]; conversations: Record<string, DirectoryConversationEntry[]>; senderName: string }) {
  ...
  return <section className="grid gap-3" aria-label="Leads">
    {leads.map((lead) => <article key={lead.id} className="grid min-w-0 gap-3 rounded-lg border border-slate-200 bg-white p-3 shadow-sm lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.85fr)]">
      <div className="min-w-0">
        <div className="flex min-w-0 items-start gap-3">
          <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><EditOutreachLead lead={lead}>{lead.full_name}</EditOutreachLead><span className="rounded-full bg-sky-50 px-2 py-0.5 text-[10px] font-bold text-[#0066cc]">{leadGroupLabel(lead.relationship_level)}</span></div><p className="mt-1 truncate text-sm text-slate-600">{lead.company_name || "No company"}</p><p className="mt-1 break-all text-xs text-slate-500">{[lead.phone, lead.email].filter(Boolean).join(" · ") || "Contact details needed"}</p></div>
          <div className="ml-auto flex max-w-[70%] shrink-0 items-center gap-1 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"><select aria-label={`Group for ${lead.full_name}`} defaultValue={lead.relationship_level} disabled={pending} onChange={(event) => run(() => updateOutreachLeadRelationshipAction({ id: lead.id, relationshipLevel: Number(event.target.value) }))} className="h-9 w-[8.5rem] shrink-0 rounded-md border border-slate-300 bg-white px-2 text-xs font-semibold">{LEAD_GROUPS.map((group) => <option key={group.value} value={group.value}>{group.label}</option>)}</select><select aria-label={`Language for ${lead.full_name}`} defaultValue={lead.preferred_language} disabled={pending} onChange={(event) => run(() => updateClientLanguageAction({ id: lead.id, target: "lead", language: event.target.value }))} className="h-9 w-16 shrink-0 rounded-md border border-slate-300 bg-white px-2 text-xs font-semibold"><option value="en">EN</option><option value="es">ES</option></select><select aria-label={`Status for ${lead.full_name}`} defaultValue={lead.status} disabled={pending} onChange={(event) => run(() => updateOutreachLeadStatusAction({ id: lead.id, status: event.target.value }))} className="h-9 w-[8.5rem] shrink-0 rounded-md border border-slate-300 bg-white px-2 text-xs font-semibold"><option value="new">New</option><option value="contacted">Contacted</option><option value="qualified">Qualified</option><option value="not_interested">Not interested</option></select><ContactActions name={lead.full_name} phone={lead.phone} email={lead.email} senderName={senderName} showWelcomePackageButton /><button type="button" disabled={pending} onClick={() => window.confirm(`Remove ${lead.full_name} from leads?`) && run(() => deleteOutreachLeadAction(lead.id))} aria-label={`Remove ${lead.full_name}`} className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-md text-slate-400 hover:bg-rose-50 hover:text-rose-600"><Trash2 className="h-4 w-4" /></button></div>
        </div>
