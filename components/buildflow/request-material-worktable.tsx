...
import { ChevronDown, Copy, FileText, Layers3, MoreHorizontal, Route, Search, Sparkles } from "lucide-react"
  ...
  return (
    <section className="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_8px_24px_rgba(15,23,42,0.05)]" aria-labelledby="request-items-heading">
      ...
      {rawDraft ? <div data-testid="original-request-draft" className="border-b border-slate-200 bg-slate-50/70 px-3 py-3 sm:px-4">
        <div className="flex items-center justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2"><p className="text-[10px] font-extrabold uppercase tracking-[.1em] text-slate-500">Original request draft</p><span className={`rounded-full px-1.5 py-0.5 text-[9px] font-bold ${draftChanged ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"}`}>{draftChanged ? "AI needs refresh" : "Saved"}</span></div><p className="mt-1 line-clamp-3 whitespace-pre-line text-xs leading-5 text-slate-700">{rawDraftText || "The request is in the attached photo or document."}</p></div><OriginalRequestItemEditor requestId={requestId} item={rawDraft} buttonLabel="Edit draft" /></div>
      ...
      {selectedRouteIds.length ? <div className="sticky top-0 z-30 grid gap-2 border-b border-sky-200 bg-sky-50 p-3 shadow-sm sm:grid-cols-[auto_12rem_minmax(10rem,1fr)_auto] sm:items-center sm:p-2"><span className="text-xs font-bold text-sky-900 sm:text-[10px]">{selectedRouteIds.length} items selected</span><select value={batchSupplier} onChange={(event) => setBatchSupplier(event.target.value)} className="h-11 rounded-md border border-sky-200 bg-white px-2 text-xs font-semibold sm:h-9 sm:text-[11px]"><option value="">Choose supplier…</option>{orderedSuppliers.map((supplier) => <option key={supplier.id} value={supplier.name}>{supplier.name}</option>)}</select><input value={batchManual} onChange={(event) => setBatchManual(event.target.value)} placeholder="Or type supplier names" className="h-11 rounded-md border border-sky-200 px-2 text-xs sm:h-9 sm:text-[11px]" /><button type="button" onClick={applyBatchRoute} disabled={batchPending || (!batchSupplier && !batchManual.trim())} className="inline-flex h-11 items-center justify-center gap-1 rounded-md bg-[#0071e3] px-3 text-xs font-bold text-white disabled:opacity-40 sm:h-9 sm:text-[11px]"><Route className="h-3.5 w-3.5" />Send group to supplier</button>{batchFeedback ? <p className="text-[10px] font-bold text-rose-700 sm:col-span-4">{batchFeedback}</p> : null}</div> : null}
        ...
        <div className="grid gap-3 bg-slate-50/70 p-2.5 sm:p-3" aria-label="AI organized request items">
          {rowGroups.map((group) => {
            ...
            return <section key={group.label} className="overflow-hidden rounded-lg border border-slate-200 bg-white">
              ...
              <div className="divide-y divide-slate-100">{group.rows.map(({ item, sourceItem, hasAi }, index) => {
                ...
                const details = requestItemFieldSummary(item.metadata).slice(0, 4)
                ...
                return <article key={item.id}>
                  <div className="grid min-h-14 grid-cols-[2.5rem_3rem_minmax(0,1fr)_2.5rem] items-center gap-2 px-2 py-2 sm:grid-cols-[2.5rem_2rem_3rem_minmax(13rem,1.4fr)_minmax(10rem,1fr)_8rem_minmax(12rem,1fr)_2.5rem] sm:px-3">
                    ...
                    <div className="min-w-0"><div className="flex items-baseline gap-2"><p className="truncate text-sm font-extrabold text-slate-950">{item.name}</p><span className="shrink-0 text-[10px] font-bold text-slate-500">{materialQuantity(item)} {materialSalesUnit(item)}</span></div><div className="mt-1 flex min-w-0 gap-1 overflow-hidden"><span className={`shrink-0 rounded px-1.5 py-0.5 text-[9px] font-extrabold sm:hidden ${missing ? "bg-amber-50 text-amber-800" : "bg-emerald-50 text-emerald-700"}`}>{missing ? `${reasons.length} missing` : "Ready"}</span>{details.length ? details.map((detail) => <span key={detail} title={detail} className="max-w-32 truncate rounded bg-slate-100 px-1.5 py-0.5 text-[9px] font-semibold text-slate-600">{detail}</span>) : <span className="text-[9px] font-semibold text-slate-400">{hasAi ? "No extra details" : "Awaiting AI"}</span>}</div></div>
