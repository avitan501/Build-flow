          ...
          <table className="block w-full border-collapse text-left sm:table sm:min-w-[58rem] sm:table-fixed">
            ...
            <tbody className="block space-y-3 bg-slate-100/70 p-3 sm:table-row-group sm:divide-y sm:divide-slate-100 sm:space-y-0 sm:bg-transparent sm:p-0">
              {rows.map(({ item, sourceItem, showSource, hasAi }) => {
                ...
                return (
                  <tr key={item.id} className="grid overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_4px_14px_rgba(15,23,42,0.06)] sm:table-row sm:overflow-visible sm:rounded-none sm:border-0 sm:align-top sm:shadow-none">
                    <td className="order-2 block border-t border-slate-100 bg-white p-0 sm:table-cell sm:px-3 sm:py-3 md:sticky md:left-0 md:z-10 md:shadow-[6px_0_10px_-10px_rgba(15,23,42,.35)]">
                      ...
                      <div id={originalSectionId} className={`${originalExpanded ? "block" : "hidden"} border-t border-slate-100 px-3 py-3 ${showSource ? "sm:block" : "sm:hidden"} sm:border-0 sm:p-0`}>
                        {sourceItem ? <><div className={`mb-1 items-center justify-between gap-2 ${showSource ? "sm:flex" : "sm:hidden"}`}><label className="hidden items-center gap-1 text-[9px] font-bold text-slate-500 sm:inline-flex"><input type="checkbox" checked={selectedRouteIds.includes(item.id)} onChange={(event) => setSelectedRouteIds((current) => event.target.checked ? [...new Set([...current, item.id])] : current.filter((id) => id !== item.id))} />Route</label><div className="max-sm:[&_button]:min-h-11 max-sm:[&_input:not([type])]:min-h-11 max-sm:[&_input[type=number]]:min-h-11"><OriginalRequestItemEditor requestId={requestId} item={sourceItem} /></div></div><p className="text-sm font-extrabold tabular-nums text-slate-950">{materialQuantity(sourceItem)} <span className="text-xs font-semibold text-slate-500">{materialSalesUnit(sourceItem)}</span></p><p className="text-sm font-bold text-slate-950">{sourceItem.name}</p>{itemDetails(sourceItem) ? <p title={itemDetails(sourceItem)} className="mt-0.5 line-clamp-4 cursor-help text-xs leading-5 text-slate-500">{itemDetails(sourceItem)}</p> : null}</> : <p className="text-xs font-semibold text-amber-700">Original link unavailable</p>}
                      </div>
                      {sourceItem && !showSource ? <p className="hidden text-xs font-semibold text-slate-400 sm:block">Same original item</p> : null}
                    </td>
                    <td className="order-1 block px-3 py-3 sm:table-cell sm:border-l sm:border-slate-100">
                      ...
                      {hasAi ? <><p className="text-xl font-extrabold tabular-nums text-slate-950 sm:text-sm">{materialQuantity(item)} <span className="text-sm font-semibold text-slate-500 sm:text-xs">{materialSalesUnit(item)}</span></p><p className="mt-0.5 text-base font-bold text-slate-950 sm:mt-0 sm:text-sm">{item.name}</p>{itemDetails(item) ? <p className="mt-1 text-xs leading-5 text-slate-500 sm:mt-0.5">{itemDetails(item)}</p> : null}<button type="button" onClick={() => setPriceItemId(priceOpen ? null : item.id)} aria-expanded={priceOpen} className="mt-1 inline-flex min-h-11 items-center gap-1 text-xs font-bold text-[#0066cc]"><Search className="h-3.5 w-3.5" />Online prices</button>{priceOpen ? <MaterialPriceCheck requestId={requestId} query={materialSearchQuery(item)} department={item.department} defaultZipCode={defaultZipCode} onClose={() => setPriceItemId(null)} /> : null}</> : <p className="text-xs font-semibold text-slate-400">Not organized yet</p>}
                    </td>
                    <td aria-label={!missing ? "No missing information — Ready" : undefined} className={`order-3 block border-t border-slate-100 p-0 sm:table-cell sm:border-l sm:px-3 sm:py-3 ${missing ? "bg-amber-50/70" : ""}`}>
                    ...
                    <td className="order-4 block border-t border-slate-100 px-3 py-3 sm:table-cell sm:border-l sm:px-2 sm:py-2"><p className="mb-2 text-[10px] font-bold uppercase tracking-[.08em] text-slate-500 sm:hidden">Supplier route</p><div className="max-sm:[&_button]:min-h-11 max-sm:[&_input:not([type])]:min-h-11 max-sm:[&_label]:min-h-11 max-sm:[&_select]:min-h-11"><RequestSupplierRouteEditor key={`${item.id}-${supplierRouteVersion(item.metadata)}`} requestId={requestId} itemId={item.id} itemIds={items.map((requestItem) => requestItem.id)} metadata={item.metadata} suppliers={orderedSuppliers} /></div></td>
                  </tr>
                )
              })}
            </tbody>
