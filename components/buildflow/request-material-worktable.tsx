...
import { Check, ChevronDown, Copy, FileText, Info, MoreHorizontal, Route, Search } from "lucide-react"
                  ...
                  <tr key={item.id} className="grid overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_4px_14px_rgba(15,23,42,0.06)] sm:table-row sm:overflow-visible sm:rounded-none sm:border-0 sm:align-middle sm:shadow-none">
                    <td className="order-2 block border-t border-slate-100 bg-white p-0 sm:table-cell sm:px-3 sm:py-1.5 md:sticky md:left-0 md:z-10 md:shadow-[6px_0_10px_-10px_rgba(15,23,42,.35)]">
                      ...
                      <div id={originalSectionId} className={`${originalExpanded ? "block" : "hidden"} border-t border-slate-100 px-3 py-3 ${showSource ? "sm:block" : "sm:hidden"} sm:border-0 sm:p-0`}>
                        {sourceItem ? <><div className={`mb-0.5 items-center justify-between gap-2 ${showSource ? "sm:flex" : "sm:hidden"}`}><label className="hidden items-center gap-1 text-[9px] font-bold text-slate-500 sm:inline-flex"><input type="checkbox" checked={selectedRouteIds.includes(item.id)} onChange={(event) => setSelectedRouteIds((current) => event.target.checked ? [...new Set([...current, item.id])] : current.filter((id) => id !== item.id))} />Route</label></div><OriginalRequestItemEditor requestId={requestId} item={sourceItem} trigger="content"><div className="flex items-start justify-between gap-2 p-1 sm:p-0"><div className="min-w-0">{isRawFreeTextContainer(sourceItem) ? <><p className="text-sm font-bold leading-4 text-slate-950">Original request draft</p><p className="mt-1 line-clamp-2 whitespace-pre-line text-[11px] leading-4 text-slate-500">{String(sourceItem.metadata?.request_details ?? "") || "Photo or document request"}</p></> : <><p className="text-sm font-extrabold tabular-nums text-slate-950">{materialQuantity(sourceItem)} <span className="text-xs font-semibold text-slate-500">{materialSalesUnit(sourceItem)}</span></p><p className="text-sm font-bold leading-4 text-slate-950">{sourceItem.name}</p></>}</div>{itemDetails(sourceItem) ? <span title={itemDetails(sourceItem)} aria-label={`Item details: ${itemDetails(sourceItem)}`} className="inline-flex h-7 w-7 shrink-0 cursor-help items-center justify-center rounded-full text-slate-400 hover:bg-sky-50 hover:text-[#0066cc]"><Info className="h-3.5 w-3.5" /></span> : null}</div></OriginalRequestItemEditor></> : <p className="text-xs font-semibold text-amber-700">Original link unavailable</p>}
                      </div>
                    ...
                    <td className="order-1 block px-3 py-3 sm:table-cell sm:border-l sm:border-slate-100 sm:py-1.5">
                      ...
                      {hasAi ? <><OriginalRequestItemEditor requestId={requestId} item={item} itemKind="organized" trigger="content"><div className="p-1 sm:p-0"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="text-xl font-extrabold tabular-nums text-slate-950 sm:text-sm">{materialQuantity(item)} <span className="text-sm font-semibold text-slate-500 sm:text-xs">{materialSalesUnit(item)}</span></p><p className="mt-0.5 text-base font-bold leading-5 text-slate-950 sm:mt-0 sm:text-sm sm:leading-4">{item.name}</p></div>{itemDetails(item) ? <span title={itemDetails(item)} aria-label={`Item details: ${itemDetails(item)}`} className="inline-flex h-7 w-7 shrink-0 cursor-help items-center justify-center rounded-full text-slate-400 hover:bg-sky-50 hover:text-[#0066cc]"><Info className="h-3.5 w-3.5" /></span> : null}</div></div></OriginalRequestItemEditor><button type="button" onClick={() => setPriceItemId(priceOpen ? null : item.id)} aria-expanded={priceOpen} className="mt-1 inline-flex min-h-11 items-center gap-1 text-xs font-bold text-[#0066cc] sm:min-h-6 sm:text-[11px]"><Search className="h-3.5 w-3.5" />Online prices</button>{priceOpen ? <MaterialPriceCheck requestId={requestId} query={materialSearchQuery(item)} department={item.department} defaultZipCode={defaultZipCode} onClose={() => setPriceItemId(null)} /> : null}</> : <p className="text-xs font-semibold text-slate-400">Not organized yet</p>}
                    </td>
