...
export function RequestMaterialWorktable({
  ...
  function applyBatchRoute() {
    const names = [...new Set([...batchSuppliers, ...batchManual.split(",")].map((name) => name.trim()).filter(Boolean))]
    setBatchFeedback("")
  ...
  return (
    <section className="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_8px_24px_rgba(15,23,42,0.05)]" aria-labelledby="request-items-heading">
      ...
      {rawDraft ? <div data-testid="original-request-draft" className="border-b border-slate-200 bg-slate-50/70 px-3 py-3 sm:px-4">
        ...
        <div className="mt-2 flex items-center justify-between gap-2"><p className="text-[10px] text-slate-500">Edit first. AI organizes only after you choose it.</p>{!organizationInProgress ? <OrganizeMaterialListButton requestId={requestId} refresh={organizedItems.length > 0} compact /> : <MaterialOrganizationStatus status={organizationStatus} />}</div>
      </div> : null}
      ...
      {aiItems.length ? <div className="flex flex-col gap-2 border-b border-sky-100 bg-sky-50/55 px-3 py-2.5 sm:flex-row sm:items-center sm:justify-between sm:px-4">
        ...
        <div className="flex gap-1.5"><button type="button" onClick={() => setExpandedItemIds(rows.filter(({ item }) => materialReviewStatus(item) !== "ready" && materialReviewReasons(item).length > 0).map(({ item }) => item.id))} disabled={!missingItemCount} className="h-8 rounded-md border border-slate-200 bg-white px-2.5 text-[10px] font-bold text-slate-700 disabled:opacity-40">Review missing</button><button type="button" aria-pressed={groupSimilar} onClick={() => setGroupSimilar((value) => !value)} className={`inline-flex h-8 items-center gap-1 rounded-md border px-2.5 text-[10px] font-bold ${groupSimilar ? "border-sky-300 bg-sky-100 text-[#0066cc]" : "border-slate-200 bg-white text-slate-700"}`}><Layers3 className="h-3.5 w-3.5" />Group similar</button></div>
      </div> : null}
      <p className="sr-only" role="status" aria-live="polite">{copyNotice}</p>
      {selectedRouteIds.length ? <div className="sticky top-0 z-30 grid gap-2 border-b border-sky-200 bg-sky-50 p-3 shadow-sm sm:grid-cols-[auto_minmax(12rem,1fr)_minmax(10rem,1fr)_auto] sm:items-start sm:p-2"><span className="pt-2 text-xs font-bold text-sky-900 sm:text-[10px]">{selectedRouteIds.length} items selected</span><details className="group relative"><summary className="flex h-11 cursor-pointer list-none items-center justify-between rounded-md border border-sky-200 bg-white px-2 text-xs font-bold text-slate-700 sm:h-9 sm:text-[11px]">Choose suppliers · {batchSuppliers.length}<ChevronDown className="h-3.5 w-3.5 transition group-open:rotate-180" /></summary><fieldset className="absolute left-0 top-[calc(100%+.25rem)] z-50 max-h-64 w-full min-w-56 overflow-y-auto rounded-md border border-slate-200 bg-white shadow-xl" aria-label="Choose one or more suppliers">{orderedSuppliers.map((supplier) => <label key={supplier.id} className={`flex min-h-9 cursor-pointer items-center gap-2 border-b border-slate-100 px-2 text-[10px] font-semibold last:border-0 ${batchSuppliers.includes(supplier.name) ? "bg-sky-50 text-sky-900" : "text-slate-700"}`}><input type="checkbox" checked={batchSuppliers.includes(supplier.name)} onChange={() => toggleBatchSupplier(supplier.name)} className="h-4 w-4 rounded border-slate-300 accent-[#0071e3]" /><span className="truncate">{supplier.name}</span></label>)}</fieldset></details><input value={batchManual} onChange={(event) => setBatchManual(event.target.value)} placeholder="Or type supplier names" className="h-11 rounded-md border border-sky-200 px-2 text-xs sm:h-9 sm:text-[11px]" /><button type="button" onClick={applyBatchRoute} disabled={batchPending || (!batchSuppliers.length && !batchManual.trim())} className="inline-flex h-11 items-center justify-center gap-1 rounded-md bg-[#0071e3] px-3 text-xs font-bold text-white disabled:opacity-40 sm:h-9 sm:text-[11px]"><Route className="h-3.5 w-3.5" />Apply supplier route</button><details className="group sm:col-span-4"><summary className="inline-flex h-8 cursor-pointer list-none items-center gap-1.5 rounded-md border border-sky-200 bg-white px-2.5 text-[10px] font-bold text-[#0066cc]"><Sparkles className="h-3.5 w-3.5" />Generate 5 suppliers<ChevronDown className="h-3.5 w-3.5 transition group-open:rotate-180" /></summary><div className="mt-1.5 grid gap-2 rounded-md border border-sky-200 bg-white p-2"><div className="flex gap-1.5"><input aria-label="Supplier search ZIP code" inputMode="numeric" maxLength={5} value={discoveryZip} onChange={(event) => setDiscoveryZip(event.target.value.replace(/\D/g, "").slice(0, 5))} className="h-9 w-24 rounded-md border border-slate-300 px-2 text-xs font-bold" /><button type="button" onClick={discoverFiveSuppliers} disabled={discoveryPending} className="h-9 rounded-md bg-slate-950 px-3 text-[10px] font-bold text-white disabled:opacity-50">{discoveryPending ? "Finding…" : "Generate 5"}</button><p className="self-center text-[9px] text-slate-500">Matches selected items. Existing and previously requested suppliers are removed.</p></div>{discoveryResults.length ? <fieldset className="grid gap-1" aria-label="Review generated suppliers">{discoveryResults.map((supplier) => <label key={supplier.identity} className="flex min-h-10 cursor-pointer items-center gap-2 rounded-md border border-slate-200 px-2"><input type="checkbox" checked={batchSuppliers.includes(supplier.name)} onChange={() => toggleBatchSupplier(supplier.name)} className="h-4 w-4 rounded border-slate-300 accent-[#0071e3]" /><span className="min-w-0 flex-1"><span className="block truncate text-[10px] font-bold text-slate-800">{supplier.name}</span><span className="block truncate text-[9px] text-slate-500">{supplier.summary || supplier.domain}</span></span><a href={supplier.url} target="_blank" rel="noreferrer" aria-label={`Open source for ${supplier.name}`} className="inline-flex h-8 w-8 items-center justify-center text-[#0066cc]"><ExternalLink className="h-3.5 w-3.5" /></a></label>)}</fieldset> : null}{discoveryError ? <p role="alert" className="text-[10px] font-bold text-rose-700">{discoveryError}</p> : null}<p className="text-[9px] text-slate-400">Nothing is saved or contacted until you select suppliers and press Apply supplier route.</p></div></details>{batchFeedback ? <p className="text-[10px] font-bold text-rose-700 sm:col-span-4">{batchFeedback}</p> : null}</div> : null}
              ...
              <div className="divide-y divide-slate-100">{visibleRows.map(({ item, sourceItem, hasAi }, index) => {
                ...
                return <article key={item.id}>
                  <div className="grid min-h-14 grid-cols-[2.5rem_3rem_minmax(0,1fr)_2.5rem] items-center gap-2 px-2 py-2 sm:grid-cols-[2.5rem_2rem_3rem_minmax(13rem,1.4fr)_minmax(10rem,1fr)_8rem_minmax(12rem,1fr)_2.5rem] sm:px-3">
                    <label className="inline-flex h-10 w-10 cursor-pointer items-center justify-center"><input type="checkbox" aria-label={`Select ${item.name}`} checked={selectedRouteIds.includes(item.id)} onChange={(event) => setSelectedRouteIds((current) => event.target.checked ? [...new Set([...current, item.id])] : current.filter((id) => id !== item.id))} className="h-4 w-4 rounded border-slate-300 accent-[#0071e3]" /></label>
                    ...
                    <div className="min-w-0"><div className="flex items-baseline gap-2"><p className="truncate text-sm font-extrabold text-slate-950">{item.name}</p><span className="shrink-0 text-[10px] font-bold text-slate-500">{materialQuantity(item)} {materialSalesUnit(item)}</span></div><div className="mt-1 flex min-w-0 items-center gap-1 overflow-hidden"><OriginalRequestItemEditor requestId={requestId} item={item} itemKind={hasAi ? "organized" : "original"} buttonLabel="Details" /><span className={`shrink-0 rounded px-1.5 py-0.5 text-[9px] font-extrabold sm:hidden ${missing ? "bg-amber-50 text-amber-800" : "bg-emerald-50 text-emerald-700"}`}>{missing ? `${reasons.length} missing` : "Ready"}</span>{details.length ? details.map((detail) => <span key={detail} title={detail} className="max-w-32 truncate rounded bg-slate-100 px-1.5 py-0.5 text-[9px] font-semibold text-slate-600">{detail}</span>) : <span className="text-[9px] font-semibold text-slate-400">{hasAi ? "No extra details" : "Awaiting AI"}</span>}</div></div>
                    ...
                    <button type="button" onClick={() => toggleExpandedItem(item.id)} title={routeSummary || "Choose supplier route"} className="hidden min-w-0 items-center gap-1.5 rounded-md border border-slate-200 px-2 py-2 text-left text-[10px] font-bold text-slate-700 sm:flex"><Route className="h-3.5 w-3.5 shrink-0 text-[#0066cc]" /><span className="truncate">{routeSummary || "Choose supplier"}</span></button>
                  ...
                  {expanded ? <div className="border-t border-slate-100 bg-slate-50/45 p-3">
                    <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(16rem,.8fr)_minmax(16rem,.8fr)]">
                      ...
                      <div><p className="mb-2 text-[9px] font-extrabold uppercase tracking-[.1em] text-slate-500">Supplier group</p><RequestSupplierRouteEditor key={`${item.id}-${supplierRouteVersion(item.metadata)}`} requestId={requestId} itemId={item.id} itemIds={groupIds} metadata={item.metadata} suppliers={orderedSuppliers} /></div>
                    </div>
                  </div> : null}
                </article>
              })}</div>
