    ...
    try {
      const next = await Promise.all(items.map(async ({ item, quantity }): Promise<ItemReview> => {
        const response = await fetch("/api/admin/catalog/exa-search", {
          ...
          body: JSON.stringify({ query: item, department: "Construction materials", zipCode }),
        });
        const payload = await response.json().catch(() => null) as { buyNow?: PriceResult[]; checkedAt?: string; error?: string } | null;
