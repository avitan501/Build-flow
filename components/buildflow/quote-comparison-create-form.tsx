...
export function QuoteComparisonCreateForm({
  ...
  function submit() {
    ...
    startTransition(async () => {
      const result = await createQuoteComparisonAction({ title, department: "", jobAddress: "", projectId: null });
      if (!result.ok) {
    ...
    <section className="border border-slate-200 bg-white shadow-sm" aria-labelledby="new-comparison-title">
      <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4 sm:px-6">
        <div>
          ...
          <h2 id="new-comparison-title" className="mt-1 text-xl font-bold text-slate-950">Name this comparison</h2>
        </div>
