...
export function SupplierNetworkWorkspace({
  ...
  async function shareSupplier(row: SupplierNetworkRow) {
    const details = [
      row.name,
      row.departments && row.departments !== "Departments not set"
        ? row.departments
        : "",
      row.phone,
      row.link,
  ...
  return (
    <div className="overflow-hidden rounded-md border border-slate-200 bg-white">
      ...
      <div className="overflow-x-auto">
        <table className="w-full table-fixed border-collapse text-left sm:min-w-[720px]">
          <thead className="sticky top-0 z-10 hidden bg-[#071225] text-[9px] uppercase tracking-[.08em] text-white sm:table-header-group">
            <tr>
              ...
              return (
                ...
                >
                  <td className="p-0" colSpan={6}>
                    <div className="grid min-h-14 w-full grid-cols-[minmax(0,1fr)_6rem_3rem] items-center text-left sm:min-h-11 sm:grid-cols-[10rem_12rem_8rem_minmax(12rem,1fr)_5rem_4rem]">
                      ...
                      <span className="hidden line-clamp-2 px-2 text-[10px] leading-3.5 text-slate-600 sm:block">
                        {row.departments}
                      </span>
                      <span className="hidden flex-wrap gap-0.5 px-2 sm:flex">
                        <button
                          type="button"
                          onClick={() =>
                            setExpanded(
                      ...
                      <span className="hidden truncate px-2 text-[10px] text-slate-600 sm:block">
                        {row.ask}
                      </span>
                      <span className="truncate px-2 text-right text-[9px] font-semibold text-slate-500 sm:text-left">
                    ...
                    {optionsOpen ? (
                      ...
                      <div className="border-t border-sky-100 bg-sky-50/50 px-2 py-2">
                        {edit.hidden ? (
                          ...
                          <>
                          <div className="mb-2 rounded-md border border-sky-100 bg-white px-2.5 py-2 sm:hidden">
                            <p className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Sells / departments</p>
                            <p className="mt-1 text-xs font-semibold text-slate-800">{row.departments}</p>
                            <div className="mt-2 flex flex-wrap gap-1">
                              {SUPPLIER_NETWORK_CHANNELS.map((channel) => (
                                <button
                                  ...
                                  className={`min-h-8 rounded-md border px-2 text-[10px] font-bold ${selectedChannels.includes(channel) ? CHANNEL_COLORS[channel] : "border-slate-200 bg-white text-slate-500"}`}
                          ...
                          <div className="grid gap-2 lg:grid-cols-[10rem_11rem_minmax(14rem,1fr)_auto] lg:items-end">
                            ...
                            <div className="flex items-center gap-1 lg:justify-end">
                              ...
                              <button
                                type="button"
                                onClick={() => void shareSupplier(row)}
                                className="inline-flex h-8 items-center gap-1 rounded-md border border-sky-200 bg-white px-2 text-[10px] font-bold text-[#0066cc]"
                              >
