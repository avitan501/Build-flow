...
import { supplierQuoteLineTotal } from "@/lib/supplier-quote-pricing"
...
export function SupplierQuoteWorkspace({ quote, initialItems, documentUrl, departments, suppliers, comparisonItems = [] }: {
  ...
  const subtotal = useMemo(() => items.filter((item) => item.selected).reduce((total, item) => total + (supplierQuoteLineTotal(item) ?? 0), 0), [items])
        ...
        <section className="mt-4 border border-slate-200 bg-white shadow-sm" aria-labelledby="quote-items-heading">
          ...
          <div className="divide-y divide-slate-200">
            {items.map((item, index) => <article key={item.id} className={`grid grid-cols-2 gap-3 p-3 sm:grid-cols-4 lg:grid-cols-[2.5rem_minmax(13rem,2fr)_minmax(8rem,1fr)_6rem_6rem_7rem_7rem_2.5rem] lg:items-end lg:p-4 ${item.selected ? "bg-white" : "bg-slate-50 opacity-65"}`}>
              ...
              <label className="grid gap-1 text-[11px] font-bold uppercase tracking-[0.06em] text-slate-500">Qty<input type="number" min="0.001" step="0.001" value={item.quantity} onChange={(event) => updateItem(item.id, { quantity: Number(event.target.value), lineTotal: null })} className="min-h-10 rounded-lg border border-slate-300 px-2 text-sm text-slate-950" /></label>
              ...
              <label className="grid gap-1 text-[11px] font-bold uppercase tracking-[0.06em] text-slate-500">Unit price<input type="number" min="0" step="0.01" value={item.unitPrice ?? ""} onChange={(event) => updateItem(item.id, { unitPrice: event.target.value === "" ? null : Number(event.target.value), lineTotal: null })} className="min-h-10 rounded-lg border border-slate-300 px-2 text-sm text-slate-950" /></label>
              <div><p className="text-[11px] font-bold uppercase tracking-[0.06em] text-slate-500">Line total</p><p className="flex min-h-10 items-center text-sm font-bold tabular-nums">{money(supplierQuoteLineTotal(item) ?? 0)}</p></div>
