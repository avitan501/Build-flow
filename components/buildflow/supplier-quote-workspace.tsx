    ...
    <main className="min-h-screen bg-[#f5f5f7] px-3 pb-28 pt-4 text-slate-950 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[92rem]">
        <header className="flex flex-col gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0"><Link href="/admin/supplier-quotes" className="inline-flex items-center gap-2 text-sm font-bold text-[#0071e3]"><ArrowLeft className="h-4 w-4" /> Quote storage</Link><p className="mt-4 text-[11px] font-bold uppercase tracking-[0.16em] text-slate-500">{quote.department}</p><h1 className="mt-1 truncate text-2xl font-bold tracking-tight sm:text-3xl">{quote.supplier_name}</h1><p className="mt-1 truncate text-sm text-slate-600">{quote.quote_number ? `Quote ${quote.quote_number} · ` : ""}{quote.file_name}</p></div>
        ...
        <section className="mt-4 grid gap-3 border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-2 lg:grid-cols-4" aria-label="Quote details">
        ...
        <section className="mt-4 border border-slate-200 bg-white shadow-sm" aria-labelledby="quote-items-heading">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-4 py-3"><div><h2 id="quote-items-heading" className="font-bold">Extracted items</h2><p className="mt-0.5 text-xs text-slate-500">Review every selected row before routing</p></div><button type="button" onClick={addLine} disabled={pending} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-slate-300 px-3 text-sm font-bold"><Plus className="h-4 w-4" /> Add line</button></div>
          <div className="divide-y divide-slate-200">
            {items.map((item, index) => <article key={item.id} className={`grid gap-3 p-4 lg:grid-cols-[2.5rem_minmax(13rem,2fr)_minmax(8rem,1fr)_6rem_6rem_7rem_7rem_2.5rem] lg:items-end ${item.selected ? "bg-white" : "bg-slate-50 opacity-65"}`}>
              <label className="flex items-center gap-2 text-xs font-bold text-slate-500 lg:grid lg:justify-items-center"><span className="lg:hidden">Use item</span><input type="checkbox" checked={item.selected} onChange={(event) => updateItem(item.id, { selected: event.target.checked })} className="h-5 w-5 accent-[#0071e3]" aria-label={`Use item ${index + 1}`} /><span className="tabular-nums">{index + 1}</span></label>
              <label className="grid gap-1 text-[11px] font-bold uppercase tracking-[0.06em] text-slate-500">Material<input value={item.description} onChange={(event) => updateItem(item.id, { description: event.target.value })} className="min-h-10 rounded-lg border border-slate-300 px-3 text-sm font-semibold normal-case tracking-normal text-slate-950" /></label>
              <label className="grid gap-1 text-[11px] font-bold uppercase tracking-[0.06em] text-slate-500">Specification<input value={item.specification} onChange={(event) => updateItem(item.id, { specification: event.target.value })} className="min-h-10 rounded-lg border border-slate-300 px-3 text-sm normal-case tracking-normal text-slate-950" /></label>
        ...
        <section className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_22rem]">
          <div className="grid gap-2 sm:grid-cols-3">
            ...
            <button type="button" onClick={() => route("comparison")} disabled={pending || !selectedIds.length} className="flex min-h-16 items-center gap-3 rounded-lg border border-slate-300 bg-white px-4 text-left text-sm font-bold shadow-sm disabled:opacity-40"><Columns3 className="h-5 w-5 text-[#0071e3]" /><span>Compare suppliers<span className="mt-0.5 block text-xs font-medium text-slate-500">Open comparison room</span></span></button>
