...
export function AvantiaBuildClientShell({ children }: AvantiaBuildClientShellProps) {
  ...
  return (
    <div className={isShopLanding || isHome || isHomepagePreview || isCinematicStory ? `min-h-screen ${isCinematicStory ? "bg-[#071126]" : "bg-white"}` : "min-h-screen bg-[radial-gradient(circle_at_top,#d9e7fb_0%,#f4f8fd_34%,#dbe8f9_100%)] lg:px-3 lg:py-3"}>
      <div
        className={`mx-auto flex min-h-screen w-full flex-col ${isShopLanding ? "max-w-none bg-black" : isHome || isHomepagePreview ? "max-w-none bg-white" : isCinematicStory ? "max-w-none bg-[#071126]" : "bg-[linear-gradient(180deg,#f9fcff_0%,#f2f7ff_42%,#ffffff_100%)] lg:min-h-[calc(100vh-3rem)] lg:rounded-[34px] lg:border lg:border-white/70 lg:shadow-[0_28px_80px_rgba(15,23,42,0.16)]"} ${
          isShopLanding || isHome || isHomepagePreview || isCinematicStory
            ? "max-w-none"
            : isOwnerWorkspace
