...
function messageCanStartMaterialRequest(message: AuraCommunicationRow) {
  if (message.channel !== "sms" || message.direction !== "incoming") return false
...
export function UnifiedCommunicationInbox({ communications, contacts, customers, leads = [], suppliers = [], materialRequests = [], connections, initialChannelFilter = "all", initialQuery = "" }: {
  ...
  return <section className="h-[calc(100dvh-7rem)] min-h-[34rem] max-h-[56rem] overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm md:h-[calc(100dvh-8rem)] md:min-h-[38rem] md:max-h-none" aria-label="Unified communications inbox">
