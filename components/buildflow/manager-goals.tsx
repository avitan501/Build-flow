...
export function CustomManagerGoals({ goals }: { goals: ManagerGoalRecord[] }) {
  ...
  return <section className="mt-3 overflow-hidden rounded-md border border-slate-200 bg-white" aria-labelledby="carlos-custom-tasks">
    <header className="flex items-end justify-between gap-3 border-b border-slate-200 bg-slate-50 px-3 py-2.5"><div><h3 id="carlos-custom-tasks" className="text-xs font-bold uppercase tracking-[.12em] text-slate-700">Focus & tasks</h3><p className="mt-0.5 text-[11px] text-slate-500">Focus stays at the top. Open a task for notes and controls.</p></div><span className="shrink-0 text-[10px] font-bold text-slate-500">{activeGoals.length} active</span></header>
    {focusGoals.length ? <div><p className="border-b border-amber-100 bg-amber-50 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wide text-amber-800">Focus</p>{focusGoals.map(goalRow)}</div> : null}
