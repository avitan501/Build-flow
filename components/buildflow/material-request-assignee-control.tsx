...
export function MaterialRequestAssigneeControl({ requestId, assignee, compact = false, hideLabel = false }: { requestId: string; assignee: string; compact?: boolean; hideLabel?: boolean }) {
  ...
  return <label className="grid gap-1 text-[10px] font-bold uppercase tracking-[.08em] text-slate-500">
    <span className={hideLabel ? "sr-only" : undefined}>Assigned to</span>
    <span className="relative block">
      <select aria-label="Assigned to" title={`Assigned to ${selected === "david" ? "David" : "Carlos"}`} value={selected} onChange={(event) => update(event.target.value as MaterialRequestAssignee)} disabled={pending} className={`${compact ? "h-8 pr-8 text-xs" : "h-9 pr-9 text-sm"} w-full rounded-md border border-slate-300 bg-white pl-2.5 font-semibold normal-case tracking-normal text-slate-900 disabled:opacity-60`}>
    ...
    {message ? <span role="status" className={`${hideLabel && message.startsWith("Assigned") ? "sr-only" : "normal-case tracking-normal"} ${message.startsWith("Assigned") ? "text-emerald-700" : "text-rose-700"}`}>{message}</span> : null}
  </label>
}
