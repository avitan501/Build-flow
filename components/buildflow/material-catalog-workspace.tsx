        ...
        <section className={`mt-2 grid min-h-[30rem] overflow-hidden rounded-md border border-slate-200 bg-white ${selectedItem ? "lg:grid-cols-[minmax(0,1fr)_22rem]" : "grid-cols-1"}`} aria-label={`${selectedCategory} catalog`}>
          ...
          {selectedItem ? <aside className="border-t border-slate-200 bg-slate-50/70 lg:border-l lg:border-t-0" aria-label={`${selectedItem.name} details`}>
            <div className="sticky top-2 p-3">
              <div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[0.12em] text-sky-700">Details</p><h2 className="truncate text-lg font-bold">{selectedItem.name}</h2><p className="truncate text-sm text-slate-500">{[selectedItem.brand, selectedItem.measurement, selectedItem.thickness].filter(Boolean).join(" · ") || selectedItem.item_code}</p>{selectedItem.status === "active" ? <span className="mt-1 inline-flex rounded-full bg-sky-50 px-2.5 py-1 text-xs font-bold text-sky-800 ring-1 ring-sky-200">Published</span> : null}</div><button type="button" onClick={() => setSelectedItemId(null)} className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-500" aria-label="Close details"><X className="h-4 w-4" /></button></div>
