...
function catalogItemSizeLabel(item: MaterialCatalogItem) {
  return [item.measurement, item.thickness].filter(Boolean).join(" · ") || "Needs completion"
}

function catalogItemFamilyKey(item: MaterialCatalogItem) {
  const family = item.name
    .toLowerCase()
    .replace(/\b\d+(?:[- ]\d+\/\d+|\/\d+)?(?:\.\d+)?\s*[x×]\s*\d+(?:[- ]\d+\/\d+|\/\d+)?(?:\.\d+)?(?:\s*[x×]\s*\d+(?:[- ]\d+\/\d+|\/\d+)?(?:\.\d+)?)?\s*(?:in(?:ch(?:es)?)?|ft|feet|foot)?\.?/g, " ")
    .replace(/\b\d+(?:[- ]\d+\/\d+|\/\d+)?(?:\.\d+)?\s*(?:in(?:ch(?:es)?)?|ft|feet|foot|lb|lbs|oz|gal|gallon|sq\.?\s*ft|lin\.?\s*ft)\.?/g, " ")
    .replace(/\b#\d+\b/g, " ")
    .replace(/[^a-z0-9]+/g, " ")
    .replace(/\s+/g, " ")
    .trim()
...
export function MaterialCatalogWorkspace({
  ...
  const selectedGroup = selectedItem ? catalogGroups.find((group) => group.some((item) => item.id === selectedItem.id)) ?? [selectedItem] : []
    ...
    <main className="min-h-screen bg-[#f7f8fa] px-2 pb-20 pt-2 text-slate-950 sm:px-3 lg:px-4">
      <div className="mx-auto max-w-[100rem]">
        ...
        <section className={`mt-2 grid min-h-[30rem] overflow-hidden rounded-md border border-slate-200 bg-white ${selectedItem ? "lg:grid-cols-[minmax(0,1fr)_22rem]" : "grid-cols-1"}`} aria-label={`${selectedCategory} catalog`}>
          <div className="min-w-0 divide-y divide-slate-100">{catalogGroups.map((group) => {
            const item = group.find((entry) => entry.id === selectedItemId) ?? group[0]
            const cheapest = bestSupplierPrices(item)
            const issues = group.flatMap((entry) => catalogItemIssues(entry, initialPrices))
            return <button type="button" key={catalogItemFamilyKey(item)} onClick={() => setSelectedItemId(item.id)} aria-pressed={selectedGroup === group} className={`grid min-h-24 w-full gap-3 px-3 py-3 text-left transition hover:bg-slate-50 sm:grid-cols-[minmax(22rem,1.35fr)_minmax(15rem,1fr)_auto] sm:items-center ${selectedGroup === group ? "bg-sky-50/70" : ""}`}><span className="flex min-w-0 items-center gap-3">{item.image_url ? <span className="relative h-20 w-20 shrink-0 overflow-hidden rounded-lg border border-slate-200 bg-white"><Image src={item.image_url} alt={`${item.name} manufacturer product photo`} fill sizes="80px" className="object-contain p-1" /></span> : <span className="inline-flex h-20 w-20 shrink-0 flex-col items-center justify-center rounded-lg border border-dashed border-slate-300 bg-slate-50 text-slate-400"><ImageIcon className="h-5 w-5" /><span className="mt-1 text-[10px] font-bold">Add photo</span></span>}<span className="min-w-0 flex-1"><span className="flex min-w-0 items-center gap-2"><span className="min-w-0 truncate text-base font-bold text-slate-950">{item.name}</span>{group.length > 1 ? <span className="shrink-0 rounded-full bg-violet-50 px-2 py-0.5 text-xs font-bold text-violet-700 ring-1 ring-violet-200">{group.length} variants</span> : null}</span><span className="mt-0.5 block truncate text-sm text-slate-600">{catalogItemSubtitle(item)}</span><span className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-xs"><span className="truncate"><span className="font-bold text-slate-500">Brand:</span> <span className={item.brand ? "font-semibold text-slate-800" : "font-bold text-amber-700"}>{item.brand || "Needs completion"}</span></span><span className="truncate"><span className="font-bold text-slate-500">Model:</span> <span className={item.manufacturer_model_number ? "font-semibold text-slate-800" : "font-bold text-amber-700"}>{item.manufacturer_model_number || "Needs completion"}</span></span><span className="truncate"><span className="font-bold text-slate-500">Size:</span> <span className={catalogItemSizeLabel(item) === "Needs completion" ? "font-bold text-amber-700" : "font-semibold text-slate-800"}>{group.length > 1 ? `${group.length} available sizes/types` : catalogItemSizeLabel(item)}</span></span><span className="truncate"><span className="font-bold text-slate-500">Pack:</span> <span className="font-semibold text-slate-800">{catalogItemPackLabel(item)} · {item.unit}</span></span></span></span></span><span className="flex min-w-0 gap-2">{cheapest.length ? cheapest.map((price) => <span key={price.supplier_id} className="min-w-0 flex-1 rounded-md border border-slate-200 bg-white px-2 py-1.5"><span className="block truncate text-xs text-slate-500">{price.supplier_name_snapshot}</span><span className="block text-sm font-bold tabular-nums text-emerald-800">${normalizedComparisonPrice(item, price)!.toFixed(2)}</span></span>) : <span className="text-sm text-slate-500">No verified prices</span>}</span><span className={`justify-self-start rounded-full px-2.5 py-1 text-xs font-bold sm:justify-self-end ${issues.length ? "bg-amber-50 text-amber-800 ring-1 ring-amber-200" : "bg-emerald-50 text-emerald-800 ring-1 ring-emerald-200"}`}>{issues.length ? "Review" : "Ready"}</span></button>
          ...
          {selectedItem ? <aside className="border-t border-slate-200 bg-slate-50/70 lg:border-l lg:border-t-0" aria-label={`${selectedItem.name} details`}>
            <div className="sticky top-2 p-3">
              <div className="flex items-start justify-between gap-2"><div className="flex min-w-0 gap-3">{selectedItem.image_url ? <span className="relative h-20 w-20 shrink-0 overflow-hidden rounded-lg border border-slate-200 bg-white"><Image src={selectedItem.image_url} alt={`${selectedItem.name} manufacturer product photo`} fill sizes="80px" className="object-contain p-1" /></span> : <span className="inline-flex h-20 w-20 shrink-0 flex-col items-center justify-center rounded-lg border border-dashed border-slate-300 bg-white text-slate-400"><ImageIcon className="h-6 w-6" /><span className="mt-1 text-[10px] font-bold">Add photo</span></span>}<div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[0.12em] text-sky-700">Details</p><h2 className="truncate text-lg font-bold">{selectedItem.name}</h2><p className="line-clamp-2 text-sm text-slate-600">{catalogItemSubtitle(selectedItem)}</p><dl className="mt-2 grid gap-1 text-xs"><div><dt className="inline font-bold text-slate-500">Brand: </dt><dd className="inline font-semibold">{selectedItem.brand || "Needs completion"}</dd></div><div><dt className="inline font-bold text-slate-500">Model: </dt><dd className="inline font-semibold">{selectedItem.manufacturer_model_number || "Needs completion"}</dd></div><div><dt className="inline font-bold text-slate-500">Size: </dt><dd className="inline font-semibold">{catalogItemSizeLabel(selectedItem)}</dd></div><div><dt className="inline font-bold text-slate-500">Pack: </dt><dd className="inline font-semibold">{catalogItemPackLabel(selectedItem)} · {selectedItem.unit}</dd></div></dl>{selectedItem.status === "active" ? <span className="mt-1 inline-flex rounded-full bg-sky-50 px-2.5 py-1 text-xs font-bold text-sky-800 ring-1 ring-sky-200">Published</span> : null}</div></div><button type="button" onClick={() => setSelectedItemId(null)} className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-500" aria-label="Close details"><X className="h-4 w-4" /></button></div>
              {selectedGroup.length > 1 ? <div className="mt-3"><p className="text-xs font-bold uppercase tracking-[0.1em] text-slate-500">Variants</p><div className="mt-1.5 flex flex-wrap gap-1.5">{selectedGroup.map((variant) => <button key={variant.id} type="button" onClick={() => setSelectedItemId(variant.id)} className={`rounded-md border px-2.5 py-1.5 text-xs font-bold ${variant.id === selectedItem.id ? "border-violet-600 bg-violet-600 text-white" : "border-slate-200 bg-white text-slate-700"}`}>{catalogItemSizeLabel(variant) === "Needs completion" ? variant.name : catalogItemSizeLabel(variant)}</button>)}</div></div> : null}
              <div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => setPriceCheckItem(selectedItem)} className="inline-flex h-10 items-center justify-center gap-1.5 rounded-md bg-sky-700 px-3 text-sm font-bold text-white"><MapPin className="h-4 w-4" />Online Prices</button><button type="button" onClick={() => setShowAllSupplierColumns(true)} className="h-10 rounded-md border border-slate-200 bg-white px-3 text-sm font-bold text-slate-700">All Prices</button><button type="button" onClick={() => setEditor(itemEditor(selectedItem))} className="h-10 rounded-md border border-slate-200 bg-white px-3 text-sm font-bold text-slate-700">Complete</button><Link href="/admin/supplier-approvals" className="inline-flex h-10 items-center justify-center rounded-md border border-slate-200 bg-white px-3 text-sm font-bold text-slate-700">Send for Approval</Link><button type="button" onClick={() => deleteItem(selectedItem)} className="col-span-2 inline-flex h-10 items-center justify-center gap-1.5 rounded-md border border-rose-200 bg-white px-3 text-sm font-bold text-rose-700 hover:bg-rose-50"><Archive className="h-4 w-4" />Archive product</button></div>
