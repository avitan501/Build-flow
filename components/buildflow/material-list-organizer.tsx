...
export function MaterialListOrganizer() {
  ...
  const organizedText = useMemo(() => rows.filter((row) => row.item.trim()).map((row, index) => `${index + 1}. ${row.quantity ? `${row.quantity} ` : ""}${row.unit ? `${row.unit} · ` : ""}${row.item.trim()}${row.notes.trim() ? ` — ${row.notes.trim()}` : ""}`).join("\n"), [rows])
  ...
  function downloadCsv() {
    const csv = ["Quantity,Unit,Material / Specification,Notes", ...rows.map((row) => [row.quantity, row.unit, row.item, row.notes].map(csvCell).join(","))].join("\n")
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[minmax(20rem,.72fr)_minmax(0,1.28fr)]">
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div className="flex items-start gap-3"><span className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-slate-950 text-white"><ClipboardList className="h-5 w-5" /></span><div><h2 className="text-lg font-bold text-slate-950">Messy material list</h2><p className="mt-1 text-sm text-slate-500">Paste English or Spanish notes from a text, email, estimate, or field list.</p></div></div>
      ...
      <section className="min-w-0 rounded-lg border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#0066cc]">Organized output</p><h2 className="mt-1 text-lg font-bold text-slate-950">{rows.length} material {rows.length === 1 ? "line" : "lines"}</h2></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setRows((current) => [...current, { id: newId(), quantity: "", unit: "", item: "", notes: "" }])} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-slate-200 px-3 text-xs font-semibold text-slate-700"><Plus className="h-4 w-4" />Add line</button><button type="button" disabled={!organizedText} onClick={copy} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-slate-200 px-3 text-xs font-semibold text-slate-700 disabled:opacity-40">{copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}{copied ? "Copied" : "Copy"}</button><button type="button" disabled={!rows.length} onClick={downloadCsv} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-slate-200 px-3 text-xs font-semibold text-slate-700 disabled:opacity-40"><Download className="h-4 w-4" />CSV</button></div></div>
        <p className="mt-3 text-xs leading-5 text-slate-500">This tool only collects and organizes what you entered. It does not choose a product, solve a request, confirm price or availability, place an order, or send anything. Review every highlighted interpretation.</p>
        ...
        {rows.length ? <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[48rem] border-separate border-spacing-y-2 text-left"><thead><tr className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-500"><th className="w-24 px-2">Quantity</th><th className="w-28 px-2">Unit</th><th className="px-2">Material / specification</th><th className="px-2">Notes / confirmation</th><th className="w-12"><span className="sr-only">Remove</span></th></tr></thead><tbody>{rows.map((row) => <tr key={row.id} className={row.needsReview ? "bg-amber-50" : undefined}><td className="px-1"><input aria-label="Material quantity" inputMode="decimal" value={row.quantity} onChange={(event) => update(row.id, "quantity", event.target.value)} className={fieldClass} /></td><td className="px-1"><input aria-label="Material unit" value={row.unit} onChange={(event) => update(row.id, "unit", event.target.value)} className={fieldClass} /></td><td className="px-1"><input aria-label="Material description" value={row.item} onChange={(event) => update(row.id, "item", event.target.value)} className={fieldClass} /></td><td className="px-1"><input aria-label="Material notes" value={row.notes} onChange={(event) => update(row.id, "notes", event.target.value)} className={fieldClass} /></td><td><button type="button" onClick={() => setRows((current) => current.filter((entry) => entry.id !== row.id))} className="inline-flex h-11 w-11 items-center justify-center rounded-md border border-slate-200 text-slate-500 hover:border-rose-200 hover:text-rose-700" aria-label="Remove material line"><Trash2 className="h-4 w-4" /></button></td></tr>)}</tbody></table></div> : <div className="mt-4 flex min-h-80 items-center justify-center rounded-lg border border-dashed border-slate-300 bg-slate-50 px-6 text-center"><div><ClipboardList className="mx-auto h-8 w-8 text-slate-400" /><p className="mt-3 text-sm font-semibold text-slate-700">Paste a list and select Organize list.</p><p className="mt-1 text-xs text-slate-500">You can edit every line before copying or downloading it.</p></div></div>}
      </section>
    </div>
  )
}
