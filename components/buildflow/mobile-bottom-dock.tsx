      ...
      <span
        className={`flex items-center justify-center transition-all duration-150 ease-out group-active:scale-[1.06] ${
          accent
            ...
            : active
              ? "h-9.5 w-9.5 rounded-[18px] border border-[#efca75]/25 bg-[linear-gradient(180deg,rgba(242,202,112,0.16),rgba(242,202,112,0.06))] text-[#f4d184] shadow-[0_10px_20px_rgba(220,164,69,0.12)]"
              : "h-9.5 w-9.5 rounded-[18px] border border-transparent bg-transparent text-slate-300"
        }`}
      >
        {children}
      </span>
      <span className={`mt-0.5 text-[9px] font-medium leading-none ${accent ? "-mt-0.5 text-white" : active ? "text-[#f4d184]" : "text-slate-300"}`}>
        {label}
      </span>
    </Link>
  );
    ...
    <>
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-[calc(env(safe-area-inset-bottom)+0.35rem)] z-50 px-3 sm:hidden">
        <nav
          ...
          className="pointer-events-auto mx-auto flex max-w-[21.75rem] items-end justify-between gap-0.5 rounded-[22px] border border-white/10 bg-[linear-gradient(180deg,rgba(8,21,43,0.96),rgba(10,24,46,0.94))] px-2.5 pb-1.5 pt-2.5 shadow-[0_18px_38px_rgba(2,8,23,0.38)] backdrop-blur-xl"
        >
