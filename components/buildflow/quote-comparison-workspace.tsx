...
import { ProductQuoteCard } from "@/components/buildflow/product-quote-card";
...
export function QuoteComparisonWorkspace({
    ...
    return {
      ...
      quote_comparison_prices: items.map((item) => {
        ...
        return {
          ...
          quote_product_match_confirmations: bid.quote_comparison_prices?.find(price=>price.item_id===item.id)?.quote_product_match_confirmations,
  ...
  function confirmMatch(key: string, bidId: string, itemId: string) {
    void key; void bidId;
    setActiveStep(1);
    setMessage("Expand the supplier offer and review its source and selling unit.");
    requestAnimationFrame(()=>{const details=document.getElementsByName(`draft-product-${itemId}`)[0]?.closest("details");if(details){details.open=true;details.scrollIntoView({block:"center"});}});
  }
  ...
  return (
    <main className={`min-h-screen pb-12 text-slate-950 ${activeStep === 0 ? "bg-white" : "bg-[#f5f5f7]"}`}>
      ...
      <div className={`mx-auto px-4 py-5 sm:px-8 sm:py-8 ${activeStep === 0 ? "max-w-5xl" : "max-w-[96rem] lg:px-10"}`}>
        ...
        {activeStep === 0 && receivedSupplierQuotes.length ? <details className="mb-3"><summary className="min-h-11 cursor-pointer py-3 text-xs font-semibold">Original quotes · {receivedSupplierQuotes.length}</summary><ReceivedSupplierQuoteTable quotes={receivedSupplierQuotes} /></details> : null}
              ...
              <table className="w-full min-w-[860px] border-collapse text-left">
                ...
                <tbody>
                  {items.map((item) => <tr key={item.id} className="border-b border-slate-100"><th className="sticky left-0 z-10 bg-white px-5 py-3"><p className="text-sm font-bold">{item.description}</p><p className="mt-1 text-xs font-medium text-slate-500">{item.quantity.toLocaleString()} {item.unit}{item.specification ? ` · ${item.specification}` : ""}</p></th><td className="border-l border-amber-100 bg-amber-50/50 px-4 py-3"><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-bold text-amber-700">$</span><input type="number" min="0" step="0.01" value={clientTargetDrafts[item.id] ?? ""} disabled={locked} onChange={(event) => setClientTargetDrafts((current) => ({ ...current, [item.id]: event.target.value }))} placeholder="0.00" aria-label={`Client target unit price for ${item.description}`} className="min-h-10 w-full rounded-lg border border-amber-300 bg-white pl-7 pr-2 text-right text-sm font-bold tabular-nums disabled:bg-slate-100" /></div></td>{bids.map((bid) => {
                    ...
                    const matchStatus = productPreview.rows.find(row=>row.item.id===item.id)?.offers.find(offer=>offer.bid.id===bid.id)?.matchStatus ?? "review";
