      ...
      <div className="mx-auto max-w-[96rem] px-2.5 py-3 sm:px-8 sm:py-5 lg:px-10">
        ...
        {activeStep === 0 ? <section aria-labelledby="product-comparison-heading" data-testid="product-comparison-overview" className="space-y-4">
          <header className="rounded-xl border border-slate-200 bg-white p-4 sm:p-6">
            <h2 id="product-comparison-heading" className="text-lg font-bold sm:text-xl">Compare products</h2>
            <p className="mt-1 text-xs text-slate-600">{productPreview.comparableCount}/{items.length} with confirmed prices</p>
          ...
          <div className="grid gap-3 xl:grid-cols-2">{productPreview.rows.map((row) => <ProductQuoteCard key={row.item.id} row={row} onSelect={(bidId) => setProductSelections((current) => ({ ...current, [row.item.id]: bidId }))} onClear={() => setProductSelections((current) => ({ ...current, [row.item.id]: "" }))} />)}</div>
