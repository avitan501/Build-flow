...
export function QuoteComparisonWorkspace({
  ...
  function awardBid(bidId: string, supplierName: string) {
    ...
    run(() => awardQuoteComparisonBidAction({ comparisonId: comparison.id, bidId }), `${supplierName} selected.`);
  }
      ...
      <div className="mx-auto max-w-[96rem] px-4 py-5 sm:px-8 lg:px-10">
        ...
        <section className="border border-slate-200 bg-white shadow-sm" aria-labelledby="materials-heading">
          <div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            ...
            {!locked ? <button type="button" onClick={() => setShowItemForm((value) => !value)} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-3 text-sm font-bold"><PackagePlus className="h-4 w-4" /> Add material</button> : null}
          </div>
          {showItemForm && !locked ? (
        ...
        <section className="mt-5 border border-slate-200 bg-white shadow-sm" aria-labelledby="quotes-heading">
          <div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            ...
            {!locked ? <div className="flex flex-wrap gap-2">
              <button type="button" onClick={() => setShowSupplierForm((value) => !value)} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 text-sm font-bold"><Store className="h-4 w-4" /> Add supplier</button>
          ...
          {showSupplierForm && !locked ? <div className="flex flex-col gap-3 border-b border-slate-200 bg-slate-50 p-4 sm:flex-row sm:items-end sm:px-6"><label className="grid flex-1 gap-1 text-xs font-bold text-slate-600">Supplier<select value={supplierId} onChange={(event) => setSupplierId(event.target.value)} className="min-h-11 rounded-lg border border-slate-300 bg-white px-3 text-sm"><option value="">Choose from Supplier Directory</option>{availableSuppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name} · {trustLabel(supplier.trustLevel || "not-reviewed")}</option>)}</select></label><button type="button" onClick={addSupplier} disabled={pending || !supplierId} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-[#0071e3] px-4 text-sm font-bold text-white disabled:opacity-40"><Plus className="h-4 w-4" /> Add supplier</button></div> : null}

          {bids.length > 0 && items.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[860px] border-collapse text-left">
                <thead><tr className="border-b border-slate-200 bg-slate-50"><th className="sticky left-0 z-10 min-w-64 bg-slate-50 px-5 py-3 text-xs font-bold uppercase tracking-[0.08em] text-slate-500">Material</th>{bids.map((bid) => <th key={bid.id} className="min-w-56 border-l border-slate-200 px-4 py-3 align-top"><div className="flex items-start justify-between gap-2"><div><p className="text-sm font-bold normal-case tracking-normal text-slate-950">{bid.supplier_name_snapshot}</p><p className="mt-1 text-[10px] font-bold uppercase tracking-[0.08em] text-slate-500">{trustLabel(bid.trust_level_snapshot)}</p></div>{!locked ? <button type="button" onClick={() => window.confirm(`Remove ${bid.supplier_name_snapshot} from this comparison?`) && run(() => removeQuoteComparisonSupplierAction({ comparisonId: comparison.id, bidId: bid.id }), "Supplier removed.")} className="text-slate-400 hover:text-rose-600" aria-label={`Remove ${bid.supplier_name_snapshot}`}><X className="h-4 w-4" /></button> : null}</div></th>)}</tr></thead>
                <tbody>
        ...
        <section className="mt-5" aria-labelledby="analysis-heading">
          ...
          {analyses.length > 0 ? <div className="mt-4 grid gap-4 xl:grid-cols-3">{analyses.map((analysis) => { const bid = liveBids.find((entry) => entry.id === analysis.bidId)!; const selected = comparison.awarded_bid_id === bid.id; return <article key={bid.id} className={`relative border bg-white p-5 shadow-sm ${analysis.isRecommended ? "border-[#0071e3] ring-1 ring-[#0071e3]" : selected ? "border-emerald-300" : "border-slate-200"}`}>
