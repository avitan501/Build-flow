  ...
  return (
    <section id="supplier-quote-upload" className="w-full scroll-mt-6 border border-slate-200 bg-white shadow-sm sm:max-w-2xl" aria-labelledby="supplier-quote-upload-title">
      ...
      <form ref={formRef} action={submit} className="grid gap-4 p-4 sm:grid-cols-2 sm:p-5">
        <label className="grid gap-1.5 text-sm font-semibold text-slate-800 sm:col-span-2">Quote connection
          <select name="linkMode" value={linkMode} onChange={(event) => { setLinkMode(event.target.value as "unlinked" | "request"); setClientSelection(""); setRequestId(""); setError("") }} className="min-h-11 rounded-lg border border-slate-300 bg-white px-3 text-sm"><option value="unlinked">Don&apos;t link it to anyone</option><option value="request">Attach to a client request</option></select>
        </label>
        {linkMode === "request" ? <div className="grid gap-4 border border-sky-100 bg-sky-50/60 p-4 sm:col-span-2 sm:grid-cols-2">
          <label className="grid gap-1.5 text-sm font-semibold text-slate-800">Client<select name="clientSelection" required value={clientSelection} onChange={(event) => { setClientSelection(event.target.value); setRequestId(""); setError("") }} className="min-h-11 rounded-lg border border-slate-300 bg-white px-3 text-sm"><option value="">Choose client</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.name}{client.email ? ` - ${client.email}` : ""}</option>)}</select></label>
          <label className="grid gap-1.5 text-sm font-semibold text-slate-800">Client request<select name="requestId" required value={requestId} disabled={!clientSelection} onChange={(event) => { setRequestId(event.target.value); setError("") }} className="min-h-11 rounded-lg border border-slate-300 bg-white px-3 text-sm disabled:bg-slate-100"><option value="">{clientSelection ? "Choose request / case" : "Choose client first"}</option>{clientRequests.map((request) => <option key={request.id} value={request.id}>{request.caseNumber} · {request.title}{request.projectName ? ` · ${request.projectName}` : ""}</option>)}</select></label>
