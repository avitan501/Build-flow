...
function Actions({ primary, secondary, dark = false, compact = false }: { primary: string; secondary: string; dark?: boolean; compact?: boolean }) {
  ...
  return <div className={`relative mt-7 flex flex-col gap-2.5 sm:ml-0 sm:flex-row ${compact ? "w-[82%] max-w-[30rem]" : "-ml-2 w-[calc(100vw-1.5rem)] max-w-none sm:w-full sm:max-w-xl"}`}>
...
export function HomepageConceptPreview({ initialConceptId = 1, reviewOnly = false }: { initialConceptId?: number; reviewOnly?: boolean }) {
  ...
  return <main className="min-h-screen bg-[#030507] text-white">
    ...
    <section className={`relative min-h-[100svh] overflow-hidden ${conceptId === 5 ? "bg-[#edf2f7] text-slate-950" : "bg-black"}`}>
      ...
      {conceptId === 2 ? <><div className="absolute inset-0 bg-[linear-gradient(90deg,#f4efe6_0%,#f4efe6_42%,rgba(244,239,230,.95)_58%,rgba(244,239,230,.18)_100%)]" /><div className="relative z-10 flex min-h-[100svh] items-center px-5 pt-16 text-slate-950 sm:w-[54%] sm:px-10 lg:px-16"><div className="max-w-2xl"><p className="mb-3 inline-flex border-b-2 border-slate-950 pb-2 text-xs font-black uppercase tracking-[.2em]">Before / after</p>{text}<Actions primary={concept.primary} secondary={concept.secondary} dark compact={reviewOnly} /><p className="mt-4 text-xs font-semibold text-slate-500">One business day review.</p></div></div></> : null}
