...
export function ManagerNotificationControl({ navigation = false }: { navigation?: boolean }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supported ? navigator.serviceWorker.register("/sw.js").then((registration) => registration.pushManager.getSubscription()) : Promise.resolve(null),
  ...
  return <>
    <button type="button" onClick={() => setOpen(true)} className={navigation ? "flex min-h-12 w-full items-center gap-3 border-t border-slate-100 px-1 text-sm font-semibold text-slate-800 hover:text-[#0066cc]" : "inline-flex min-h-10 items-center gap-2 rounded-md border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-800 shadow-sm hover:border-sky-300"} aria-label="Phone notification settings"><Bell className="h-4 w-4 text-[#0071e3]" />Notifications</button>
    {open ? <div className="fixed inset-0 z-[170] grid place-items-end bg-slate-950/40 sm:place-items-center" role="dialog" aria-modal="true" aria-labelledby="notification-settings-title" onMouseDown={(event) => { if (event.currentTarget === event.target) setOpen(false); }}>
      <section className="max-h-[92dvh] w-full overflow-y-auto rounded-t-xl bg-white p-5 shadow-2xl sm:max-w-md sm:rounded-lg sm:p-6">
        ...
        {!supported ? <div className="mt-5 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm font-semibold text-amber-900">This browser does not support website notifications. Use Safari on iPhone or Chrome, Edge, or Safari on a computer.</div> : null}
        <div className="mt-5 flex items-center gap-3 rounded-md border border-slate-200 p-3"><span className={`inline-flex h-10 w-10 items-center justify-center rounded-md ${subscribed ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{subscribed ? <Bell className="h-5 w-5" /> : <BellOff className="h-5 w-5" />}</span><div className="min-w-0 flex-1"><p className="text-sm font-semibold">{subscribed ? "Active on this device" : "Not active on this device"}</p><p className="text-xs text-slate-500">{status ? `${status.deviceCount} connected device${status.deviceCount === 1 ? "" : "s"}` : "Checking devices..."}</p></div></div>
        <button type="button" disabled={loading || !supported || !status} onClick={subscribed ? disable : enable} className="mt-3 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-md bg-slate-950 px-4 text-sm font-semibold text-white disabled:opacity-50">{loading ? <LoaderCircle className="h-4 w-4 animate-spin" /> : subscribed ? <BellOff className="h-4 w-4" /> : <Smartphone className="h-4 w-4" />}{subscribed ? "Turn off on this device" : "Enable on this device"}</button>
