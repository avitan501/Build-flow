...
function emailLink(email: string, subject: string, message: string) {
...
export function RequestManagementPanel({
  ...
  return (
    <section className="mt-4 rounded-lg border border-slate-200 bg-white p-5" aria-labelledby="request-management-heading">
        ...
        <div className="grid gap-4">
          <div className="rounded-lg border border-slate-200 p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {client.email ? <a href={emailLink(client.email, `Avantia Build request: ${requestTitle}`, clientMessage)} className={actionClass}><Mail className="h-4 w-4" />Email client</a> : null}
          ...
          <div className="rounded-lg border border-slate-200 p-4">
            ...
            {supplier ? <div className="mt-3 flex flex-wrap gap-2">
              {supplier.email ? <a href={emailLink(supplier.email, `Pricing request: ${requestTitle}`, supplierMessage)} className={actionClass}><Mail className="h-4 w-4" />Email supplier</a> : null}
      ...
      {feedback ? <p className="mt-4 rounded-lg border border-sky-200 bg-sky-50 px-4 py-3 text-sm font-semibold text-sky-900" role="status">{feedback}</p> : null}
    </section>
  )
}
