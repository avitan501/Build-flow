...
export function WebsiteDefectInbox({ issues, checks }: { issues: WebsiteDefectRecord[]; checks: WebsiteQaCheckRecord[] }) {
  ...
  return <div>
    ...
    <section className="mt-6"><div className="flex items-end justify-between gap-3"><div><h2 className="inline-flex items-center gap-2 text-xl font-bold text-slate-950"><ClipboardCheck className="h-4 w-4 text-[#0066cc]" />Required website checks</h2><p className="mt-1 text-xs text-slate-500">Run each journey and save the result.</p></div><span className="rounded-lg bg-white px-3 py-2 text-xs font-bold text-slate-600">{checks.filter((check) => check.lastResult === "pass").length}/{checks.length} passed</span></div><div className="mt-3 grid gap-3 md:grid-cols-2">{checks.map((check) => <QaCheckCard key={check.id} check={check} />)}</div></section>
