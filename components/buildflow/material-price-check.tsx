...
export function MaterialPriceCheck({ requestId, query, department, defaultZipCode = "11516", onClose }: { requestId?: string; query: string; department: string; defaultZipCode?: string; onClose: () => void }) {
  ...
  const storeSearches = links.filter((link) => !["Home Depot", "Lowe's"].includes(link.label) && !directDomains.has(linkDomain(link.url))).slice(0, Math.max(0, 3 - buyNow.length))
  ...
  return <div className="mt-3 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
    ...
    {hasSearched ? <div className="p-3 sm:p-4">
      ...
      {majorRetailers.length ? <section className="mt-4" aria-labelledby="major-retailers-heading"><div className="mb-2 flex items-center gap-2"><Store className="h-4 w-4 text-[#0066cc]" /><h4 id="major-retailers-heading" className="text-sm font-bold text-slate-900">Major retailers</h4><span className="text-xs text-slate-500">Exact price not shown until verified</span></div><div className="grid gap-2 sm:grid-cols-2">{majorRetailers.map((link) => <a key={link.label} href={link.url} target="_blank" rel="noopener noreferrer" className="flex min-h-14 items-center justify-between gap-3 rounded-md border border-sky-200 bg-sky-50/50 px-4 text-sm font-bold text-sky-900 hover:border-sky-400 hover:bg-sky-50"><span>{link.label}</span><span className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#0066cc]">Check online <ExternalLink className="h-4 w-4" /></span></a>)}</div></section> : null}
      {totalOptions ? <div className="mt-4 grid gap-4 lg:grid-cols-3">
        <section className="min-w-0" aria-labelledby="buy-now-heading"><div className="mb-2 flex items-center gap-2"><ShoppingCart className="h-4 w-4 text-emerald-600" /><h4 id="buy-now-heading" className="text-sm font-bold">Prices & store options</h4><span className="ml-auto text-xs font-bold text-slate-500">{buyingOptions}</span></div><div className="grid gap-2">{buyNow.slice(0, 3).map((result, index) => { const badge = matchBadge(result.matchScore); return <a key={result.url} href={result.url} target="_blank" rel="noopener noreferrer" className="group rounded-md border border-slate-200 p-3 hover:border-emerald-300"><div className="flex items-start gap-2"><span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded bg-emerald-50 text-xs font-bold text-emerald-700">{index + 1}</span><div className="min-w-0 flex-1"><p className="line-clamp-2 text-sm font-bold leading-5">{result.title}</p><p className="mt-1 truncate text-xs text-slate-500">{result.domain}</p></div><ExternalLink className="h-4 w-4 shrink-0 text-slate-400" /></div><div className="mt-2 flex items-center justify-between"><strong className="text-base text-slate-950">{result.priceText}</strong><span className={`text-xs font-bold uppercase ${badge.className}`}>{badge.label}</span></div></a> })}{storeSearches.map((link, index) => <a key={link.url} href={link.url} target="_blank" rel="noopener noreferrer" className="group rounded-md border border-dashed border-slate-300 bg-slate-50 p-3 hover:border-[#0071e3]"><div className="flex items-start gap-2"><span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded bg-white text-xs font-bold text-slate-600">{buyNow.slice(0, 3).length + index + 1}</span><div className="min-w-0 flex-1"><p className="text-sm font-bold leading-5">Search {link.label}</p><p className="mt-1 text-xs text-slate-500">Store search near ZIP {zipCode}</p></div><ExternalLink className="h-4 w-4 shrink-0 text-slate-400" /></div><div className="mt-2 flex items-center justify-between"><span className="text-xs font-semibold text-slate-600">Price not confirmed</span><span className="text-xs font-bold uppercase text-[#0066cc]">Search store</span></div></a>)}</div></section>
