...
const MAJOR_RETAILER_LABELS = new Set(["The Home Depot", "Home Depot", "Lowe's"])
...
export function MaterialPriceCheck({ requestId, query, department, defaultZipCode = "11516", onClose }: { requestId?: string; query: string; department: string; defaultZipCode?: string; onClose: () => void }) {
  ...
  const majorRetailers = links.filter((link) => MAJOR_RETAILER_LABELS.has(link.label))
  const storeSearches = links.filter((link) => !MAJOR_RETAILER_LABELS.has(link.label) && !directDomains.has(linkDomain(link.url))).slice(0, Math.max(0, 3 - buyNow.length))
  const buyingOptions = buyNow.slice(0, 3).length + storeSearches.length
  const totalOptions = totalResults + storeSearches.length + majorRetailers.length
  ...
  <section className="max-h-[94dvh] w-full max-w-6xl overflow-y-auto overscroll-contain rounded-xl border border-slate-200 bg-white shadow-[0_28px_90px_rgba(15,23,42,0.38)]">
    ...
    {hasSearched ? <div className="p-3 sm:p-4">
      <div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm text-slate-600">{notice}</p>{checkedAt ? <span className="text-xs font-semibold text-slate-500">Checked {formatSiteDate(checkedAt, { month: "numeric", day: "numeric", year: "numeric" })}</span> : null}</div>
      {majorRetailers.length ? <section className="mt-4" aria-labelledby="major-retailers-heading"><div className="mb-2 flex flex-wrap items-center gap-2"><Store className="h-4 w-4 text-[#0066cc]" /><h4 id="major-retailers-heading" className="text-sm font-bold text-slate-900">Direct retailer searches</h4><span className="text-xs text-slate-500">Exact price not shown until verified</span></div><AffiliateDisclosure className="mb-2" /><div className="grid gap-2 sm:grid-cols-2">{majorRetailers.map((link) => <a key={link.label} href={link.url} target="_blank" rel="noopener noreferrer" aria-label={`Search ${link.label} for ${itemQuery} (opens in a new tab)`} className="flex min-h-14 items-center justify-between gap-3 rounded-md border border-sky-200 bg-sky-50/50 px-4 text-sm font-bold text-sky-900 hover:border-sky-400 hover:bg-sky-50"><span>Search {link.label}</span><span className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#0066cc]">Check online <ExternalLink className="h-4 w-4" aria-hidden="true" /></span></a>)}</div></section> : null}
      {totalOptions ? <div className="mt-4 grid gap-4 lg:grid-cols-3">
        <section className="min-w-0" aria-labelledby="buy-now-heading"><div className="mb-2 flex items-center gap-2"><ShoppingCart className="h-4 w-4 text-emerald-600" /><h4 id="buy-now-heading" className="text-sm font-bold">Prices & store options</h4><span className="ml-auto text-xs font-bold text-slate-500">{buyingOptions}</span></div><AffiliateDisclosure className="mb-2" /><div className="grid gap-2">{buyNow.slice(0, 3).map((result, index) => { const badge = matchBadge(result.matchScore); return <a key={result.url} href={result.url} target="_blank" rel="noopener noreferrer" aria-label={`Open ${result.title} at ${result.domain} (opens in a new tab)`} className="group rounded-md border border-slate-200 p-3 hover:border-emerald-300"><div className="flex items-start gap-2"><span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded bg-emerald-50 text-xs font-bold text-emerald-700">{index + 1}</span><div className="min-w-0 flex-1"><p className="line-clamp-2 text-sm font-bold leading-5">{result.title}</p><p className="mt-1 truncate text-xs text-slate-500">{result.domain}</p></div><ExternalLink className="h-4 w-4 shrink-0 text-slate-400" aria-hidden="true" /></div><div className="mt-2 flex items-center justify-between"><strong className="text-base text-slate-950">{result.priceText}</strong><span className={`text-xs font-bold uppercase ${badge.className}`}>{badge.label}</span></div></a> })}{storeSearches.map((link, index) => <a key={link.url} href={link.url} target="_blank" rel="noopener noreferrer" aria-label={`Search ${link.label} for ${itemQuery} near ZIP ${zipCode} (opens in a new tab)`} className="group rounded-md border border-dashed border-slate-300 bg-slate-50 p-3 hover:border-[#0071e3]"><div className="flex items-start gap-2"><span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded bg-white text-xs font-bold text-slate-600">{buyNow.slice(0, 3).length + index + 1}</span><div className="min-w-0 flex-1"><p className="text-sm font-bold leading-5">Search {link.label}</p><p className="mt-1 text-xs text-slate-500">Store search near ZIP {zipCode}</p></div><ExternalLink className="h-4 w-4 shrink-0 text-slate-400" aria-hidden="true" /></div><div className="mt-2 flex items-center justify-between"><span className="text-xs font-semibold text-slate-600">Price not confirmed</span><span className="text-xs font-bold uppercase text-[#0066cc]">Search store</span></div></a>)}</div></section>

        <section className="min-w-0" aria-labelledby="call-price-heading"><div className="mb-2 flex items-center gap-2"><Store className="h-4 w-4 text-amber-600" /><h4 id="call-price-heading" className="text-xs font-bold">Call for price</h4><span className="ml-auto text-[10px] font-bold text-slate-400">{callForPrice.length}</span></div><div className="grid gap-2">{callForPrice.slice(0, 3).map((result, index) => { const badge = matchBadge(result.matchScore); return <div key={result.url} className="rounded-md border border-slate-200 p-2.5"><div className="flex items-start gap-2"><span className="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded bg-amber-50 text-[10px] font-bold text-amber-700">{index + 1}</span><div className="min-w-0 flex-1"><p className="line-clamp-2 text-xs font-bold leading-4">{result.title}</p><p className="mt-1 truncate text-[10px] text-slate-500">{result.domain}</p></div><span className={`shrink-0 text-[9px] font-bold uppercase ${badge.className}`}>{badge.label}</span></div><div className="mt-2 flex gap-1.5">{result.phone ? <a href={`tel:${result.phone}`} className="inline-flex min-h-8 flex-1 items-center justify-center gap-1 rounded-md bg-slate-950 px-2 text-[10px] font-bold text-white"><Phone className="h-3 w-3" />Call</a> : null}<a href={result.url} target="_blank" rel="noopener noreferrer" aria-label={`Open source for ${result.title} at ${result.domain} (opens in a new tab)`} className="inline-flex min-h-8 flex-1 items-center justify-center gap-1 rounded-md border border-slate-300 px-2 text-[10px] font-bold"><ExternalLink className="h-3 w-3" aria-hidden="true" />Source</a></div></div> })}</div>{!callForPrice.length ? <p className="rounded-md bg-slate-50 p-3 text-xs text-slate-500">No reliable call option found.</p> : null}</section>

        <section className="min-w-0" aria-labelledby="sales-contact-heading"><div className="mb-2 flex items-center gap-2"><UsersRound className="h-4 w-4 text-violet-600" /><h4 id="sales-contact-heading" className="text-xs font-bold">Sales contacts</h4><span className="ml-auto text-[10px] font-bold text-slate-400">{salesContacts.length}</span></div><div className="grid gap-2">{salesContacts.slice(0, 3).map((contact, index) => <div key={`${contact.domain}-${index}`} className="rounded-md border border-slate-200 p-2.5"><div className="flex items-start gap-2"><span className="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded bg-violet-50 text-[10px] font-bold text-violet-700">{index + 1}</span><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">{contact.contactName || contact.company}</p><p className="truncate text-[10px] text-slate-500">{contact.contactName ? `${contact.role} · ${contact.company}` : contact.role}</p></div></div><div className="mt-2 flex flex-wrap gap-1.5">{contact.phone ? <a href={`tel:${contact.phone}`} className="inline-flex min-h-8 items-center gap-1 rounded-md border border-slate-300 px-2 text-[10px] font-bold"><Phone className="h-3 w-3" />Call</a> : null}{contact.email ? <a href={`mailto:${contact.email}`} className="inline-flex min-h-8 items-center rounded-md border border-slate-300 px-2 text-[10px] font-bold">Email</a> : null}<a href={contact.url} target="_blank" rel="noopener noreferrer" aria-label={`Open source page for ${contact.company} at ${contact.domain} (opens in a new tab)`} className="inline-flex min-h-8 items-center gap-1 rounded-md border border-slate-300 px-2 text-[10px] font-bold"><ExternalLink className="h-3 w-3" aria-hidden="true" />Source page</a></div></div>)}</div>{!salesContacts.length ? <p className="rounded-md bg-slate-50 p-3 text-xs text-slate-500">No public sales contact found.</p> : null}</section>
      </div> : null}
