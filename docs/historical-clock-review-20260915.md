...
Verification: eight deterministic date-boundary/seasonal-timezone/closed/open tests pass; two actual React browser tests pass (Chromium and WebKit), selecting historical summary and asserting no Working/85hr label and checkout disabled. Targeted ESLint and next typegen + standalone TypeScript pass. Browser fixtures mock actions to throw, so no financial actions occur. Browser build uses existing cached esbuild via ESBUILD_MODULE=/root/.npm/_npx/fd45a72a545557e9/node_modules/esbuild; no production dependency added.
