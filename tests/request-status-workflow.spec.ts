...
test("paid requests wait for supplier delivery before completion", () => {
  expect(QUOTE_REQUEST_PROGRESS_STEPS).toEqual([
    "Created",
    "Review",
    "Approval",
    "Supplier Delivery",
  ...
  expect(quoteRequestStatusLabel("quoted")).toBe(
...
test("manager status controls keep paid delivery and completion separate", async () => {
  const [requestStatus, workflowManager, workflowActions] = await Promise.all([
    readFile(
      path.join(
        process.cwd(),
        "components/buildflow/customer-request-status.tsx",
      ),
      "utf8",
    ),
    readFile(
      path.join(
        process.cwd(),
        "components/buildflow/project-workflow-manager.tsx",
      ),
      "utf8",
    ),
    readFile(
      path.join(process.cwd(), "app/preview-admin/workflow-actions.ts"),
      "utf8",
    ),
  ]);
