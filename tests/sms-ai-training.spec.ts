...
test("auto-safe replies use multilingual deterministic blocks and concise missing-detail rules", async () => {
  ...
  expect(broker).toContain("occurred_at >= now() - interval '120 seconds'")
  expect(broker).toContain("const exactRecentDuplicate = recentExactCustomerMessages.some")
  expect(broker).toContain('exactRecentDuplicate\n    ? "duplicate"')
