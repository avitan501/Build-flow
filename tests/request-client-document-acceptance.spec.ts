...
test("the public action derives terms server-side and rejects stale or incomplete consent", async () => {
  ...
  expect(action).toContain("const signerEmail = document.clientEmail || null")
  expect(action).toContain("p_signer_email: signerEmail")
  expect(action).not.toContain('formData.get("signerEmail")')
...
test("estimate and invoice links show one terms block and an explicit unchecked gate", async () => {
  ...
  expect(form).not.toContain('name="signerEmail"')
  expect(form).not.toContain("Signer email")
