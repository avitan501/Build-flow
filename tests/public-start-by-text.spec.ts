...
test("public text starter is fixed-copy, consented, rate-limited, and audited", async () => {
  ...
  expect(broker).toContain("await sendQuoSms(phone, PUBLIC_START_TEXT_WELCOME)")
  expect(broker).toContain("await sendQuoSms(phone, PUBLIC_START_TEXT_EXAMPLE)")
  expect(broker).toContain("EdgeRuntime.waitUntil")
  expect(broker).toContain("interval '5 minutes'")
  expect(broker).toContain('delivery: suppressed ? "already_sent" : "processing"')
  expect(broker).toContain('return json({ error: "We couldn\'t send the text.')
  expect(broker).toContain("Hi—Avantia Build here.")
  expect(broker).toContain('"Example:\\n50 sheets 5/8 regular Sheetrock\\n45 pcs 2x4x8')
