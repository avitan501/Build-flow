...
test("delete action verifies the exact version, protects accepted documents, and records activity", async () => {
  ...
  expect(actions).toContain('action: "delete_request_client_document"')
  expect(actions).not.toContain('Document deletion is not available right now')
  expect(broker).toContain('input.action === "delete_request_client_document"')
  expect(broker).toContain('not exists (')
  expect(broker).toContain('document.public_token = ${publicToken}::uuid')
  expect(broker).toContain('document.version = ${version}')
  expect(broker).toContain('request_client_document_acceptances')
