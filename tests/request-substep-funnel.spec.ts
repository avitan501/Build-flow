...
test("one tap updates only the selected substep and refreshes the request", async () => {
  ...
  expect(funnel).toContain("pending || regression")
  expect(funnel).toContain("useOptimistic(currentSubstep)")
  expect(funnel).toContain("updateRequestSubstepAction({ requestId, substep: substep.id })")
