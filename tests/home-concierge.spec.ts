...
test("cinematic homepage presents Construction Concierge clearly", async ({
  ...
  await expect(
    homepageHeader.getByRole("link", { name: "Account" }),
  ).toHaveCount(0);
  await expect(
    page.getByRole("heading", {
...
test("homepage material showroom stays compact and expandable on phones", async ({
  page,
}) => {
  test.skip(
    (page.viewportSize()?.width ?? 1024) >= 640,
    "Mobile-only showroom controls",
  );
  await page.goto("/");

  const showroom = page.getByRole("region", { name: "Shop materials" });
  await expect(
    showroom.getByText("Browse eight construction departments", {
      exact: true,
    }),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    showroom.getByRole("heading", { name: "Deals & Liquidation" }),
  ).toBeVisible();
