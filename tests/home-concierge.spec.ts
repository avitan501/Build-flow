...
test("cinematic homepage presents Construction Concierge clearly", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(
    page.getByText("Avantia Construction Concierge", { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", {
      name: "Construction Materials, Priced and Delivered.",
    }),
  ).toBeVisible();
  await expect(
    page.getByText(
      "Send us your plans or material list. We compare suppliers, organize your order, and coordinate jobsite delivery.",
      { exact: true },
    ),
  ).toBeVisible();
  await expect(
    page.getByRole("link", { name: "Start a Material Request" }),
  ).toHaveCount(0);
  await expect(
    page.getByRole("link", { name: "Start a Material Request" }),
  ).toHaveAttribute("href", "/shop", { timeout: 4000 });
  await expect(
  ...
  await expect(
    homepageHeader.getByRole("link", { name: "Log in" }),
  ).toHaveCount(0);
  ...
  await expect(
    heroMobileVideo.locator('source[type="video/mp4"]'),
  ).toHaveAttribute("src", "/videos/avantia-hero-background-v13-mobile.mp4");
  await expect(heroMobileVideo).toHaveAttribute(
    "poster",
    "/videos/avantia-hero-background-v13-mobile-poster.png",
  );
  await expect(heroMobileVideo).toHaveAttribute("data-loop-end", "12.25");
  await expect(
    heroDesktopVideo.locator('source[type="video/mp4"]'),
  ).toHaveAttribute("src", "/videos/avantia-hero-background-v13-desktop.mp4");
  await expect(heroDesktopVideo).toHaveAttribute(
    "poster",
    "/videos/avantia-hero-background-v13-desktop-poster.png",
  );
  await expect(heroDesktopVideo).toHaveAttribute("data-loop-end", "12.25");
  await expect(
    page.locator("main section").first().locator('img[alt="Avantia Build"]'),
  ).toHaveCount(0);
  await expect(
    page
      .locator("main section")
      .first()
      .locator("[aria-hidden=true] span.rounded-full"),
  ).toHaveCount(0);
  ...
  await expect
    ...
    .toBeLessThan(1.5);
  await expect(
    page.getByRole("button", { name: "Ver página en español" }),
  ).toHaveText("ES");
  await expect(
    page.getByRole("heading", {
      name: "One place for material pricing and delivery.",
    }),
  ).toBeVisible();
  await expect(page.locator("main video")).toHaveCount(2);
  await expect(
    page.getByRole("heading", { name: "Less purchasing work. More building." }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Serving 41 states." }),
  ).toBeVisible();
  await expect(
    page.getByTestId("coverage-map").getByTestId("coverage-dot"),
  ).toHaveCount(41);
  await expect(
    page.getByRole("heading", { name: "Shop Our Brands" }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Pause background video" }),
  ).toHaveCount(0);
  await expect(
    page.getByRole("button", { name: "Pause brand logos" }),
  ).toHaveCount(0);
...
test("homepage video reaches the top and does not reserve a hidden bottom dock", async ({
  page,
}) => {
  await page.goto("/");
...
test("desktop hero scales with the page instead of cropping at narrower widths", async ({
  page,
}) => {
...
test("homepage switches all principal sales content to Spanish", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(
    page.getByRole("heading", { name: "Materiales cotizados y entregados." }),
  ).toBeVisible();
  await expect(
    page.getByRole("link", { name: "Iniciar solicitud" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", {
      name: "Un solo lugar para precios y entrega de materiales.",
    }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Menos compras. Más construcción." }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Servicio en 41 estados." }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Marcas que conseguimos" }),
  ).toBeVisible();
  await expect(page.getByText("Shop Materials", { exact: true })).toBeVisible();
});

test("homepage material departments stay compact and expandable on phones", async ({
  page,
}) => {
  test.skip(
    (page.viewportSize()?.width ?? 1024) >= 640,
    "Mobile-only compact showroom",
  );
  await page.goto("/");
  ...
  await expect(
    showroom.getByRole("heading", { name: "Frame & Structure" }),
  ).toBeVisible();
  await expect(showroom.locator("details details")).toHaveCount(8);
  await page.evaluate(() => window.scrollTo(0, window.innerHeight));
  await expect(page.getByRole("link", { name: "Start Order" })).toHaveAttribute(
    "href",
    "/shop",
  );
});

test("Order Materials opens the full responsive service and department hub", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Don't see the exact item?" }),
  ).toBeVisible();
  await expect(
    page.getByRole("link", { name: "Find an item" }),
  ).toHaveAttribute("href", "/request-quote?request=custom-item");
  await expect(page.getByRole("link", { name: /Send a list/ })).toHaveAttribute(
    "href",
    "/request-quote",
  );
...
test("Learn More opens the cinematic nine-story Avantia experience", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: /One job.*Too many calls/ }),
  ).toBeAttached();
  await expect(
    page.getByRole("heading", { name: "What did the material actually cost?" }),
  ).toBeAttached();
  await expect(page.getByText("Plumbing", { exact: true })).toBeAttached();
  await expect(
    page.getByText("Lighting & fixtures", { exact: true }),
  ).toBeAttached();
  await expect(
    page.getByText("Special & custom items", { exact: true }),
  ).toBeAttached();
  ...
  for (const [file, caption] of [
    ...
    await expect(
      page.locator(`source[src="/videos/avantia-story/${file}.mp4"]`),
    ).toHaveCount(1);
    await expect(
      page.locator(`track[src="/videos/avantia-story/${caption}.vtt"]`),
    ).toHaveCount(1);
  }
  await expect(
    page.getByRole("link", { name: /Send a material request/ }),
  ).toHaveAttribute("href", "/shop");
  await expect(
    page.getByRole("link", { name: /Text \(516\) 908-8319/ }),
  ).toHaveAttribute("href", "tel:+15169088319");
  await expect(
    page
      .getByRole("main")
      .getByRole("link", { name: "office@build.avantiap.com" }),
  ).toHaveAttribute("href", "mailto:office@build.avantiap.com");

  await page
    .getByRole("button", { name: "Play How many calls for one job?" })
    .click();
  await expect
    .poll(async () =>
      page
        .locator("main video")
        .evaluateAll((items) => items.filter((item) => !item.paused).length),
    )
    .toBeLessThanOrEqual(1);

  const cinematicWidth = await page
    .getByRole("main")
    .evaluate((main) => main.getBoundingClientRect().width);
  expect(cinematicWidth).toBeCloseTo(
    page.viewportSize()?.width ?? cinematicWidth,
    0,
  );

  await page.locator("#problem").scrollIntoViewIfNeeded();
  await expect(stickyRequest).not.toHaveAttribute("aria-hidden", "true");
