...
test("existing-request uploader prepares signed uploads only through the authenticated action", async () => {
  ...
  expect(actions).toContain("buildProjectUploadStoragePath({ ownerId: request.owner_id, projectId: request.project_id")
  expect(actions).toContain('supabase.storage.from("project-uploads").createSignedUploadUrl(storagePath)')
  expect(actions).not.toContain('createAdminClient().storage.from("project-uploads").createSignedUploadUrl(storagePath)')
  expect(actions).toContain("createSignedUploadUrl(storagePath)")
  expect(actions).toContain("verifyRequestAttachmentStorage")
  expect(actions).toContain('const expectedStoragePrefix = `${request.owner_id}/${request.project_id}/`')
  expect(actions).toContain('const storage = supabase.storage.from("project-uploads")')
  expect(actions).toContain('supabase.from("quote_request_attachments").insert')
...
test("staff request uploads have narrowly request-bound Storage and attachment policies", async () => {
  const migration = await readFile(path.join(root, "supabase/migrations/20260904180758_allow_staff_request_attachment_uploads.sql"), "utf8")
  expect(migration).toContain("private.has_staff_capability('customers')")
  expect(migration).toContain("request.owner_id::text = (storage.foldername(storage.objects.name))[1]")
  expect(migration).toContain("request.project_id::text = (storage.foldername(storage.objects.name))[2]")
