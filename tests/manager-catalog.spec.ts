...
test("manager catalog is protected, seeded, editable, and supplier based", async () => {
  ...
  expect(workspace).toMatch(/<h1[^>]*>\s*Materials\s*<\/h1>/)
  expect(workspace).toMatch(/<button[^>]*>[\s\S]*?Import\s*<\/button>/)
  ...
  expect(workspace).toMatch(/Suppliers are filtered by the departments selected in\s*Supplier Settings\./)
  ...
  expect(workspace).toMatch(/draft\.productUrl \? \(\s*<a/)
  expect(workspace).toContain('"not_available"')
  ...
  expect(workspace).toMatch(/Order by\{" "\}\s*\{item\.unit\}/)
...
test("request estimate PDF has the branded estimate structure and a safe payment request", async () => {
  ...
  expect(panel).toContain("Never enter a full card number")
  ...
  expect(pdf).toContain("paymentCopy.heading")
