...
async function mount(page: Page, hydrate = false) {
  ...
  let serverHtml = ''
  if (hydrate) {
    const compiled = ts.transpileModule(readFileSync('components/buildflow/account-name-autosave.tsx','utf8'),{compilerOptions:{jsx:ts.JsxEmit.ReactJSX,module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2020}}).outputText
    const exports: {AccountNameAutosave?: import('react').ComponentType<{actorId:string;initialName:string;inputClass:string}>} = {}
    const nodeRequire = createRequire(resolve('package.json'))
    new Function('require','exports',compiled)((id:string)=>id==='@/app/account/name-action'?{}:nodeRequire(id),exports)
    serverHtml = renderToString(createElement(exports.AccountNameAutosave!,{actorId:'actor-a',initialName:'Carlos',inputClass:''}))
  }
  await page.route('http://name.test/',route=>route.fulfill({contentType:'text/html; charset=utf-8',body:`<meta charset="utf-8"><meta name="viewport" content="width=device-width"><div id="root">${serverHtml}</div>`}))
  await page.goto('http://name.test/')
  await page.addScriptTag({content:`(()=>{window.calls=[];const process={env:{NODE_ENV:'production'}},modules=[${modules.map(code=>`function(module,exports,require){${code}}`).join(',')}],cache={};function require(id){if(cache[id])return cache[id].exports;const m=cache[id]={exports:{}};modules[id](m,m.exports,require);return m.exports}const R=require(${react}),C=require(${component});let root;const element=(actor='actor-a')=>R.createElement(C.AccountNameAutosave,{key:actor,actorId:actor,initialName:actor==='actor-a'?'Carlos':'David',inputClass:''});window.showActor=(actor='actor-a')=>root.render(element(actor));window.startHydration=()=>{root=require(${dom}).hydrateRoot(document.getElementById('root'),element())};${hydrate ? '' : `root=require(${dom}).createRoot(document.getElementById('root'));window.showActor()`}})()`})
}
...
test('server-rendered name cannot be edited until hydration connects autosave',async({page})=>{
  const errors:string[]=[];page.on('pageerror',error=>errors.push(error.message))
  page.on('console',message=>{if(message.type()==='error')errors.push(message.text())})
  await mount(page,true)
  const input=page.getByLabel('Name',{exact:true})
  await expect(input).toBeDisabled();await expect(page.getByRole('status')).toHaveText('Loading name…')
  expect((await calls(page)).length).toBe(0)
  await page.evaluate(()=>(window as unknown as {startHydration:()=>void}).startHydration())
  await expect(input).toBeEnabled();await expect(input).toHaveValue('Carlos')
  await input.fill('Carlos Updated');await input.blur()
  await expect.poll(async()=>(await calls(page)).length).toBe(1)
  expect((await calls(page))[0].expectedName).toBe('Carlos')
  await finish(page,{ok:true,name:'Carlos Updated'});await expect(page.getByRole('status')).toHaveText('Saved')
