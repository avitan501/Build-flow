...
test("each incoming SMS can be reviewed and converted into a Carlos request", async () => {
  ...
  expect(workspace).toContain("The customer must explicitly confirm the exact list before one request is created")
  expect(workspace).toContain("Create one request")
