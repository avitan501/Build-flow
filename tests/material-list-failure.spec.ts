...
test("provider budget covers body read and remains below worker and cron budgets", async () => {
  ...
  const budgetMigration = await readFile("supabase/migrations/20260911025047_align_material_list_processing_timeouts.sql", "utf8")
  expect(budgetMigration).toContain("timeout_milliseconds := 135000")
  expect(organizer).toContain("connect_timeout: 5")
  expect(organizer).toContain("query.cancel()")
  expect(organizer).toContain("clearTimeout(timer)")
