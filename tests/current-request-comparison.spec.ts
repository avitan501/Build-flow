...
test('fresh comparison matrix shows all supplier columns, clean requested text and no document overflow',async({page})=>{
  ...
  for(const width of [1440,390]){
    ...
    await expect(page.getByTestId('toggle-unverified-dots')).toHaveText('Show yellow dots')
...
test('request review carries verbatim source-backed original separately from organized wording',()=>{
 const raw=source('raw',{ai_organized:false,request_details:'First floor\n22 pc 10 inch TJI 230 — 16 ft'})
 const product=source('client',{source_item_id:'raw',request_item_fields:[{id:'custom-section',label:'Section',value:'First floor'}]})
 const view=currentRequestComparison([raw,product],[stored('active','client')])
 expect(view.originalLines.active).toBe('22 pc 10 inch TJI 230 — 16 ft')
 expect(view.materialLines.active).not.toBe(view.originalLines.active)
})
