...
test("Next actions persist the job before returning and only nudge the worker after the response", async () => {
  ...
  expect(scheduler).not.toContain("invokeDirectFallback")
  expect(scheduler).toContain('queued: false as const, status: "failed" as const')
...
test("the request screen shows queue, processing, and retry states while continuing to poll", async () => {
  ...
  expect(status).toContain("useRequestRefresh(active ?")
  expect(await source("lib/use-request-refresh.ts")).toContain("router.refresh()")
