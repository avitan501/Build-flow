...
test("Quo fast ingress is authenticated, bounded, and idempotent", async () => {
  ...
  expect(broker).toContain("for (let cycle = 0; cycle < 5; cycle += 1)")
  expect(broker).toContain("if (cycle < 4)")
  expect(broker).toContain("cycles: 5")
  ...
  expect(broker).toContain("if (cycleIngested > 0) break")
