...
test("Quo fast ingress is authenticated, bounded, and idempotent", async () => {
  ...
  const ingestionFunction = broker.slice(broker.indexOf("async function ingestPolledQuoMessage"), broker.indexOf("async function pollRecentQuoMessagesOnce"))
  ...
  expect(ingestionFunction).toContain('route: "quo_outgoing_poll"')
