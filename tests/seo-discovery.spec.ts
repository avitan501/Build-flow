...
test("publishes crawler rules and protects private areas", async ({ request }) => {
  ...
  expect(robots).toContain("Sitemap: https://avantiabuild.com/sitemap.xml")
})
...
test("publishes only customer-facing canonical pages in the sitemap", async ({ request }) => {
  ...
  expect(sitemap).toContain("<loc>https://avantiabuild.com</loc>")
  expect(sitemap).toContain("<loc>https://avantiabuild.com/shop</loc>")
  expect(sitemap).toContain("<loc>https://avantiabuild.com/beat-a-quote</loc>")
