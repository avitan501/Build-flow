...
test("review and organization use one compact grouped item workspace", async () => {
  ...
  expect(worktable).not.toContain("<table")
  expect(worktable).toContain("Copy original")
  expect(worktable).toContain("Copy AI")
  expect(worktable).toContain("AI split {aiItems.length} items")
  ...
  expect(worktable).toContain('label: "REF"')
  expect(worktable).toContain("product_image_verified === true")
  expect(worktable).toContain('attachment.file_type?.startsWith("image/")')
  expect(page).toContain('select("id,item_id,material_response_id,file_name,file_path,file_type,file_size,source_party")')
  expect(worktable).toContain("min-h-14")
...
test("supplier progress persists centrally and refreshed staff screens do not keep the first local snapshot", async () => {
  ...
  expect(actions).toContain('action: "save_request_supplier_status"')
  expect(actions).toContain('data?.error || "The supplier status could not be saved."')
...
test("AI controls and notes appear only on rows that are actually missing information", async () => {
  ...
  expect(worktable).toContain("MaterialReviewEditor")
  expect(worktable).toContain('materialReviewStatus(item) !== "ready"')
  expect(worktable).toContain("Nothing is missing. This item is ready.")
...
test("supplier quotes move into compact step two instead of expanding the request grid", async () => {
  ...
  expect(worktable).toContain("Choose supplier route")
  ...
  expect(management).toContain("Upload File or Photo")
  expect(management).toContain("Open supplier comparison")
...
test("request item edits save explicitly while supplier routes autosave safely", async () => {
  ...
  expect(originalEditor).toContain("Save changes")
  expect(originalEditor).toContain("updateOrganizedMaterialItemAction")
  expect(originalEditor).toContain('mode === "add" ? <Plus')
...
test("client contact is available at the request header and does not consume step four", async () => {
  ...
  expect(management).toContain("RelatedEmailTimeline title=\"Client messages\"")
  expect(management).toContain('party="client"')
...
test("request header is compact and request text edits directly on click", async () => {
  ...
  expect(worktable).toContain('itemKind={hasAi ? "organized" : "original"}')
