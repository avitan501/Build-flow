...
test("shop exposes ordering answers, contact options, and policy links", async ({ page }) => {
  ...
  await expect(page.locator("#shop-faq").getByRole("link", { name: "Call (516) 990-1990" })).toHaveAttribute("href", "tel:+15169901990")
...
test("concrete and masonry offers one-yard bags and supplied bagged-material pricing", async ({ page }) => {
  ...
  const sandCard = page.getByRole("heading", { name: "1 Yard Sand Bag" }).locator("..")
  await expect(sandCard).toContainText("$54.00")
  await expect(page.getByText("1-yard bag fee: $35")).toBeVisible()
  await expect(page.getByText("Delivery fee: $250")).toBeVisible()
  await expect(page.getByText("Crushed stone and mulch pricing is confirmed before delivery.")).toHaveCount(0)
  await expect(page.getByLabel("Sand bag price breakdown")).toHaveCount(0)
...
test("shop category hub stays compact and opens into material cards", async ({ page }) => {
  await page.goto("/shop")
  const departments = page.locator("main details[id]")
  ...
  await departments.first().locator("summary").click()
  await expect(page.getByRole("link", { name: /Framing systems/ })).toBeVisible()
  await expect(page.getByRole("link", { name: /Lumber & plywood/ })).toBeVisible()
...
test("shop header keeps the compact Avantia wordmark beside material search", async ({ page }) => {
  await page.goto("/shop")

  const header = page.getByTestId("site-header")
  const logo = header.getByRole("link", { name: "Avantia Build home" })
  const search = header.getByRole("button", { name: /Search materials/ })

  await expect(logo).toBeVisible()
  await expect(logo).toHaveText("Avantia")
  ...
  const logoSurface = await logo.evaluate((element) => getComputedStyle(element).backgroundColor)
  expect(headerSurface.backgroundImage).toBe("none")
  expect(headerSurface.backgroundColor).not.toBe("rgba(0, 0, 0, 0)")
  expect(logoSurface).toBe("rgba(0, 0, 0, 0)")
  ...
  const narrowHeader = await header.evaluate((element) => {
    ...
    const boxes = row ? Array.from(row.children).map((child) => child.getBoundingClientRect()).filter((box) => box.width > 0) : []
    return {
...
test("department categories use distinct construction photos", async ({ page }) => {
  await page.goto("/shop")
  await page.locator("main details[id]").first().locator("summary").click()
  const cardImages = page.locator("main details[id]").first().getByRole("img")
...
test("customer-facing department names are clear and organized", async ({ page }) => {
  await page.goto("/shop")
  await expect(page.getByRole("heading", { name: "Walls & Surfaces" })).toBeVisible()
  await expect(page.getByRole("heading", { name: "Floors, Doors & Millwork" })).toBeVisible()
  await page.locator("main details[id]").filter({ hasText: "Walls & Surfaces" }).locator("summary").click()
  await expect(page.getByRole("link", { name: /Drywall/ })).toBeVisible()
  await expect(page.getByRole("link", { name: /Tile/ })).toBeVisible()
})

test("category hub omits retired labels and stays visually clean", async ({ page }) => {
  await page.goto("/shop")
  await expect(page.getByText("Eitan", { exact: true })).toHaveCount(0)
  await expect(page.getByText("Services", { exact: true })).toHaveCount(0)
  const summaries = page.locator("main details[id] > summary")
...
test("footer has the complete Avantia Build contact lockup", async ({ page }) => {
  ...
  await expect(footer.getByRole("link", { name: "office@avantiabuild.com" })).toHaveAttribute("href", "mailto:office@avantiabuild.com")
  await expect(footer.getByRole("link", { name: "(516) 990-1990" })).toHaveAttribute("href", "tel:+15169901990")
  const whatsappLink = footer.getByRole("link", { name: "WhatsApp us" })
  await expect(whatsappLink).toHaveAttribute("href", "https://wa.me/15169901990?text=Hi%2C%20I%27d%20like%20to%20send%20my%20material%20list%20or%20current%20quote%20so%20you%20can%20check%20if%20there%27s%20a%20better%20price.")
...
test("traffic dashboard exposes live status to approved operations managers", async () => {
  ...
  expect(identityMigration).toContain("private.site_traffic_ingest_config")
...
test("siding and roofing are separate departments with a complete request flow", async ({ page }) => {
  await page.goto("/shop")
  await page.locator("main details[id]").filter({ hasText: "Exterior Envelope" }).locator("summary").click()
...
test("every active shop department shows common materials", async ({ page }) => {
  await page.goto("/shop")
  const departments = await page.locator("main details[id] a[href^='/shop/']").evaluateAll((links) =>
    links
  ...
  for (const department of [...new Set(departments)].filter((href) => href !== "/shop/appliances")) {
  ...
  await expect(page.getByRole("heading", { name: "Available appliances" })).toBeVisible()
...
test("sheetrock uses the compact on-page contractor configurator", async ({ page }) => {
  ...
  await expect(page.getByText("Choose materials", { exact: true })).toHaveCount(0)
  const featuredPrices = page.getByTestId("sheet-rock-price-guide")
  await expect(featuredPrices.getByRole("heading", { name: "Sheet Rock and Compound" })).toBeVisible()
  await expect(featuredPrices.getByText("5/8 in. Type X Firecode Drywall", { exact: true })).toBeVisible()
  await expect(featuredPrices.getByText("5/8 in. Mold Tough Type X Drywall", { exact: true })).toBeVisible()
  await expect(featuredPrices.getByText("Blue / Green Joint Compound", { exact: true })).toBeVisible()
  await expect(featuredPrices.getByText("Bulk", { exact: true })).toHaveCount(3)
  await expect(featuredPrices.getByText("$11.25", { exact: true })).toBeVisible()
  await expect(featuredPrices.getByText("$15.50", { exact: true })).toBeVisible()
  await expect(featuredPrices.getByText("$18.25", { exact: true })).toBeVisible()
  await expect(featuredPrices.getByText(/Jack's Building Materials/)).toHaveCount(0)
  await expect(featuredPrices.getByText(/Quote #2331/)).toHaveCount(0)
  ...
  await expect(page.getByText("$1.00 per sheet", { exact: true })).toBeVisible()
  ...
  await expect(page.getByRole("button", { name: "Drywall screws Tap to add" })).toBeVisible()
  ...
  await expect(page.getByText("Compound color", { exact: true })).toHaveCount(0)
  await page.getByRole("button", { name: "All-purpose joint compound Tap to add" }).click()
  await expect(page.getByText("Compound color", { exact: true })).toBeVisible()
  await page.getByRole("button", { name: "Blue", exact: true }).click()
  await expect(page.getByRole("button", { name: "Blue", exact: true })).toHaveAttribute("aria-pressed", "true")
  await page.reload()
  await expect(page.getByRole("button", { name: "All-purpose joint compound Tap to add" })).toBeVisible()
  await expect(page.getByText("Compound color", { exact: true })).toHaveCount(0)
...
test("door and molding reveals the matching order fields", async ({ page }) => {
  ...
  await expect(page.getByLabel("Molding profile code")).toHaveCount(0)
...
test("shop showroom keeps one contact action and a direct custom-item path", async ({ page }) => {
  await page.goto("/shop")
