...
test("sheetrock uses the compact on-page contractor configurator", async ({ page }) => {
  ...
  await expect(featuredPrices.getByText("Contractor price", { exact: true })).toHaveCount(3)
  ...
  await expect(featuredPrices.getByText(/Jack's Building Materials/).filter({ visible: true })).toBeVisible()
