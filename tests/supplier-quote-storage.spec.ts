...
test("manager supplier quote storage is private, durable, and routable", async () => {
  ...
  expect(uploadForm).toContain("extractDocumentTextInBrowser")
  ...
  expect(browserExtraction).toContain('import("pdfjs-dist/legacy/build/pdf.mjs")')
  expect(browserExtraction).toContain('pdfjs.GlobalWorkerOptions.workerSrc = "/pdf.worker.min.mjs"')
  expect(await readFile(path.join(root, "lib/supplier-quote-extraction.ts"), "utf8")).toContain("using browser-extracted text instead")
