...
test("manager can create, assign, and archive a structured request on behalf of a client", async () => {
  ...
  expect(component).toContain("Add photos or files")
  expect(component).toContain('multiple className="sr-only"')
  ...
  expect(actions).toContain('supabase.rpc("staff_create_client_request_once"')
  expect(actions).toContain("p_idempotency_key: idempotencyKey")
  ...
  expect(customerPage).toContain("activeFilteredRequests")
  expect(customerPage).toContain("archivedFilteredRequests")
  expect(customerPage).toContain("ArchivedRequestRestoreButton")
  expect(customerPage).toContain('open={status === "closed"}')
  expect(customerPage).toContain("No active requests match. Archived requests are available below.")
  ...
  expect(requestActions).toContain("addRequestAttachmentsAction")
  expect(requestActions).toContain('from("quote_request_attachments").insert')
  ...
  expect(statusControl).toContain("Payment received")
...
test("staff can attach documents and photos to an existing request", async () => {
  const uploader = await readFile(path.join(root, "components/buildflow/request-attachment-uploader.tsx"), "utf8")
  expect(uploader).toContain("Add documents or photos")
  expect(uploader).toContain("multiple")
  expect(uploader).toContain("uploadToSignedUrl")
  expect(uploader).toContain("addRequestAttachmentsAction")
  expect(uploader).toContain("router.refresh()")
  expect(uploader).toContain("Uploading and attaching")
  expect(uploader).not.toContain("Attach ${files.length} to request")
})
...
test("manager reply composer supports templates attachments email and text", async () => {
  ...
  expect(panel).not.toContain("Request Details &amp; Notes")
  expect(panel).not.toContain("Supplier pricing note")
...
test("manager can review and send one request to multiple suppliers", async () => {
  ...
  expect(panel).toContain("Contact Suppliers")
  expect(panel).toContain("Suppliers selected in Step 1")
  expect(panel).toContain("Contact {selectedSupplierNames.length")
  ...
  expect(page).toContain("supplier_route_names")
  expect(page).toContain("supplierIdentityKeys")
  ...
  expect(draft).toContain("itemIdsBySupplier")
  expect(draft).toContain("materialRequestItemIds: selectedItemIds")
...
test("supplier directory opens a compact profile dialog above the list", async () => {
  ...
  expect(manager).toContain("Way to contact")
  expect(manager).toContain("Salespeople & emails")
  expect(manager).toContain("Relationship updates")
  expect(manager).not.toContain("Automatic service routing ·")
})
