...
test("actual inbox retains subjects through conversation and channel controls and failed send", async ({ page }) => {
  ...
  const script = `(()=>{const process={env:{NODE_ENV:'production'}},modules=[${modules.map(code => `function(module,exports,require){${code}}`).join(",")}],cache={};function require(id){if(cache[id])return cache[id].exports;const m=cache[id]={exports:{}};modules[id](m,m.exports,require);return m.exports}window.fetch=async(url)=>({ok:true,json:async()=>url==='/api/admin/communications/status'?({verified:true,connections:${JSON.stringify(props.connections)}}):({})});require(${dom}).createRoot(document.getElementById('root')).render(require(${react}).createElement(require(${component}).UnifiedCommunicationInbox,${JSON.stringify(props)}));})()`
