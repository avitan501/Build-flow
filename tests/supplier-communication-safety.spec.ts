...
test("request supplier activity uses structured links across all communication channels", async () => {
  ...
  expect(page).toContain('const requestCommunicationsAvailable = !requestCommunicationError')
  expect(page).toContain('requestCommunicationsAvailable\n    ? requestCommunicationData.communications ?? []\n    : []')
  expect(page).not.toContain('throw new Error(`Could not load request communications')
